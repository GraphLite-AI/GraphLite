...
**Signature:** `FUZZY_MATCH(str1, str2, max_distance)`
...
```sql
MATCH (p:Product)
WHERE FUZZY_MATCH(p.name, 'iPhone', 2)
...
WHERE FUZZY_MATCH(customer.name, 'John Smith', 3)
...
```sql
MATCH (addr:Address)
WHERE FUZZY_MATCH(addr.street, '123 Main Street', 5)
RETURN addr.street
...
FUZZY_MATCH('test', 'test', null) -- Returns: false
```
...
**Signature:** `FUZZY_SEARCH(text, query)`
...
```sql
MATCH (doc:Document)
LET relevance = FUZZY_SEARCH(doc.content, 'machine learning')
WHERE relevance > 0.7
RETURN
    doc.title,
    doc.content,
...
WITH
    p,
    FUZZY_SEARCH(p.description, 'wireless headphones') AS score
WHERE score > 0.8
...
WITH
    article,
    FUZZY_SEARCH(article.title, 'graph database') AS title_score,
    FUZZY_SEARCH(article.abstract, 'graph database') AS abstract_score
...
```sql
FUZZY_SEARCH(null, 'query')  -- Returns: null
FUZZY_SEARCH('text', null)   -- Returns: null
FUZZY_SEARCH(null, null)     -- Returns: null
```
...
**Signature:** `CONTAINS_FUZZY(text, query, max_distance)`
...
WHERE CONTAINS_FUZZY(comment.text, 'awesome', 2)
RETURN comment.text
...
```sql
MATCH (user:User)
WHERE CONTAINS_FUZZY(user.email, '@gmail.com', 1)
RETURN user.email
...
**Signature:** `HYBRID_SEARCH(text, query)` or `HYBRID_SEARCH(text, query, exact_weight, fuzzy_weight, similarity_weight)`
...
RETURN
    doc.title,
    HYBRID_SEARCH(doc.content, 'neural networks') AS score
ORDER BY score DESC
...
**Signature:** `WEIGHTED_SEARCH(text, query, exact_weight, fuzzy_weight, similarity_weight)`
...
**Signature:** `KEYWORD_MATCH(text, 'keyword1', 'keyword2', ...)`
...
**Signature:** `KEYWORD_MATCH_ALL(text, 'keyword1', 'keyword2', ...)`
...
WHERE FUZZY_MATCH(name, 'John', 2)  -- Fast

-- Exact match detection
WHERE FUZZY_SEARCH(title, 'GraphLite')  -- Fast (early exit)
```
...
WHERE FUZZY_SEARCH(paragraph, 'search query')
```

**Slower Performance:**
```sql
-- Very long strings
WHERE FUZZY_SEARCH(entire_book_content, 'query')  -- O(millions)
...
MATCH (p:Product)
WHERE FUZZY_SEARCH(p.name, 'wireles headfones') > 0.7
RETURN p.name, p.price
ORDER BY FUZZY_SEARCH(p.name, 'wireles headfones') DESC
LIMIT 10
...
WITH
    doc,
    FUZZY_SEARCH(doc.title, 'graph databases') AS title_score,
    FUZZY_SEARCH(doc.abstract, 'graph databases') AS abstract_score,
    KEYWORD_MATCH(doc.keywords, 'graph', 'database', 'NoSQL') AS has_keywords
...
WHERE user.id <> other.id
  AND FUZZY_MATCH(user.name, other.name, 3)
...
WHERE FUZZY_MATCH(city_variant, standard.name, 2)
...
**Example:**
```sql
FUZZY_MATCH('iPhone', 'IPHONE', 0)  -- Returns: true
...
FUZZY_SEARCH('Hello World!', 'World')  -- Handles special characters
```
...
MATCH (p:Product)
WHERE FUZZY_SEARCH(p.name, 'query') > 0.8
RETURN p
```
...
WHERE doc.content CONTAINS 'GraphLite'
  AND FUZZY_SEARCH(doc.content, 'graph database') > 0.7
```
...
WITH
    product,
    FUZZY_SEARCH(product.name, 'laptop') * 0.6 +
    FUZZY_SEARCH(product.description, 'laptop') * 0.3 +
...
WHERE FUZZY_MATCH(TRIM(field), TRIM('query'), 2)
```
