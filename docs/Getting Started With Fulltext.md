# Getting Started With Fulltext (Fuzzy Search)

A step-by-step tutorial for learning GraphLite's **fuzzy search** capabilities using **Levenshtein distance** algorithms.
...
|-----------------|----------------------|
| ✅ Fuzzy matching (typo tolerance) | ❌ Prefix/suffix pattern matching |
| ✅ Similarity scoring | ❌ Wildcard patterns |
| ✅ Levenshtein distance | ❌ Regex patterns |
| ✅ Hybrid search | ❌ Tantivy indexed search |

**For pattern matching** (prefix, suffix, wildcard): See [PATTERN_MATCHING_TUTORIAL.md](../PATTERN_MATCHING_TUTORIAL.md)
**For Tantivy-indexed search**: See [FULLTEXT_PATTERN_MATCHING_SPEC.md](../FULLTEXT_PATTERN_MATCHING_SPEC.md)

---

**Related Documentation:**
- [Full Text Search Guide New.md](Full%20Text%20Search%20Guide%20New.md) - Comprehensive fuzzy search reference
- [Fuzzy Search Functions Reference.md](Fuzzy%20Search%20Functions%20Reference.md) - Technical algorithm details
- [PATTERN_MATCHING_TUTORIAL.md](../PATTERN_MATCHING_TUTORIAL.md) - Prefix/suffix/wildcard tutorial
- [PATTERN_MATCHING_GUIDE.md](../PATTERN_MATCHING_GUIDE.md) - Pattern matching reference
- [FULLTEXT_PATTERN_MATCHING_SPEC.md](../FULLTEXT_PATTERN_MATCHING_SPEC.md) - Tantivy functions spec
...
**Algorithm:** All functions in this tutorial use **Levenshtein distance** for approximate string matching.
...
MATCH (p:Paper)
WHERE FT_FUZZY_MATCH(p.abstract, 'mashine lurning', 1)
RETURN p.title;
```
...
```gql
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'mashine', 1)
  AND FT_CONTAINS_FUZZY(p.abstract, 'lurning', 2)
RETURN p.title;
...
MATCH (p:Paper)
WHERE FT_FUZZY_MATCH(p.abstract, 'artifical inteligence', 3)
RETURN p.title;
```
## Add one line why should this query doesn't return results?
...
```gql
MATCH (p:Paper)
WHERE FT_FUZZY_MATCH(p.abstract, 'machine learning', 2)
RETURN p.title;
...
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'lurning', 2)
RETURN p.title;
```
...
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'mashine', 1)
  AND FT_CONTAINS_FUZZY(p.abstract, 'lurning', 1)
RETURN p.title;
```
## Add one line why should this query doesn't return results?
...
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'mashine', 2)
  AND FT_CONTAINS_FUZZY(p.abstract, 'lurning', 2)
RETURN p.title;
```
...
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'neural', 1)
  OR FT_CONTAINS_FUZZY(p.abstract, 'network', 1)
  OR FT_CONTAINS_FUZZY(p.abstract, 'deep', 1)
RETURN p.title;
```
...
```gql
MATCH (p:Paper)
WHERE FT_FUZZY_SEARCH(p.abstract, 'neural network') > 0.7
   OR FT_FUZZY_SEARCH(p.abstract, 'deep learning') > 0.7
RETURN p.title,
       FT_FUZZY_SEARCH(p.abstract, 'neural network') AS nn_score,
       FT_FUZZY_SEARCH(p.abstract, 'deep learning') AS dl_score
...
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'graph', 1)
  AND FT_CONTAINS_FUZZY(p.abstract, 'neural', 1)
  AND FT_CONTAINS_FUZZY(p.abstract, 'network', 1)
RETURN p.title;
```
...
```gql
MATCH (p:Paper)
WHERE FT_HYBRID_SEARCH(p.abstract, 'neural networks') > 0.3
RETURN p.title,
       FT_HYBRID_SEARCH(p.abstract, 'neural networks') AS hybrid_score
ORDER BY hybrid_score DESC;
...
MATCH (p:Paper)
WHERE FT_HYBRID_SEARCH(p.abstract, 'healthcare diagnostics') > 0.4
RETURN p.title,
       FT_HYBRID_SEARCH(p.abstract, 'healthcare diagnostics') AS hybrid_score
ORDER BY hybrid_score DESC
LIMIT 10;
```
...
MATCH (p:Paper)
WHERE FT_WEIGHTED_SEARCH(p.abstract, 'machine learning', 0.7, 0.2, 0.1) > 0.5
RETURN p.title,
       FT_WEIGHTED_SEARCH(p.abstract, 'machine learning', 0.7, 0.2, 0.1) AS score
ORDER BY score DESC;
```
...
RETURN p.title,
       ...
       FT_WEIGHTED_SEARCH(p.abstract, 'machine learning', 0.2, 0.2, 0.6) AS similarity_focused
...
MATCH (p:Paper)
WHERE FT_HYBRID_SEARCH(p.abstract, 'neural networks') > 0.3
   OR FT_HYBRID_SEARCH(p.abstract, 'drug discovery') > 0.3
RETURN p.title,
       FT_HYBRID_SEARCH(p.abstract, 'neural networks') AS nn_score,
       FT_HYBRID_SEARCH(p.abstract, 'drug discovery') AS dd_score
...
WHERE FT_HYBRID_SEARCH(paper.abstract, 'quantom computting and machine lurning') > 0.3
RETURN author.name,
       author.affiliation,
       COUNT(paper) AS relevant_papers,
       AVG(FT_HYBRID_SEARCH(paper.abstract, 'quantom computting and machine lurning')) AS avg_score
...
```gql
MATCH (author:Author)-[:WROTE]->(paper:Paper)
WHERE FT_HYBRID_SEARCH(paper.abstract, 'quantum machine learning') > 0.3
RETURN author.name,
       author.affiliation,
       COUNT(paper) AS relevant_papers,
       AVG(FT_HYBRID_SEARCH(paper.abstract, 'quantum machine learning')) AS avg_score
GROUP BY author.name, author.affiliation
ORDER BY avg_score DESC, relevant_papers DESC;
...
MATCH (author:Author)-[:WROTE]->(paper:Paper)
WHERE FT_CONTAINS_FUZZY(paper.abstract, 'neural', 1)
  AND FT_CONTAINS_FUZZY(paper.abstract, 'network', 1)
RETURN author.name,
       author.affiliation,
...
WHERE a1.name < a2.name
  AND FT_HYBRID_SEARCH(p.abstract, 'graph neural networks') > 0.35
RETURN a1.name AS author1,
       a2.name AS author2,
       p.title AS collaborative_paper,
       FT_HYBRID_SEARCH(p.abstract, 'graph neural networks') AS relevance
...
MATCH (a1:Author)-[:WROTE]->(p:Paper)<-[:WROTE]-(a2:Author)
WHERE a1.name < a2.name
  AND FT_HYBRID_SEARCH(p.abstract, 'neural networks') > 0.3
RETURN a1.name AS author1,
       a2.name AS author2,
       p.title AS collaborative_paper,
       FT_HYBRID_SEARCH(p.abstract, 'neural networks') AS relevance
ORDER BY relevance DESC;
...
RETURN author.name,
       author.affiliation,
       COUNT(CASE WHEN FT_CONTAINS_FUZZY(paper.abstract, 'machine', 1) THEN 1 END) AS machine_learning_papers,
       COUNT(CASE WHEN FT_CONTAINS_FUZZY(paper.abstract, 'quantum', 1) THEN 1 END) AS quantum_papers,
       COUNT(CASE WHEN FT_CONTAINS_FUZZY(paper.abstract, 'neural', 1) THEN 1 END) AS neural_papers,
...
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'neural', 1)
  OR FT_CONTAINS_FUZZY(p.abstract, 'quantom', 1)
RETURN p.title;
```
...
MATCH (p:Paper)
WHERE (FT_CONTAINS_FUZZY(p.abstract, 'graph', 1) AND FT_CONTAINS_FUZZY(p.abstract, 'network', 1))
  OR (FT_CONTAINS_FUZZY(p.abstract, 'deep', 1) AND FT_CONTAINS_FUZZY(p.abstract, 'learning', 1))
RETURN p.title;
```
...
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'mashine', 1)
  AND NOT FT_CONTAINS_FUZZY(p.abstract, 'statistics', 1)
RETURN p.title;
...
MATCH (p:Paper)
WHERE FT_HYBRID_SEARCH(p.abstract, 'learning algorithms') > 0.4
  AND NOT FT_CONTAINS_FUZZY(p.abstract, 'quantum', 1)
  AND NOT FT_CONTAINS_FUZZY(p.abstract, 'biology', 1)
RETURN p.title,
       FT_HYBRID_SEARCH(p.abstract, 'learning algorithms') AS score
ORDER BY score DESC;
```
...
```gql
MATCH (p:Paper)
WHERE FT_FUZZY_SEARCH(p.abstract, 'learning') > 0.7
  AND NOT FT_CONTAINS_FUZZY(p.abstract, 'quantum', 1)
RETURN p.title,
       FT_FUZZY_SEARCH(p.abstract, 'learning') AS score
ORDER BY score DESC;
...
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'neural', 1)
  AND (FT_CONTAINS_FUZZY(p.abstract, 'network', 1) OR FT_CONTAINS_FUZZY(p.abstract, 'networks', 1))
  AND NOT FT_CONTAINS_FUZZY(p.abstract, 'convolutional', 1)
RETURN p.title;
```
...
WHERE p.title CONTAINS 'Learning'
  AND FT_CONTAINS_FUZZY(p.abstract, 'mashine', 2)
RETURN p.title;
```
...
MATCH (p:Paper)
WHERE FT_KEYWORD_MATCH(p.abstract, 'Python', 'JavaScript', 'Java')
RETURN p.title;
```
## Add one line why should this query doesn't return results?
...
```gql
MATCH (p:Paper)
WHERE FT_KEYWORD_MATCH(p.abstract, 'learning', 'neural', 'quantum', 'attention')
RETURN p.title;
...
```gql
MATCH (p:Paper)
WHERE FT_WEIGHTED_SEARCH(p.abstract, 'federated lurning privasy', 0.7, 0.2, 0.1) > 0.6
RETURN p.title,
       FT_WEIGHTED_SEARCH(p.abstract, 'federated lurning privasy', 0.7, 0.2, 0.1) AS final_score
...
```gql
MATCH (p:Paper)
WHERE FT_WEIGHTED_SEARCH(p.abstract, 'federated learning privacy', 0.7, 0.2, 0.1) > 0.4
RETURN p.title,
       FT_WEIGHTED_SEARCH(p.abstract, 'federated learning privacy', 0.7, 0.2, 0.1) AS final_score
ORDER BY final_score DESC;
...
```gql
MATCH (p:Paper)
WHERE FT_CONTAINS_FUZZY(p.abstract, 'federated', 2)
  AND FT_CONTAINS_FUZZY(p.abstract, 'lurning', 2)
  AND FT_CONTAINS_FUZZY(p.abstract, 'privasy', 2)
RETURN p.title,
       FT_WEIGHTED_SEARCH(p.abstract, 'federated learning privacy', 0.7, 0.2, 0.1) AS score
ORDER BY score DESC
LIMIT 15;
```
## Add one line why should this query doesn't return results?
...
WHERE FT_CONTAINS_FUZZY(p.abstract, 'federated', 2)
  AND FT_CONTAINS_FUZZY(p.abstract, 'lurning', 2)
RETURN p.title,
       FT_WEIGHTED_SEARCH(p.abstract, 'federated learning', 0.7, 0.2, 0.1) AS score
ORDER BY score DESC;
-- Returns: Federated Learning: Privacy-Preserving ML

-- Or search title as well:
MATCH (p:Paper)
WHERE (FT_CONTAINS_FUZZY(p.abstract, 'federated', 2) OR FT_CONTAINS_FUZZY(p.title, 'federated', 2))
  AND (FT_CONTAINS_FUZZY(p.abstract, 'learning', 2) OR FT_CONTAINS_FUZZY(p.title, 'learning', 2))
  AND (FT_CONTAINS_FUZZY(p.abstract, 'privacy', 2) OR FT_CONTAINS_FUZZY(p.title, 'privacy', 2))
RETURN p.title;
-- Returns: Federated Learning: Privacy-Preserving ML
```
...
FT_WEIGHTED_SEARCH(text, query, 0.7, 0.2, 0.1)  -- 70% exact, 20% fuzzy, 10% similarity

-- For flexible matching: emphasize fuzzy and similarity
FT_WEIGHTED_SEARCH(text, query, 0.2, 0.5, 0.3)  -- 20% exact, 50% fuzzy, 30% similarity
```
...
WHERE content CONTAINS 'keyword'  -- Fast pre-filter
  AND FT_FUZZY_SEARCH(content, $query) > 0.6  -- Then fuzzy search

-- Use LIMIT to stop early
MATCH (d:Document)
WHERE FT_FUZZY_SEARCH(d.content, $query) > 0.6
RETURN d.title
ORDER BY FT_FUZZY_SEARCH(d.content, $query) DESC
...
MATCH (d:Document)
RETURN d.title,
       FT_WEIGHTED_SEARCH(d.content, $query, 0.7, 0.2, 0.1) AS exact_focused,
       FT_WEIGHTED_SEARCH(d.content, $query, 0.3, 0.5, 0.2) AS fuzzy_focused,
       FT_WEIGHTED_SEARCH(d.content, $query, 0.2, 0.3, 0.5) AS similarity_focused
