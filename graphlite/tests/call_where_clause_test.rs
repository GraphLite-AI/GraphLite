...
fn test_call_with_where_clause() {
    ...
    assert!(result.rows.len() >= 3,
        "Should have at least 3 schemas, got {}", result.rows.len());
    ...
    if result.rows.len() != 1 {
        ...
        let schema_names: Vec<String> = result.rows.iter()
            .filter_map(|row| row.values.get("schema_name").and_then(|v| v.as_string()).map(|s| s.to_string()))
            .collect();

        assert!(schema_names.contains(&"test_schema_a".to_string()),
        ...
        assert_eq!(schema_name, "test_schema_a",
    ...
    if result.rows.len() != 0 {
        ...
        assert!(!schema_names.contains(&"non_existent_schema".to_string()),
...
fn test_call_where_with_or_conditions() {
    ...
    if result.rows.len() != 2 {
        ...
        assert!(schema_names.contains(&"or_test_1".to_string()),
            "or_test_1 should be in results");
        assert!(schema_names.contains(&"or_test_2".to_string()),
...
fn test_call_where_demonstrates_bug() {
    ...
    assert_eq!(result.rows.len(), 1, "WHERE should filter to exactly 1 schema");
