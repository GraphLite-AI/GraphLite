...
fn test_simple_rollback_debug() {
    ...
    fixture.setup_graph("rollback_debug_test")
        .expect("Failed to setup graph");

    // Create a person
    let insert_result = fixture.query("INSERT (:Person {name: 'Dave', age: 40})")
        .expect("Failed to create person");

    // Verify person exists before transaction
    let before_result = fixture.query("MATCH (p:Person {name: 'Dave'}) RETURN p.age as age")
        .expect("Query before transaction should succeed");
    assert_eq!(before_result.rows.len(), 1);
    assert_eq!(before_result.rows[0].values.get("age"), Some(&Value::Number(40.0)));
    ...
    let mid_result = fixture.query("MATCH (p:Person {name: 'Dave'}) RETURN p.age as age")
        .expect("Query during transaction should succeed");
    assert_eq!(mid_result.rows.len(), 1);
    assert_eq!(mid_result.rows[0].values.get("age"), Some(&Value::Number(41.0)));
    ...
    let after_result = fixture.query("MATCH (p:Person {name: 'Dave'}) RETURN p.age as age, p.name as name");
    
    match after_result {
        Ok(result) => {
            if result.rows.len() > 0 {
                assert_eq!(result.rows[0].values.get("age"), Some(&Value::Number(40.0)), "Age should be rolled back to 40");
            } else {
