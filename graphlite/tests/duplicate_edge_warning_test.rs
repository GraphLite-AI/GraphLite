...
fn test_duplicate_edge_shows_warning() {
    ...
    // Setup graph context
    fixture.setup_graph("duplicate_edge_test").expect("Failed to setup graph");

    // Setup: Create two people
    fixture.query("INSERT (:Person {name: 'Charlie'}), (:Person {name: 'Diana'})")
    ...
    assert_eq!(result1.rows_affected, 1, "First insertion should affect 1 row");
    assert!(result1.warnings.is_empty(), "First insertion should have no warnings");
    ...
    assert!(result2.warnings[0].contains("Duplicate edge detected"),
        "Warning should mention 'Duplicate edge detected', got: {}", result2.warnings[0]);
    ...
    let count_result = fixture.query(
        "MATCH ()-[r:KNOWS]->() RETURN COUNT(r) AS edge_count"
    ...
    assert_eq!(edge_count, 1.0,
        "Should have exactly 1 edge after duplicate insertion attempt, found: {}", edge_count);
}
...
fn test_duplicate_edge_with_regular_insert() {
    ...
    // Setup graph context
    fixture.setup_graph("regular_insert_test").expect("Failed to setup graph");

    // First insertion with inline nodes - should succeed
    let result1 = fixture.query(
        "INSERT (:User {id: 'u1'})-[:FOLLOWS {since: '2020-01-01'}]->(:User {id: 'u2'})"
    ...
    assert!(!result2.warnings.is_empty(), "Should have warnings for duplicates");

    // Verify we have node duplicate warnings
    let has_node_warnings = result2.warnings.iter()
        .any(|w| w.contains("Duplicate node detected"));

    assert!(has_node_warnings,
        "Should have 'Duplicate node detected' warnings, got: {:?}", result2.warnings);
