...
fn test_storage_directory_structure() {
    ...
    assert!(db_path.join("db").exists(), "Sled database file should exist");
    assert!(db_path.join("conf").exists(), "Sled config should exist");
    assert!(db_path.join("wal").exists(), "WAL directory should exist");
    assert!(db_path.join("blobs").exists(), "Blobs directory should exist");
...
fn test_wal_directory_structure() {
    ...
    coordinator.process_query("CREATE SCHEMA /test_schema", &session_id)
        .expect("Failed to create schema");

    // Verify WAL directory structure
    let wal_dir = db_path.join("wal");
    assert!(wal_dir.exists(), "WAL directory should exist");
    assert!(wal_dir.join("catalog").exists(), "Catalog WAL directory should exist");
...
fn test_user_data_storage_separation() {
...

    {
        ...
        coordinator.process_query("CREATE SCHEMA /company", &session_id)
            .expect("Failed to create schema");
        coordinator.process_query("CREATE GRAPH /company/employees", &session_id)
            .expect("Failed to create graph");
        coordinator.process_query("SESSION SET GRAPH /company/employees", &session_id)
    ...
    let db_file_metadata = fs::metadata(db_path.join("db"))
        .expect("Failed to get db file metadata");
    assert!(db_file_metadata.len() > 1000,
...
fn test_multi_graph_tree_isolation() {
...

    {
        ...
        coordinator.process_query("CREATE SCHEMA /schema1", &session_id)
            .expect("Failed to create schema1");
        coordinator.process_query("CREATE GRAPH /schema1/graph1", &session_id)
            .expect("Failed to create graph1");

        coordinator.process_query("CREATE SCHEMA /schema2", &session_id)
            .expect("Failed to create schema2");
        coordinator.process_query("CREATE GRAPH /schema2/graph2", &session_id)
            .expect("Failed to create graph2");

        // Insert data into first graph (set context first)
        coordinator.process_query("SESSION SET GRAPH /schema1/graph1", &session_id)
        ...
        coordinator.process_query("SESSION SET GRAPH /schema2/graph2", &session_id)
            .expect("Failed to set graph2 context");
        coordinator.process_query(
            "INSERT (n:Company {name: 'TechCorp'})",
            &session_id
    ...
    assert!(db_file_metadata.len() > 2000,
...
fn test_no_unwanted_data_directories() {
    ...
    coordinator.process_query("CREATE SCHEMA /test", &session_id)
        .expect("Failed to create schema");
    coordinator.process_query("CREATE GRAPH /test/graph", &session_id)
        .expect("Failed to create graph");
...
fn test_catalog_and_user_data_coexist() {
    ...
    assert!(db_path.exists(), "Database directory should exist");
    assert!(db_path.join("db").exists(), "All data should be in single database file");
