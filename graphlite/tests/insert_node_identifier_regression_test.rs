...
fn create_regression_test_fixture() -> Result<TestFixture, Box<dyn std::error::Error>> {
    ...
    let graph_name = format!("regression_test_{}",
...
fn test_insert_with_identifier_reuse_basic() {
    let fixture = create_regression_test_fixture()
        .expect("Failed to create regression test fixture");
    ...
    let result = fixture.query("MATCH (n) RETURN n.name ORDER BY n.name")
    ...
    assert!(names.contains(&"Alice Smith".to_string()), "Should contain Alice Smith");
    assert!(names.contains(&"Bob Johnson".to_string()), "Should contain Bob Johnson");
...
fn test_no_empty_labels() {
    ...
    let result = fixture.query("MATCH (n) WHERE size(labels(n)) = 0 RETURN count(n) as empty_nodes")
...
fn test_content_based_deduplication() {
    ...
    // Count initial nodes
    let result1 = fixture.query("MATCH (n) RETURN count(n) as total_nodes")
        .expect("Count query should succeed");
    let initial_count = result1.rows[0].values["total_nodes"].as_number().unwrap();

    // Try to insert duplicate
    fixture.query(r#"INSERT (n:Person {name: "Alice Smith"})"#)
        .expect("Duplicate INSERT should succeed");

    // Verify count didn't increase
    let result2 = fixture.query("MATCH (n) RETURN count(n) as total_nodes")
        .expect("Count query should succeed");
    let final_count = result2.rows[0].values["total_nodes"].as_number().unwrap();

    assert_eq!(initial_count, final_count, "Duplicate INSERT should not increase node count");
...
fn test_complex_pattern_with_multiple_identifier_reuses() {
    ...
    let result = fixture.query("MATCH (n) RETURN count(n) as total_nodes")
        .expect("Count query should succeed");
    ...
    let names_result = fixture.query("MATCH (n:Person) RETURN n.name ORDER BY n.name")
        .expect("Names query should succeed");

    for row in &names_result.rows {
    }

    assert_eq!(names_result.rows.len(), 3, "Should find exactly 3 Person nodes");

    let names: Vec<String> = names_result.rows.iter()
        .map(|row| row.values["n.name"].as_string().unwrap().to_string())
        .collect();
...
fn test_anonymous_nodes_with_content() {
    ...
    let anon_result = fixture.query("MATCH (n:Anonymous) RETURN n.type, n.value ORDER BY n.type")
        .expect("Anonymous nodes query should succeed");
    assert_eq!(anon_result.rows.len(), 2, "Should find exactly 2 Anonymous nodes");
}
...
fn test_hub_and_spoke_pattern() {
    ...
    let node_count = result.rows[0].values["total_nodes"].as_number().unwrap();
    assert_eq!(node_count, 4.0, "Should have exactly 4 nodes (1 hub + 3 spokes)");
    ...
    assert_eq!(edge_result.rows.len(), 3, "Should find exactly 3 hub-to-spoke edges");

    let spokes: Vec<String> = edge_result.rows.iter()
        .map(|row| row.values["spoke"].as_string().unwrap().to_string())
        .collect();

    assert!(spokes.contains(&"Spoke1".to_string()), "Should connect to Spoke1");
    assert!(spokes.contains(&"Spoke2".to_string()), "Should connect to Spoke2");
    assert!(spokes.contains(&"Spoke3".to_string()), "Should connect to Spoke3");
...
fn test_no_spurious_nodes_comprehensive() {
    ...
    fixture.query(r#"INSERT (:Anonymous {type: "Test"}) -[:LINKS]-> (:Anonymous {type: "Other"})"#)
    ...
    let total_result = fixture.query("MATCH (n) RETURN count(n) as total_nodes")
        .expect("Total count query should succeed");

    let total_count = total_result.rows[0].values["total_nodes"].as_number().unwrap();
...
fn test_person_nodes_all_have_names() {
    ...
    fixture.query(r#"INSERT (n:Person {name: "Alice"}), (m:Person {name: "Bob"}), (n) -[r:KNOWS]-> (m)"#)
        .expect("INSERT should succeed");
    ...
    let result = fixture.query("MATCH (n:Person) WHERE n.name IS NULL RETURN count(n) as null_names")
    ...
    assert_eq!(names_result.rows.len(), 4, "Should have exactly 4 Person nodes");
