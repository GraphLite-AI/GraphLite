...
fn test_insert_operations() {
    ...
    fixture.setup_graph("test_insert_operations").expect("Failed to setup graph");

    // Test basic node insertion
    fixture.assert_query_succeeds(
        "INSERT (new_node:Person {name: 'John Doe', age: 30})"
    );
    ...
    fixture.assert_query_succeeds(
        "INSERT (company:Company {name: 'TechCorp'})"
    );
    ...
    fixture.assert_query_succeeds(
        "INSERT (ai_proj:Project {name: 'AI System'})"
    );
...
fn test_set_operations() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_set_operations").expect("Failed to setup graph");
...
fn test_remove_operations() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_remove_operations").expect("Failed to setup graph");
...
fn test_count_aggregation_with_empty_results() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_count_aggregation").expect("Failed to setup graph");
    ...
    let working_result = fixture.query("MATCH (n:TestNode) RETURN count(n) as count").unwrap();
    ...
    assert_eq!(result.rows.len(), 1, "COUNT should always return exactly 1 row, even with 0 matching nodes");
    let count_value = result.rows[0].values.get("count").unwrap();
    assert_eq!(count_value, &Value::Number(0.0), "COUNT of non-existent nodes should be 0");
...
fn test_delete_operations() {
    ...
    fixture.query(&format!("CREATE GRAPH /{}/delete_test_graph", fixture.schema_name())).unwrap();
    fixture.query(&format!("SESSION SET GRAPH /{}/delete_test_graph", fixture.schema_name())).unwrap();
    ...
    assert_eq!(result.rows.len(), 1, "COUNT should always return exactly 1 row");
    let count = result.rows[0].values.get("count").unwrap();
    assert_eq!(count, &Value::Number(0.0), "Expected count to be 0 after deletion");
    ...
    fixture.assert_query_succeeds(
        "INSERT (src:Source {id: 1}), (dst:Destination {id: 2})"
    );
...
fn test_dml_data_driven_cases() {
    ...
    assert!(results.passed >= 4, "Should have at least 4 passing DML tests");
}
...
fn test_simple_match_set() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_simple_match_set").expect("Failed to setup graph");
...
fn test_complex_dml_scenarios() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_complex_dml_scenarios").expect("Failed to setup graph");
    // Re-insert fraud data since we have a fresh graph
    fixture.insert_fraud_data().expect("Failed to insert fraud data");
...
fn test_dml_transaction_behavior() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    fixture.setup_graph("test_transaction_behavior").expect("Failed to setup graph");
    ...
    fixture.assert_query_succeeds(
        "INSERT (tx_test:TransactionTest {name: 'TX Data', value: 100})"
    );
    ...
    fixture.assert_query_succeeds(
        "INSERT (commit_test:CommitTest {name: 'Committed Data'})"
    );
...
fn test_dml_error_cases() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    fixture.setup_graph("test_error_cases").expect("Failed to setup graph");

    // Test invalid INSERT syntax
    fixture.assert_query_fails(
        "INSERT (invalid syntax here)",
        "Parse error"
    );
    
    // Test SET on non-existent nodes (should succeed but affect 0 nodes)
    fixture.assert_query_succeeds(
        "MATCH (n:NonExistentLabel) SET n.prop = 'value'"
    );
    
    // Test REMOVE on non-existent properties (should succeed)
    fixture.assert_query_succeeds(
        "INSERT (test:RemoveTest {prop1: 'value'})"
    );
    
    fixture.assert_query_succeeds(
        "MATCH (test:RemoveTest) REMOVE test.non_existent_prop"
    );
    ...
    */
    ...
    fixture.assert_query_succeeds(
        "INSERT (type_test:TypeTest {number_prop: 42})"
    );
    
    // This may or may not fail depending on type coercion rules
    let result = fixture.query(
        "MATCH (t:TypeTest) SET t.number_prop = 'string_value'"
    );
...
fn test_dml_performance() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_dml_performance").expect("Failed to setup graph");
    ...
    for batch in 0..10 {
        ...
        for i in 0..10 {
            ...
            clauses.push(format!(
                ...
                node_id, node_id, batch, node_id * 2
            ));
        }
        ...
        let result = fixture.query("MATCH (p:PerfNode) RETURN count(p) as count").unwrap();
        if !result.rows.is_empty() {
            let count = result.rows[0].values.get("count").unwrap_or(&Value::Number(0.0));
        } else {
        }
    }
    ...
    if result.rows.is_empty() {
        ...
        let count = result.rows[0].values.get("count").unwrap();
        assert_eq!(count, &Value::Number(100.0), "Expected 100 nodes to be inserted");
    }
    ...
    if result.rows.is_empty() {
        ...
        let count = result.rows[0].values.get("count").unwrap();
        assert_eq!(count, &Value::Number(50.0), "Expected 50 nodes to remain after deletion");
    }
    
    assert!(insert_duration.as_secs() < 10, "Bulk insert should complete within 10 seconds");
    assert!(update_duration.as_secs() < 5, "Bulk update should complete within 5 seconds");
    assert!(delete_duration.as_secs() < 5, "Bulk delete should complete within 5 seconds");
}
