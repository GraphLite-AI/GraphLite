...
fn get_security_fixture() -> &'static TestFixture {
    SECURITY_FIXTURE.get_or_init(|| {
        TestFixture::empty().expect("Failed to create security test fixture")
    })
}
...
fn test_role_listing() {
    ...
    assert_eq!(result.variables.len(), 3, "Should have 3 columns: role_name, description, created_at");
    ...
    let role_names: Vec<String> = result.rows.iter()
        .filter_map(|row| {
            row.values.get("role_name")
                .and_then(|v| match v {
    ...
    assert!(role_names.contains(&"test_role_1".to_string()), "Should contain test_role_1");
    assert!(role_names.contains(&"test_role_2".to_string()), "Should contain test_role_2");
    assert!(role_names.contains(&"test_role_3".to_string()), "Should contain test_role_3");
...
fn test_user_lifecycle() {
    ...
    fixture.assert_query_fails(
        "CREATE USER 'alice' PASSWORD 'newpass'",
        "already exists"
    );

    // Test CREATE USER IF NOT EXISTS (may not be implemented yet)
    let if_not_exists_result = fixture.query("CREATE USER IF NOT EXISTS 'alice' PASSWORD 'ignored'");
    if if_not_exists_result.is_ok() {
...
fn test_user_listing() {
    ...
    let if_not_exists_result = fixture.query("CREATE USER IF NOT EXISTS 'test_user_1' PASSWORD 'pass1'");
    if if_not_exists_result.is_ok() {
    ...
    assert_eq!(result.variables.len(), 5, "Should have 5 columns: username, email, active, created_at, roles");
    ...
    let usernames: Vec<String> = result.rows.iter()
        .filter_map(|row| {
            row.values.get("username")
                .and_then(|v| match v {
                    Value::String(s) => Some(s.clone()),
                    _ => None,
                })
    ...
    assert!(usernames.contains(&"test_user_1".to_string()), "Should contain test_user_1");
    assert!(usernames.contains(&"test_user_2".to_string()), "Should contain test_user_2");
    assert!(usernames.contains(&"test_user_3".to_string()), "Should contain test_user_3");
...
fn test_role_assignment() {
    ...
    let if_not_exists_result = fixture.query("CREATE USER IF NOT EXISTS 'testuser' PASSWORD 'password'");
...
fn test_authentication() {
    ...
    // Create a test user
    let if_not_exists_result = fixture.query("CREATE USER IF NOT EXISTS 'authtest' PASSWORD 'testpass123'");
    if if_not_exists_result.is_err() {
    ...
    match auth_result {
        Ok(result) => {
            ...
            // Verify the result structure
            assert!(!result.rows.is_empty(), "Authentication should return user info");
...
fn test_security_edge_cases() {
    ...
    if if_not_exists_result.is_ok() {
        ...
        let long_role_name = "very_long_role_name_that_exceeds_normal_length_limits_to_test_boundary_conditions";
    ...
    assert!(role_names.contains(&"CaseTest".to_string()), "Should contain CaseTest");
    assert!(role_names.contains(&"casetest".to_string()), "Should contain casetest");
}
...
fn test_transaction_integrity() {
    ...
    let role_names: Vec<String> = result.rows.iter()
        ...
        .collect();
    assert!(role_names.contains(&"tx_test_role".to_string()), "Role should exist in transaction");
    ...
    let role_names_after: Vec<String> = result_after_rollback.rows.iter()
        .filter_map(|row| {
            row.values.get("role_name")
                .and_then(|v| match v {
                    Value::String(s) => Some(s.clone()),
    ...
    let role_names_committed: Vec<String> = result_after_commit.rows.iter()
        .filter_map(|row| {
            row.values.get("role_name")
                .and_then(|v| match v {
                    Value::String(s) => Some(s.clone()),
        ...
        .collect();
    assert!(role_names_committed.contains(&"committed_role".to_string()), "Role should persist after commit");
}
