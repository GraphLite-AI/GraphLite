...
fn test_readme_pattern_matching_setup() {
    ...
    fixture.setup_graph("readme_pattern_test")
...
fn test_readme_friends_of_friends() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_fof_test")
        .expect("Failed to setup graph");
    ...
    assert!(result.rows.len() >= 1, "Should find at least one friend of friend");
    ...
    assert!(names.contains(&"Carol".to_string()) || names.contains(&"Dave".to_string()),
...
fn test_readme_variable_length_paths() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_varlen_test")
        .expect("Failed to setup graph");
...
fn test_readme_multiple_patterns_coworkers() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_coworkers_test")
        .expect("Failed to setup graph");
    ...
    assert!(result.rows.len() > 0, "Should find coworkers who know each other");
...
fn test_readme_insert_simple_node() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_insert_test")
        .expect("Failed to setup graph");

    // Insert nodes and relationships
    fixture.query("INSERT (n:Person {name: 'Charlie', age: 35})")
        .expect("Simple INSERT should succeed");

    // Verify
    let result = fixture.query("MATCH (p:Person {name: 'Charlie'}) RETURN p.age")
        .expect("Query should succeed");

    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].values.get("p.age"), Some(&Value::Number(35.0)));
...
fn test_readme_insert_with_functions() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_func_insert_test")
        .expect("Failed to setup graph");
    ...
    // Verify
    let result = fixture.query("MATCH (p:Product) RETURN p.name, p.price, p.warranty")
        .expect("Query should succeed");

    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].values.get("p.name"), Some(&Value::String("LAPTOP".to_string())));
    assert_eq!(result.rows[0].values.get("p.price"), Some(&Value::Number(1299.99)));
...
fn test_readme_insert_edge_with_functions() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_edge_func_test")
        .expect("Failed to setup graph");

    // Create nodes first
    fixture.query("INSERT (:Person {name: 'Alice'}), (:Person {name: 'Bob'})")
    ...
    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].values.get("r.strength"), Some(&Value::Number(1.0)));
...
fn test_readme_nested_functions_insert() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_nested_insert_test")
        .expect("Failed to setup graph");
    ...
    // Verify
    let result = fixture.query("MATCH (d:Data) RETURN d.value, d.text")
        .expect("Query should succeed");

    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].values.get("d.value"), Some(&Value::Number(43.0)));
    assert_eq!(result.rows[0].values.get("d.text"), Some(&Value::String("MIXED CASE".to_string())));
...
fn test_readme_set_node_with_functions() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_set_node_test")
        .expect("Failed to setup graph");

    // Create node
    fixture.query("INSERT (:Person {name: 'Alice', age: 30})")
    ...
    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].values.get("p.age"), Some(&Value::Number(31.0)));
    assert_eq!(result.rows[0].values.get("p.name_upper"), Some(&Value::String("ALICE".to_string())));
...
fn test_readme_set_edge_with_functions() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_set_edge_test")
        .expect("Failed to setup graph");
    ...
    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].values.get("r.strength"), Some(&Value::Number(0.9)));
    assert_eq!(result.rows[0].values.get("r.label"), Some(&Value::String("RELATIONSHIP".to_string())));
...
fn test_readme_nested_functions_set() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_nested_set_test")
        .expect("Failed to setup graph");

    // Create node
    fixture.query("INSERT (:Product {name: 'Widget', price: 100.0})")
        .expect("Insert should succeed");
    ...
    // Verify
    let result = fixture.query("MATCH (p:Product) RETURN p.price, p.name")
        .expect("Query should succeed");

    assert_eq!(result.rows.len(), 1);
    assert_eq!(result.rows[0].values.get("p.price"), Some(&Value::Number(100.0)));
    assert_eq!(result.rows[0].values.get("p.name"), Some(&Value::String("MIXED".to_string())));
...
fn test_readme_delete_node() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_delete_test")
        .expect("Failed to setup graph");

    // Create nodes
    fixture.query("INSERT (:Person {name: 'Bob'}), (:Person {name: 'Alice'})")
        .expect("Insert should succeed");

    // Delete nodes
    fixture.query("MATCH (p:Person {name: 'Bob'}) DELETE p")
        .expect("DELETE should succeed");

    // Verify Bob is deleted
    let result = fixture.query("MATCH (p:Person {name: 'Bob'}) RETURN p")
        .expect("Query should succeed");
    assert_eq!(result.rows.len(), 0, "Bob should be deleted");

    // Verify Alice still exists
    let result = fixture.query("MATCH (p:Person {name: 'Alice'}) RETURN p")
        .expect("Query should succeed");
...
fn test_readme_count_relationships() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_count_test")
        .expect("Failed to setup graph");
    ...
    if let Some(row) = alice_row {
        assert_eq!(row.values.get("friend_count"), Some(&Value::Number(2.0)),
...
fn test_readme_group_by_aggregate() {
    let fixture = TestFixture::new().expect("Failed to create fixture");
    fixture.setup_graph("readme_groupby_test")
        .expect("Failed to setup graph");
    ...
    if let Some(row) = nyc_row {
        ...
        assert_eq!(row.values.get("population"), Some(&Value::Number(3.0)),
