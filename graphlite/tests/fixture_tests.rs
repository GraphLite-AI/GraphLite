...
use testutils::test_fixture::{TestFixture, TestSuite, TestCase, FixtureType, AggregateStats};
use graphlite::Value;
...
fn test_fraud_detection_queries() {
    ...
    fixture.setup_graph("test_fraud_detection_queries").expect("Failed to setup graph");
    ...
    assert_eq!(result.rows.len(), 1, "Should have 1 merchant category (all retail)");
...
fn test_simple_graph_operations() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_simple_graph_operations").expect("Failed to setup graph");
    ...
    let test_suite = TestSuite {
        ...
        test_cases: vec![
            ...
            TestCase {
                ...
                query: "MATCH (a:Account) WHERE a.status = 'active' RETURN count(a) as count".to_string(),
                expected_rows: Some(1),
            ...
            TestCase {
                ...
                query: "MATCH (m:Merchant) WHERE m.category = 'retail' RETURN count(m) as count".to_string(),
                expected_rows: Some(1),
...
fn test_data_modification_operations() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_data_modification_operations").expect("Failed to setup graph");
    // Re-insert simple data since we have a fresh graph
    fixture.insert_simple_data().expect("Failed to insert simple data");

    // Test INSERT
    fixture.assert_query_succeeds(
        "INSERT (new:TestNode {name: 'Node 21', value: 21})"
    );
    ...
    // Test UPDATE
    fixture.assert_query_succeeds(
        "MATCH (n:TestNode) WHERE n.value = 21 SET n.updated = true"
    );
    
    let result = fixture.assert_query_succeeds(
        "MATCH (n:TestNode) WHERE n.updated = true RETURN n.value as value"
    );
    assert_eq!(result.rows.len(), 1);
    
    // Test DELETE
    fixture.assert_query_succeeds(
        "MATCH (n:TestNode) WHERE n.value = 21 DELETE n"
    );
...
fn test_complex_pattern_matching() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_complex_pattern_matching").expect("Failed to setup graph");
    // Re-insert fraud data since we have a fresh graph
    fixture.insert_fraud_data().expect("Failed to insert fraud data");
...
fn test_with_clause_basic_aggregation() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_with_clause_basic_aggregation").expect("Failed to setup graph");
    // Re-insert fraud data since we have a fresh graph
    fixture.insert_fraud_data().expect("Failed to insert fraud data");
    ...
    assert!(!result.rows.is_empty(), "Should have merchants with >3 transactions");
    
    // Verify the result has the expected columns
    assert!(result.variables.len() >= 3, "Should have at least 3 columns");
    ...
    for row in &result.rows {
        ...
        if let Some(Value::Number(count)) = row.get_value("transaction_count") {
            assert!(*count > 3.0, "All returned merchants should have >3 transactions");
        }
...
fn test_with_clause_distinct_aggregation() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_with_clause_distinct_aggregation").expect("Failed to setup graph");
    // Re-insert fraud data since we have a fresh graph
    fixture.insert_fraud_data().expect("Failed to insert fraud data");
    ...
    assert_eq!(result.rows.len(), 1, "Aggregated RETURN should produce exactly one row");
    ...
    if let Some(Value::Number(avg)) = result.rows[0].get_value("avg_merchants_per_account") {
        ...
        assert!(*avg <= 20.0, "Average should not exceed total merchant count");
    } else {
...
fn test_error_handling() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_error_handling").expect("Failed to setup graph");
    // Re-insert simple data since we have a fresh graph
    fixture.insert_simple_data().expect("Failed to insert simple data");

    // Test syntax errors
    fixture.assert_query_fails(
        "MATCH (n:TestNode WHERE n.value > 10",
        "Parse error"
    );
    ...
    // Test type errors
    fixture.assert_query_fails(
        "MATCH (n:TestNode) WHERE n.name > 100 RETURN n",
        ""
    );
}
...
fn test_transaction_consistency() {
    let fixture = TestFixture::new().expect("Failed to create test fixture");
    // Setup fresh graph for this test to avoid interference
    fixture.setup_graph("test_transaction_consistency").expect("Failed to setup graph");
    // Re-insert simple data since we have a fresh graph
    fixture.insert_simple_data().expect("Failed to insert simple data");
    ...
    // Make changes
    fixture.assert_query_succeeds(
        "INSERT (temp:TempNode {name: 'Temporary', value: 999})"
    );
...
fn test_performance_with_large_dataset() {
    let fixture = TestFixture::with_large_data(1000, 3.0)
        .expect("Failed to create large data fixture");
    
    // Test query performance on larger dataset
    let start = std::time::Instant::now();
    
    fixture.assert_query_succeeds(
        "MATCH (n:PerfNode) RETURN count(n) as count"
    );
    
    let duration = start.elapsed();
    assert!(duration.as_secs() < 5, "Query should complete within 5 seconds");
    ...
    let duration = start.elapsed();
    assert!(duration.as_secs() < 10, "Aggregation should complete within 10 seconds");
}
