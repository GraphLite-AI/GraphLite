...
fn test_graph_ddl_operations() {
    ...
    fixture.assert_query_fails(&format!(
        "CREATE GRAPH {}", 
        another_graph
    ), "already exists");
...
fn test_complex_ddl_scenarios() {
    ...
    fixture.assert_query_succeeds(&format!(
        "SESSION SET GRAPH {}/tx_graph",
        schemas[0]
    ));
    
    // Switch to different graph and verify isolation
    fixture.assert_query_succeeds(&format!(
        "SESSION SET GRAPH {}/user_graph",
        schemas[0]
    ));
...
fn test_ddl_cascade_operations() {
    ...
    fixture.assert_query_succeeds(
        "INSERT (c_test:CTest {name: 'Will be deleted'})"
    );
    ...
    fixture.assert_query_fails(&format!(
        "CREATE GRAPH {}/new_graph_after_drop",
        test_schema
    ), "not found");
...
fn test_create_graph_with_relative_path() {
    ...
    fixture.assert_query_succeeds(&format!(
        "SESSION SET GRAPH {}/relative_graph",
        test_schema
    ));
...
fn test_drop_graph_with_relative_path() {
    ...
    fixture.assert_query_succeeds(&format!(
        "CREATE GRAPH {}/test_graph_1",
        test_schema
    ));

    fixture.assert_query_succeeds(&format!(
        "CREATE GRAPH {}/test_graph_2",
        test_schema
    ));
    ...
    fixture.assert_query_fails(&format!(
        "SESSION SET GRAPH {}/test_graph_1",
        test_schema
    ), "does not exist");
...
fn test_session_graph_full_path_storage() {
    ...
    for row in &result.rows {
        if let (Some(prop_name), Some(prop_value)) = (row.values.get("property_name"), row.values.get("property_value")) {
            if let (Value::String(name), Value::String(value)) = (prop_name, prop_value) {
                if name == "current_graph" {
                    ...
                    assert!(value.contains(&format!("/{}/session_test_graph", test_schema)),
                           "Expected full path containing '/{}/session_test_graph', got: '{}'", test_schema, value);
    ...
    assert!(found_full_path, "Did not find current_graph property in show_session output");
...
fn test_drop_graph_preserves_session_schema() {
    ...
    fixture.assert_query_succeeds(&format!(
        "CREATE GRAPH {}/preservation_graph",
        test_schema
    ));
    ...
    for row in &result_before.rows {
        if let (Some(prop_name), Some(_prop_value)) = (row.values.get("property_name"), row.values.get("property_value")) {
    ...
    assert!(found_schema_after, "Schema should still be set after dropping graph");
    assert!(!found_graph_after, "Graph should be cleared after dropping graph");
