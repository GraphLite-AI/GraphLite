...
fn test_fraud_fixture_creation_debug() {
    ...
    // Test CREATE GRAPH
    let create_result = fixture.query(&format!("CREATE GRAPH /{}/fraud_graph", fixture.schema_name()));
    if let Err(e) = create_result {
        panic!("✗ CREATE GRAPH failed: {}", e);
    }

    // Test SESSION SET
    let session_result = fixture.query(&format!("SESSION SET GRAPH /{}/fraud_graph", fixture.schema_name()));
    if let Err(e) = session_result {
        panic!("✗ SESSION SET failed: {}", e);
    }

    // Test Account INSERT
    let schema_id = fixture.schema_name().replace("test_", "t").replace("_", "x");
