...
fn test_unknown_gql_procedure_error() {
    ...
    assert!(err.contains("procedure not found") || err.contains("not supported"),
        "Error should mention procedure not found, got: {}", err);

    assert!(!err.contains("No graph context"),
        "Should not get graph context error for unknown procedure, got: {}", err);

    // Should say "Available system procedures" not "ISO GQL procedures"
    assert!(err.contains("Available system procedures"),
        "Error should say 'Available system procedures', got: {}", err);
...
fn test_list_available_procedures_in_error() {
    ...
    assert!(err.contains("list_schemas") || err.contains("list_graphs"),
        "Error should list available procedures to help user, got: {}", err);
