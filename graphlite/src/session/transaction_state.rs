...
impl SessionTransactionState {
    ...
    pub fn current_transaction_id(&self) -> Result<Option<TransactionId>, ExecutionError> {
        self.current_transaction
            .read()
            .map(|guard| *guard)
            .map_err(|_| ExecutionError::RuntimeError("Failed to read transaction state".to_string()))
    }
    ...
    pub fn is_auto_commit(&self) -> Result<bool, ExecutionError> {
        self.auto_commit
            .read()
            .map(|guard| *guard)
            .map_err(|_| ExecutionError::RuntimeError("Failed to read auto-commit state".to_string()))
    }
    
    /// Set auto-commit mode
    pub fn set_auto_commit(&self, enabled: bool) -> Result<(), ExecutionError> {
        let mut auto_commit = self.auto_commit
            .write()
            .map_err(|_| ExecutionError::RuntimeError("Failed to update auto-commit state".to_string()))?;
        *auto_commit = enabled;
        Ok(())
    }
    ...
    pub fn set_isolation_level(&self, level: IsolationLevel) -> Result<(), ExecutionError> {
        let mut isolation = self.isolation_level
            .write()
            .map_err(|_| ExecutionError::RuntimeError("Failed to update isolation level".to_string()))?;
        *isolation = level;
        Ok(())
    }
    ...
    pub fn begin_transaction(&self) -> Result<TransactionId, ExecutionError> {
        ...
        let mut current = self.current_transaction
            .write()
            .map_err(|_| ExecutionError::RuntimeError("Failed to update transaction state".to_string()))?;
        *current = Some(txn_id);

        // Initialize transaction log
        let mut logs = self.transaction_logs
            .write()
            .map_err(|_| ExecutionError::RuntimeError("Failed to update transaction logs".to_string()))?;
    ...
    pub fn commit_transaction(&self) -> Result<(), ExecutionError> {
        let txn_id = self.current_transaction_id()?
            .ok_or_else(|| ExecutionError::RuntimeError("No active transaction to commit".to_string()))?;
    ...
    pub fn rollback_transaction_with_storage(&self, storage: Option<&Arc<crate::storage::StorageManager>>) -> Result<(), ExecutionError> {
        log::info!("ROLLBACK: Attempting rollback, checking for active transaction...");
        let current_txn = self.current_transaction_id()?;
        
        let txn_id = current_txn
            .ok_or_else(|| ExecutionError::RuntimeError("No active transaction to rollback".to_string()))?;
        
        // Get the transaction log for rollback operations
        let logs = self.transaction_logs
            .read()
            .map_err(|_| ExecutionError::RuntimeError("Failed to read transaction logs".to_string()))?;

        let log = logs.get(&txn_id)
        ...
        if let Some(storage_manager) = storage {
            log::info!("Applying {} undo operations for transaction {:?}", undo_operations.len(), txn_id);
            for operation in undo_operations.iter().rev() {
                if let Err(e) = self.apply_undo_operation(operation, storage_manager) {
                    log::error!("Failed to apply undo operation: {:?}. Error: {}", operation, e);
                    // Continue with other undo operations even if one fails
                }
            }
        } else {
            log::warn!("No storage manager provided for rollback - undo operations will not be applied!");
        }
    ...
    fn apply_undo_operation(&self, operation: &crate::txn::UndoOperation, storage: &Arc<crate::storage::StorageManager>) -> Result<(), ExecutionError> {
        ...
        match operation {
            UndoOperation::Batch { operations } => {
                ...
                log::info!("ROLLBACK: Processing batch with {} operations", operations.len());
                for (i, op) in operations.iter().enumerate() {
                    log::debug!("ROLLBACK: Applying batch operation {}/{}", i + 1, operations.len());
                    self.apply_undo_operation(op, storage)?;
                }
                log::info!("ROLLBACK: Successfully processed batch of {} operations", operations.len());
            },
            UndoOperation::InsertNode { graph_path, node_id } => {
                log::info!("ROLLBACK: Undoing InsertNode: graph={}, node={}", graph_path, node_id);

                // Load the graph
                let mut graph = storage.get_graph(graph_path)
                    .map_err(|e| ExecutionError::StorageError(format!("Failed to load graph for rollback: {}", e)))?
                    .ok_or_else(|| ExecutionError::StorageError(format!("Graph not found for rollback: {}", graph_path)))?;

                log::info!("ROLLBACK: Graph loaded, current node count: {}", graph.node_count().unwrap_or(0));
                ...
                match graph.remove_node(node_id) {
                    ...
                    Err(e) => {
                        log::error!("ROLLBACK: Failed to remove node {} during rollback: {}", node_id, e);
                        return Err(ExecutionError::StorageError(format!("Failed to remove node during rollback: {}", e)));
                    }
                }

                log::info!("ROLLBACK: After removal, node count: {}", graph.node_count().unwrap_or(0));

                // Save the updated graph
                storage.save_graph(graph_path, graph)
                    .map_err(|e| ExecutionError::StorageError(format!("Failed to save graph after rollback: {}", e)))?;

                log::info!("ROLLBACK: Successfully rolled back InsertNode: {}", node_id);
            },
            UndoOperation::DeleteNode { graph_path, node_id: _, deleted_node } => {
                log::debug!("Undoing DeleteNode: graph={}, node={}", graph_path, deleted_node.id);
                ...
                if let Err(e) = graph.add_node(deleted_node.clone()) {
                    log::warn!("Failed to restore node {} during rollback: {}", deleted_node.id, e);
                }
            ...
            UndoOperation::DeleteEdge { graph_path, edge_id: _, deleted_edge } => {
                log::debug!("Undoing DeleteEdge: graph={}, edge={}", graph_path, deleted_edge.id);
                ...
                if let Err(e) = graph.add_edge(deleted_edge.clone()) {
                    log::warn!("Failed to restore edge {} during rollback: {}", deleted_edge.id, e);
                }
    ...
    pub fn log_operation_to_wal(
        ...
        if let Some(txn_id) = self.current_transaction_id()? {
            ...
            self.manager.log_operation(txn_id, operation_type, description)?;
        }
    ...
    pub fn log_transaction_operation(&self, operation: UndoOperation) -> Result<(), ExecutionError> {
        if let Some(txn_id) = self.current_transaction_id()? {
            log::info!("TRANSACTION: Logging undo operation for transaction: {:?}, operation: {:?}", txn_id, operation);
            ...
            let log = logs.entry(txn_id).or_insert_with(|| TransactionLog::new(txn_id));

            // Add the operation to the log
            log.undo_operations.push(operation);

            log::info!("TRANSACTION: Transaction {} now has {} undo operations",
                      txn_id, log.undo_operations.len());
        } else {
    ...
    pub fn execute_with_auto_commit<F, R>(
        &self,
        f: F
            ...
            match f() {
                ...
                Err(e) => {
                    ...
                    if let Err(rollback_err) = self.rollback_transaction() {
                        log::error!("Failed to rollback transaction {}: {}", txn_id, rollback_err);
                    }
                    Err(e)
                }
            }
