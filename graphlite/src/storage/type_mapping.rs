...
impl TypeMapping {
    ...
    pub fn is_value_compatible_with_type(value: &Value, expected_type: &GqlType) -> bool {
        match (value, expected_type) {
            ...
            // String types
            (Value::String(s), GqlType::String { max_length }) => {
                match max_length {
            ...
            // Numeric types
            (Value::Number(_), GqlType::Integer | GqlType::BigInt | GqlType::SmallInt | 
                             GqlType::Int128 | GqlType::Int256 | GqlType::Double | 
                             GqlType::Real | GqlType::Float { .. }) => true,
            ...
            // Temporal types
            (Value::DateTime(_), GqlType::ZonedDateTime { .. } | GqlType::LocalDateTime { .. } | 
                                GqlType::Timestamp { .. } | GqlType::Date) => true,
            (Value::TimeWindow(_), GqlType::Duration { .. }) => true,
            
            // Collection types
            (Value::Array(arr), GqlType::List { element_type, max_length }) => {
                ...
                arr.iter().all(|elem| Self::is_value_compatible_with_type(elem, element_type))
            }
            (Value::Vector(_), GqlType::List { element_type, .. }) => {
                // Vectors are compatible with lists of numeric types
                matches!(element_type.as_ref(), 
    ...
    pub fn coerce_value_to_type(value: &Value, target_type: &GqlType) -> TypeResult<Value> {
        ...
        match (value, target_type) {
            ...
            (Value::String(s), GqlType::Integer | GqlType::BigInt | GqlType::SmallInt) => {
                s.parse::<i64>()
                    .map(|i| Value::Number(i as f64))
                    .map_err(|_| TypeError::InvalidCast(s.clone(), "Integer".to_string()))
            }
            (Value::String(s), GqlType::Double | GqlType::Real | GqlType::Float { .. }) => {
                s.parse::<f64>()
                    .map(Value::Number)
            ...
            (Value::Vector(vec), GqlType::List { element_type, .. }) => {
                match element_type.as_ref() {
                    GqlType::Double | GqlType::Float { .. } | GqlType::Float32 | GqlType::Real => {
                        let array_values: Vec<Value> = vec.iter().map(|&f| Value::Number(f as f64)).collect();
                        Ok(Value::Array(array_values))
                    }
                    _ => Err(TypeError::InvalidCast("Vector".to_string(), format!("List<{:?}>", element_type)))
                }
            }
                    ...
                    GqlType::Double | GqlType::Float { .. } | GqlType::Float32 | GqlType::Real => {
                        ...
                        for elem in arr {
                            match elem {
                                ...
                                _ => return Err(TypeError::InvalidCast("Array".to_string(), "Vector".to_string())),
                            }
                        }
    ...
    pub fn validate_value_types(
        ...
        let type_map: std::collections::HashMap<_, _> = expected_types.iter()
    ...
    pub fn default_value_for_type(type_spec: &GqlType) -> Value {
        match type_spec {
            ...
            GqlType::Int128 | GqlType::Int256 => Value::Number(0.0),
            GqlType::Decimal { .. } | GqlType::Float { .. } | GqlType::Float32 | GqlType::Real | GqlType::Double => Value::Number(0.0),
            GqlType::Date | GqlType::Time { .. } | GqlType::Timestamp { .. } |
            GqlType::ZonedTime { .. } | GqlType::ZonedDateTime { .. } | 
            GqlType::LocalTime { .. } | GqlType::LocalDateTime { .. } => {
                Value::DateTime(chrono::Utc::now())
            }
            GqlType::Duration { .. } => {
                Value::TimeWindow(crate::storage::TimeWindow::new(
                    chrono::Utc::now(),
                    chrono::Utc::now()
                ).unwrap())
            }
...
mod tests {
    ...
    fn test_value_type_compatibility() {
        ...
        let string_type = GqlType::String { max_length: Some(10) };
        let string_type_short = GqlType::String { max_length: Some(3) };
        
        assert!(TypeMapping::is_value_compatible_with_type(&string_val, &string_type));
        assert!(!TypeMapping::is_value_compatible_with_type(&string_val, &string_type_short));
        
        let num_val = Value::Number(42.0);
        let int_type = GqlType::Integer;
        let double_type = GqlType::Double;
        
        assert!(TypeMapping::is_value_compatible_with_type(&num_val, &int_type));
        assert!(TypeMapping::is_value_compatible_with_type(&num_val, &double_type));
    }
    ...
    fn test_type_inference() {
        ...
        assert!(matches!(inferred, GqlType::String { max_length: Some(255) }));
