...
impl NGramIndex {
    ...
    pub fn add(&mut self, doc_id: u64, text: &str) {
        ...
        for ngram in self.generate_ngrams(&padded) {
            self.ngram_to_docs.entry(ngram).or_insert_with(HashSet::new).insert(doc_id);
        }
    }
    ...
    pub fn fuzzy_search_threshold(&self, query: &str, threshold: f64, max_distance: Option<usize>) -> Vec<FuzzyMatchResult> {
        let query = query.to_lowercase();
        let mut results = Vec::new();
        
        let max_distance = max_distance.unwrap_or_else(|| {
            (query.len().max(10) as f64 * 0.3).ceil() as usize
        });
