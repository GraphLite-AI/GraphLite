...
use crate::storage::persistent::{StorageDriver, StorageTree, create_storage_driver};
...
impl StorageManager {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        info!("Creating storage manager with method: {:?}, storage type: {:?}", method, storage_type);
    ...
    fn init_disk_only<P: AsRef<Path>>(
        ...
        info!("Initializing disk-only storage with {} at path: {:?}", storage_type, path.as_ref());
        ...
        let common_trees = vec![
            "nodes", "edges", "metadata", "catalog", "auth"
        ];
        ...
        info!("Storage driver initialized with {} pre-created trees", common_trees.len());
    ...
    fn init_disk_and_memory<P: AsRef<Path>>(
        ...
        info!("Initializing disk and memory storage with {} at path: {:?}", storage_type, path.as_ref());
    ...
    pub fn get_graph(&self, name: &str) -> Result<Option<GraphCache>, StorageError> {
        ...
        debug!("Graph '{}' not in memory, checking persistent storage", name);
        ...
        if let Some(persistent_store) = &self.persistent_store {
            if let Some(driver) = &self.storage_driver {
                ...
                Err(e) => {
                    debug!("Failed to load graph '{}' from persistent storage: {}", name, e);
                }
                }
            }
        }
    ...
    pub fn save_graph(&self, name: &str, graph: GraphCache) -> Result<(), StorageError> {
        ...
        if let Some(persistent_store) = &self.persistent_store {
            if let Some(driver) = &self.storage_driver {
                ...
                persistent_store.save_graph_by_path(driver.as_ref().as_ref(), &graph, name).map_err(|e| {
                    error!("Failed to persist graph '{}': {}", name, e);
                    StorageError::PersistenceError(format!("Failed to persist graph '{}': {}", name, e))
                })?;
                debug!("Successfully persisted graph '{}' to disk", name);
            } else {
                debug!("No storage driver available, skipping disk persistence for '{}'", name);
            }
        } else {
            debug!("No persistent store available, skipping disk persistence for '{}'", name);
        }
    ...
    pub fn delete_graph(&self, name: &str) -> Result<(), StorageError> {
        ...
        if let Some(memory_store) = &self.memory_store {
            ...
            if let Some(driver) = &self.storage_driver {
                memory_store.delete_graph(driver.as_ref().as_ref(), name).map_err(|e| {
                    error!("Failed to delete graph '{}' from memory store: {}", name, e);
                    StorageError::PersistenceError(format!("Failed to delete graph '{}' from memory: {}", name, e))
                })?;
            }
        ...
        if let Some(persistent_store) = &self.persistent_store {
            ...
            if let Some(driver) = &self.storage_driver {
                persistent_store.delete_graph(driver.as_ref().as_ref(), name).map_err(|e| {
                    error!("Failed to delete graph '{}' from persistent storage: {}", name, e);
                    StorageError::PersistenceError(format!("Failed to delete graph '{}': {}", name, e))
                })?;
            }
            debug!("Successfully deleted graph '{}' from persistent storage", name);
        }
        
        debug!("Successfully deleted graph '{}' from all storage tiers", name);
        Ok(())
    }
    ...
    pub fn get_storage_driver(&self) -> Option<&Arc<Box<dyn StorageDriver<Tree = Box<dyn StorageTree>>>>> {
    ...
    pub fn save_catalog(&self, catalog_manager: &CatalogManager) -> Result<(), StorageError> {
        if let Some(persistent_store) = &self.persistent_store {
            persistent_store.save_catalog(catalog_manager)
                .map_err(|e| StorageError::PersistenceError(format!("Failed to save catalog: {}", e)))
        } else if let Some(memory_store) = &self.memory_store {
            memory_store.save_catalog(catalog_manager)
                .map_err(|e| StorageError::PersistenceError(format!("Failed to save catalog: {}", e)))
        } else {
            Err(StorageError::PersistenceError("No storage backend available".to_string()))
        }
    }
    ...
    pub fn load_catalog(&self, catalog_manager: &mut CatalogManager) -> Result<(), StorageError> {
        if let Some(persistent_store) = &self.persistent_store {
            persistent_store.load_catalog(catalog_manager)
                .map_err(|e| StorageError::PersistenceError(format!("Failed to load catalog: {}", e)))
        } else if let Some(memory_store) = &self.memory_store {
            memory_store.load_catalog(catalog_manager)
                .map_err(|e| StorageError::PersistenceError(format!("Failed to load catalog: {}", e)))
        } else {
    ...
    pub fn storage_driver(&self) -> Option<Arc<Box<dyn StorageDriver<Tree = Box<dyn StorageTree>>>>> {
    ...
    pub fn has_catalog_data(&self) -> Result<bool, StorageError> {
        if let Some(persistent_store) = &self.persistent_store {
            if let Some(driver) = &self.storage_driver {
                persistent_store.has_catalog_data(driver.as_ref().as_ref())
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to check catalog data: {}", e)))
            } else {
                Ok(false)
            }
        } else if let Some(memory_store) = &self.memory_store {
            if let Some(driver) = &self.storage_driver {
                memory_store.has_catalog_data(driver.as_ref().as_ref())
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to check catalog data: {}", e)))
            } else {
                Ok(false)
            }
    ...
    pub fn clear_all_data(&self) -> Result<(), StorageError> {
        ...
        if let Some(persistent_store) = &self.persistent_store {
            if let Some(driver) = &self.storage_driver {
                persistent_store.clear(driver.as_ref().as_ref())
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to clear persistent data: {}", e)))?;
            }
        }
        ...
        if let Some(memory_store) = &self.memory_store {
            if let Some(driver) = &self.storage_driver {
                memory_store.clear(driver.as_ref().as_ref())
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to clear memory data: {}", e)))?;
            }
        }
    ...
    pub fn save_catalog_provider(&self, name: &str, data: &[u8]) -> Result<(), StorageError> {
        if let Some(persistent_store) = &self.persistent_store {
            if let Some(driver) = &self.storage_driver {
                persistent_store.save_catalog_provider(driver.as_ref().as_ref(), name, data)
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to save catalog provider '{}': {}", name, e)))
            } else {
                Err(StorageError::PersistenceError("No storage driver available".to_string()))
            }
        } else if let Some(memory_store) = &self.memory_store {
            if let Some(driver) = &self.storage_driver {
                memory_store.save_catalog_provider(driver.as_ref().as_ref(), name, data)
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to save catalog provider '{}': {}", name, e)))
            } else {
                Err(StorageError::PersistenceError("No storage driver available".to_string()))
            }
    ...
    pub fn load_catalog_provider(&self, name: &str) -> Result<Option<Vec<u8>>, StorageError> {
        if let Some(persistent_store) = &self.persistent_store {
            if let Some(driver) = &self.storage_driver {
                ...
                Err(e) => {
                    debug!("Could not load catalog provider '{}' from persistent store: {}", name, e);
                    Ok(None)
                }
                }
            } else {
            ...
            if let Some(driver) = &self.storage_driver {
                ...
                Ok(data) => Ok(Some(data)),
                Err(e) => {
                    debug!("Could not load catalog provider '{}' from memory store: {}", name, e);
                    Ok(None)
                }
                }
            } else {
    ...
    pub fn has_catalog_provider(&self, name: &str) -> Result<bool, StorageError> {
        if let Some(persistent_store) = &self.persistent_store {
            if let Some(driver) = &self.storage_driver {
                persistent_store.has_catalog_provider(driver.as_ref().as_ref(), name)
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to check catalog provider '{}': {}", name, e)))
            } else {
                Ok(false)
            }
        } else if let Some(memory_store) = &self.memory_store {
            if let Some(driver) = &self.storage_driver {
                memory_store.has_catalog_provider(driver.as_ref().as_ref(), name)
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to check catalog provider '{}': {}", name, e)))
            } else {
                Ok(false)
            }
    ...
    pub fn list_catalog_providers(&self) -> Result<Vec<String>, StorageError> {
        if let Some(persistent_store) = &self.persistent_store {
            if let Some(driver) = &self.storage_driver {
                persistent_store.list_catalog_providers(driver.as_ref().as_ref())
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to list catalog providers: {}", e)))
            } else {
                Ok(Vec::new())
            }
        } else if let Some(memory_store) = &self.memory_store {
            if let Some(driver) = &self.storage_driver {
                memory_store.list_catalog_providers(driver.as_ref().as_ref())
                    .map_err(|e| StorageError::PersistenceError(format!("Failed to list catalog providers: {}", e)))
            } else {
                Ok(Vec::new())
            }
