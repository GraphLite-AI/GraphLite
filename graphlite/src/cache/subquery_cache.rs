...
use super::{CacheKey, CacheValue, CacheEntryMetadata, CacheLevel};
        ...
        match self {
            ...
            SubqueryResult::FullResult(result) => {
                ...
                Some(result.rows.iter().any(|row| {
                    row.positional_values.contains(value) || 
                    row.values.values().any(|v| v == value)
                }))
            }
        }
    }
...
impl CacheValue for SubqueryResult {
    fn size_bytes(&self) -> usize {
        match self {
            ...
            SubqueryResult::Scalar(Some(value)) => std::mem::size_of::<Value>() + 
                match value {
...
impl PartialEq for SubqueryCacheKey {
    fn eq(&self, other: &Self) -> bool {
        ...
        self.outer_variables.iter().zip(&other.outer_variables).all(|(a, b)| {
            a.0 == b.0 && values_equal(&a.1, &b.1)
        })
    }
}
...
fn values_equal(a: &Value, b: &Value) -> bool {
    match (a, b) {
        ...
        (Value::DateTimeWithNamedTz(tz_a, dt_a), Value::DateTimeWithNamedTz(tz_b, dt_b)) => tz_a == tz_b && dt_a == dt_b,
...
pub struct SubqueryCacheEntry {
    ...
    #[allow(dead_code)] // ROADMAP v0.5.0 - Tracks correlation complexity for cache eviction policies. Currently set (line 447) but not yet used in eviction scoring. Will be used for cost-based eviction when correlated subquery optimization is implemented.
    ...
    fn update_indices(&self, key: &SubqueryCacheKey, add: bool) {
        match key.subquery_type {
            SubqueryType::Exists | SubqueryType::NotExists => {
                ...
                if add {
                    boolean_index.entry(key.subquery_hash)
            ...
            SubqueryType::Scalar => {
                ...
                if add {
                    scalar_index.entry(key.subquery_hash)
                        .or_insert_with(Vec::new)
                        .push(key.clone());
                } else {
