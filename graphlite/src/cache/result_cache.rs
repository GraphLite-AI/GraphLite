...
use super::{CacheKey, CacheValue, CacheEntryMetadata, CacheLevel, EvictionPolicy};
...
pub struct QueryResultEntry {
    ...
    #[allow(dead_code)] // ROADMAP v0.5.0 - Compression ratio for cache statistics (see ROADMAP.md §9)
...
impl CacheValue for QueryResultEntry {
    fn size_bytes(&self) -> usize {
        ...
        let variables_size = self.result.variables.iter()
            .map(|var| var.len())
            .sum::<usize>();
    ...
    fn is_valid(&self) -> bool {
        ...
        !self.result.rows.is_empty() // Don't cache empty results
    }
}
...
impl ResultCache {
    ...
    pub fn insert(&self, key: QueryCacheKey, result: QueryResult, execution_time: Duration, plan_hash: u64) {
