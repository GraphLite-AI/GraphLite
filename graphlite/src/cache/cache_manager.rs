...
use super::{
    CacheConfig, CacheLevel,
    ResultCache, PlanCache, PlanCacheKey, PlanCacheEntry,
    SubqueryCache, SubqueryResult, SubqueryCacheKey, SubqueryType, SubqueryCacheHit, InvalidationEvent, InvalidationManager,
    result_cache::{QueryCacheKey, CacheHit, CacheParameter, create_query_cache_key, CacheStats as ResultCacheStats},
    plan_cache::{create_plan_cache_key, PlanCacheStats, CacheEfficiencyMetrics},
...
impl CacheManager {
    ...
    pub fn new(config: CacheConfig) -> Result<Self, String> {
        ...
        let plan_cache = Arc::new(PlanCache::new(
            ...
            config.l3_config.default_ttl.unwrap_or(Duration::from_secs(3600)),
        ));
        ...
        let subquery_cache = Arc::new(SubqueryCache::new(
            ...
            config.l1_config.default_ttl.unwrap_or(Duration::from_secs(300)),
        ));
        
        let invalidation_strategy = match &config.invalidation_strategy {
            super::cache_config::InvalidationStrategy::Manual => super::invalidation::InvalidationStrategy::Manual,
            super::cache_config::InvalidationStrategy::TTL => super::invalidation::InvalidationStrategy::TTL {
                default_ttl: Duration::from_secs(3600),
                max_ttl: Duration::from_secs(7200),
            },
            super::cache_config::InvalidationStrategy::TagBased => super::invalidation::InvalidationStrategy::TagBased {
                sensitive_tags: ["nodes".to_string(), "edges".to_string(), "schema".to_string()].into_iter().collect(),
                propagation_delay: Duration::from_millis(100),
            },
            super::cache_config::InvalidationStrategy::Versioned => super::invalidation::InvalidationStrategy::Versioned {
                track_data_version: true,
                track_schema_version: true,
            },
            super::cache_config::InvalidationStrategy::Hybrid { primary, fallback: _ } => {
    ...
    pub fn cache_query_result(
        ...
        self.result_cache.insert(key, result, execution_time, plan_hash);
    ...
    pub fn get_query_plan(
        ...
        let key = create_plan_cache_key(query_hash, schema_version, optimization_level, hints.clone());
    ...
    pub fn cache_query_plan(
        ...
        self.plan_cache.insert(key, logical_plan, physical_plan, trace, compilation_time);
        self.update_global_stats();
    }
    ...
    pub fn cache_subquery_result(
        ...
        self.subquery_cache.insert(key, result, execution_time, complexity_score);
        self.update_global_stats();
    }
    
    /// Find cached boolean results for EXISTS/NOT EXISTS optimization
    pub fn find_boolean_subquery_matches(&self, subquery_hash: u64) -> Vec<(SubqueryCacheKey, bool)> {
        if !self.config.enabled {
            return vec![];
        }
    ...
    pub fn invalidate_on_data_change(&self, table: Option<String>, affected_rows: u64) {
        ...
        *graph_version += 1;
        ...
        self.subquery_cache.invalidate_by_graph_version(*graph_version);
    ...
    pub fn invalidate_on_schema_change(&self, table: String, change_type: String) {
        ...
        *schema_version += 1;
        ...
        let graph_version = *self.graph_version.read().unwrap();
        self.result_cache.invalidate_by_graph_version(graph_version);
        self.subquery_cache.invalidate_by_schema_version(*schema_version);
    ...
    pub fn handle_memory_pressure(&self, current_usage: usize, max_usage: usize) {
        let event = InvalidationEvent::MemoryPressure { current_usage, max_usage };
    ...
    pub fn get_health_score(&self) -> CacheHealthScore {
        ...
        let memory_usage_ratio = stats.global.total_memory_bytes as f64 / self.config.max_memory_bytes as f64;
    ...
    fn update_global_stats(&self) {
        ...
        let mut global_stats = self.global_stats.write().unwrap();
        global_stats.total_hits = result_stats.l1_hits + result_stats.l2_hits + plan_stats.hits + subquery_stats.hits;
        ...
        global_stats.total_memory_bytes = plan_stats.current_memory_bytes + subquery_stats.memory_bytes;
    }
    ...
    fn generate_recommendations(&self, stats: &CacheManagerStats) -> Vec<String> {
        ...
        if stats.result_cache.l1_hit_rate() < 0.1 {
            recommendations.push("L1 cache hit rate is very low. Consider adjusting L1 size or TTL settings.".to_string());
        }
