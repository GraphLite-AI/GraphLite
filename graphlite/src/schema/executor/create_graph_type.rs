...
impl CreateGraphTypeExecutor {
    ...
    fn build_graph_type_definition(&self, context: &ExecutionContext) -> Result<GraphTypeDefinition, ExecutionError> {
        // Get current user from context
        let created_by = context.current_user.clone().unwrap_or_else(|| "system".to_string());

        // Use provided version or default to 1.0.0
        let version = self.statement.version.clone()
    ...
    fn validate_graph_type(&self) -> Result<(), ExecutionError> {
        ...
        for node_type in &self.statement.node_types {
            if !node_labels.insert(&node_type.label) {
                return Err(ExecutionError::ValidationError(
                    format!("Duplicate node type label: {}", node_type.label)
                ));
            }
        }
        ...
        for edge_type in &self.statement.edge_types {
            if !edge_types.insert(&edge_type.type_name) {
                return Err(ExecutionError::ValidationError(
                    format!("Duplicate edge type: {}", edge_type.type_name)
                ));
            }
        }
        ...
        for edge_type in &self.statement.edge_types {
            for from_type in &edge_type.from_node_types {
                if !node_labels.contains(from_type) {
                    return Err(ExecutionError::ValidationError(
                        format!("Edge type '{}' references unknown node type '{}' in FROM clause",
            ...
            for to_type in &edge_type.to_node_types {
                if !node_labels.contains(to_type) {
                    return Err(ExecutionError::ValidationError(
                        format!("Edge type '{}' references unknown node type '{}' in TO clause",
        ...
        for node_type in &self.statement.node_types {
            ...
            for prop in &node_type.properties {
                if !prop_names.insert(&prop.name) {
                    return Err(ExecutionError::ValidationError(
                        format!("Duplicate property '{}' in node type '{}'",
        ...
        for edge_type in &self.statement.edge_types {
            ...
            for prop in &edge_type.properties {
                if !prop_names.insert(&prop.name) {
                    return Err(ExecutionError::ValidationError(
                        format!("Duplicate property '{}' in edge type '{}'",
    ...
    fn execute_ddl_operation(
        ...
        if !self.statement.if_not_exists {
            if self.check_graph_type_exists(catalog_manager)? {
                return Err(ExecutionError::ValidationError(
                    format!("Graph type '{}' already exists", self.statement.name)
                ));
            }
        } else if self.check_graph_type_exists(catalog_manager)? {
            // IF NOT EXISTS specified and type exists - return success
            return Ok((
                format!("Graph type '{}' already exists, skipping creation", self.statement.name),
                0
            ));
        }
        ...
        let response = catalog_manager.execute(
            ...
            CatalogOperation::Create {
                ...
                params: serde_json::to_value(&graph_type)
                    .map_err(|e| ExecutionError::RuntimeError(
                        format!("Failed to serialize graph type: {}", e)
                    ))?,
            }
        ).map_err(|e| ExecutionError::RuntimeError(
            format!("Failed to create graph type: {}", e)
        ))?;
        ...
        match response {
            CatalogResponse::Success { data } => {
                ...
                Ok((
                    format!("Graph type '{}' created successfully", name),
                    1
                ))
            }
...
mod tests {
    use super::*;
    use crate::schema::types::{NodeTypeDefinition, PropertyDefinition, DataType};
