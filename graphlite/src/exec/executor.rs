...
use crate::catalog::system_procedures::{SystemProcedures, is_system_procedure};
use crate::plan::physical::{PhysicalPlan, PhysicalNode, ProjectionItem, SortItem};
use crate::plan::logical::PathElement;

use crate::ast::ast::{Expression, FunctionCall, EdgeDirection, Statement, CatalogStatement, SessionStatement, GraphExpression, SimpleCaseExpression, SearchedCaseExpression, CaseType, BasicQuery, Location, PathType, SelectItems, ReturnItem, Variable, PropertyAccess, PathQuantifier, WithQuery, WithClause, MatchClause, WhereClause, ReturnClause, TypeSpec, DeclareStatement, NextStatement, AtLocationStatement, TransactionStatement, CatalogPath, ProcedureBodyStatement};
use crate::txn::{TransactionManager, TransactionId, TransactionLog, UndoOperation};
use crate::storage::{GraphCache, Value, StorageManager};
use crate::cache::CacheManager;

use crate::functions::{FunctionRegistry, FunctionContext};
use crate::catalog::manager::CatalogManager;
use crate::catalog::operations::{CatalogOperation, CatalogResponse, QueryType};
use serde_json::json;
use crate::types::{GqlType, TypeInference, TypeValidator, TypeCoercion, TypeCaster, CoercionStrategy};
...
pub struct QueryExecutor {
    ...
    transaction_logs: Arc<std::sync::RwLock<std::collections::HashMap<TransactionId, TransactionLog>>>,

    // Type system components
    #[allow(dead_code)] // FALSE POSITIVE - Used via self.type_inference in methods (lines 7053, 7075). Compiler limitation with self.field access detection.
    type_inference: TypeInference,
    #[allow(dead_code)] // FALSE POSITIVE - Used via self.type_validator in validation methods. Compiler limitation with self.field access detection.
    type_validator: TypeValidator,
    #[allow(dead_code)] // FALSE POSITIVE - Used via self.type_coercion in methods (line 7117). Compiler limitation with self.field access detection.
    type_coercion: TypeCoercion,
    #[allow(dead_code)] // FALSE POSITIVE - Used via self.type_caster in methods (line 7246). Compiler limitation with self.field access detection.
...
impl QueryExecutor {
    ...
    pub fn execute_query(&self, request: ExecutionRequest) -> Result<QueryResult, ExecutionError> {
        log::debug!("EXECUTE_QUERY: Statement type: {:?}", std::mem::discriminant(&request.statement));
        ...
        if let Some(ref query_text) = request.query_text {
            if crate::exec::unwind_preprocessor::UnwindPreprocessor::is_unwind_query(query_text) {
                ...
                let executor_fn = |query: &str| -> Result<crate::exec::result::QueryResult, ExecutionError> {
                    // Create a new request without the UNWIND (the individual queries won't have UNWIND)
                    let parsed_query = crate::ast::parser::parse_query(query)
                        .map_err(|e| ExecutionError::RuntimeError(format!("Failed to parse individual query: {}", e)))?;
                ...
                return crate::exec::unwind_preprocessor::UnwindPreprocessor::execute_unwind_query(query_text, executor_fn);
            }
        }
    ...
    fn resolve_graph_for_execution(&self, request: &ExecutionRequest) -> Result<Arc<GraphCache>, ExecutionError> {
            ...
            if let Ok(session) = session_lock.read() {
                if let Some(current_graph_path) = &session.current_graph {
                    match self.storage.get_graph(current_graph_path)? {
                        ...
                        None => return Err(ExecutionError::RuntimeError(
    ...
    fn create_execution_context_from_session(&self, session: Option<&Arc<std::sync::RwLock<UserSession>>>) -> ExecutionContext {
    ...
    fn route_and_execute(
        ...
        match &request.statement {
            ...
            _ => {
                ...
                self.execute_statement(&request.statement, context, request.graph_expr.as_ref(), request.session.as_ref())
            }
        }
    }
    ...
    fn resolve_graph_expression(&self, _graph_expr: Option<&GraphExpression>) -> Result<Arc<GraphCache>, ExecutionError> {
        ...
        match self.storage.list_graphs() {
            Ok(graph_names) if !graph_names.is_empty() => {
                ...
                match self.storage.get_graph(graph_name)? {
                    ...
                    None => Err(ExecutionError::RuntimeError(format!("Graph {} not found", graph_name)))
                }
            },
            _ => Err(ExecutionError::RuntimeError("No graphs available".to_string()))
        }
    }
    ...
    pub fn new(
        ...
        let system_procedures = SystemProcedures::new(catalog_manager.clone(), storage_manager.clone(), None);
    ...
    fn lazy_load_graph(&self, graph_name: &str) -> Result<Option<GraphCache>, ExecutionError> {
        ...
        self.storage.get_graph(graph_name)
            .map_err(|e| ExecutionError::StorageError(format!("Failed to get/load graph '{}': {}", graph_name, e)))
    }
    ...
    fn resolve_graph_reference(
        ...
        match graph_expr {
                ...
                if let Some(session_lock) = session {
                    if let Ok(session_guard) = session_lock.read() {
                        if let Some(current_graph_path) = &session_guard.current_graph {
                            match self.storage.get_graph(current_graph_path)? {
                                Some(graph) => {
                                    log::debug!("Resolved CurrentGraph from session: {}", current_graph_path);
            ...
            Some(GraphExpression::Union { left, right, all: _ }) => {
                // Resolve left and right graph names
                let left_name = match left.as_ref() {
                    GraphExpression::Reference(catalog_path) => {
                        catalog_path.segments.last()
                            .ok_or_else(|| ExecutionError::RuntimeError("Invalid left graph path".to_string()))?
                    }
                    _ => return Err(ExecutionError::RuntimeError("Unsupported left graph expression in union".to_string())),
                };
                
                let right_name = match right.as_ref() {
                    GraphExpression::Reference(catalog_path) => {
                        catalog_path.segments.last()
                            .ok_or_else(|| ExecutionError::RuntimeError("Invalid right graph path".to_string()))?
                    }
                    _ => return Err(ExecutionError::RuntimeError("Unsupported right graph expression in union".to_string())),
                };
                
                // Perform graph union using unified storage
                let union_result = self.storage.create_graph_union(vec![left_name.clone(), right_name.clone()])?;
                Ok(Arc::new(union_result))
            },
            None => {
                Err(ExecutionError::RuntimeError("No graph specified - graph must be explicitly provided".to_string()))
            },
        }
    }

    /// Resolve graph name from graph expression for data modifications
    pub fn resolve_graph_name_for_modification(&self, graph_expr: Option<&GraphExpression>) -> Result<String, ExecutionError> {
        match graph_expr {
            Some(GraphExpression::Reference(catalog_path)) => {
            ...
            Some(GraphExpression::CurrentGraph) => {
                ...
                Err(ExecutionError::RuntimeError("CurrentGraph reference requires session context for modifications".to_string()))
            },
            None => {
                Err(ExecutionError::RuntimeError("No graph specified - graph must be explicitly provided".to_string()))
            },
            Some(GraphExpression::Union { .. }) => {
                Err(ExecutionError::RuntimeError("Union graphs not supported for data modifications".to_string()))
            }
        }
    }
    ...
    pub fn start_transaction_with_session(
        ...
        let txn_id = self.transaction_manager.start_transaction_with_session(
            isolation_level, access_mode, session_id
        )?;
        
        // Update current transaction
        let mut current_txn = self.current_transaction.write()
            .map_err(|_| ExecutionError::RuntimeError("Failed to write current transaction".to_string()))?;
    ...
    pub fn get_transaction_current_graph(&self, transaction_id: TransactionId) -> Result<Option<String>, ExecutionError> {
        self.transaction_manager.get_transaction_current_graph(transaction_id)
    }
    ...
    pub fn get_session_transactions(&self, session_id: &str) -> Result<Vec<TransactionId>, ExecutionError> {
        self.transaction_manager.get_session_transactions(session_id)
    }


    /// Add a new graph
    pub fn add_graph(&self, name: String, graph: GraphCache) -> Result<(), ExecutionError> {
        self.storage.save_graph(&name, graph)
    ...
    pub fn get_graph(&self, name: &str) -> Result<Arc<GraphCache>, ExecutionError> {
        match self.storage.get_graph(name)? {
            Some(graph) => Ok(Arc::new(graph)),
            None => Err(ExecutionError::StorageError(format!("Graph not found: {}", name))),
        }
    }
    
    /// Set current graph using unified session management
    pub fn set_current_graph(&self, graph_expr: crate::ast::ast::GraphExpression) -> Result<(), ExecutionError> {
        ...
        let graph_name = match &graph_expr {
            ...
            _ => {
                return Err(ExecutionError::RuntimeError(
                    "Only direct graph references are supported for setting current graph".to_string()
                ));
            }
        };
    ...
    pub fn log_operation_to_wal(&self, operation_type: crate::txn::state::OperationType, description: String) -> Result<(), ExecutionError> {
        // Check if we have an active transaction
        let current_txn = self.current_transaction.read()
            .map_err(|_| ExecutionError::RuntimeError("Failed to read current transaction".to_string()))?;
        
        if let Some(txn_id) = *current_txn {
            drop(current_txn);
            // Log the operation to WAL
            self.transaction_manager.log_operation(txn_id, operation_type, description)?;
        }
    ...
    fn execute_statement(&self, statement: &Statement, context: &mut ExecutionContext, graph_expr: Option<&GraphExpression>, session: Option<&Arc<std::sync::RwLock<UserSession>>>) -> Result<QueryResult, ExecutionError> {
        log::debug!("EXECUTOR: execute_statement called with statement type: {:?}", 
        ...
        match statement {
            Statement::Query(query) => {
                ...
                match query {
                    crate::ast::ast::Query::Basic(basic_query) => {
                        ...
                        use crate::ast::ast::{Query, Document, Statement as AstStatement};
                        
                        let query = Query::Basic(basic_query.clone());
                        let statement_ast = AstStatement::Query(query);
                        let document = Document { 
                            statement: statement_ast,
                            location: crate::ast::ast::Location { line: 1, column: 1, offset: 0 }
                        };
                        
                        let mut planner = QueryPlanner::new();
                        let planned_query = planner.plan_query(&document)
                    ...
                    crate::ast::ast::Query::Limited { query, order_clause, limit_clause } => {
                    ...
                    crate::ast::ast::Query::Let(let_stmt) => self.execute_let_statement(let_stmt, context),
                    crate::ast::ast::Query::For(for_stmt) => self.execute_for_statement(for_stmt, context),
                    crate::ast::ast::Query::Filter(filter_stmt) => self.execute_filter_statement(filter_stmt, context),
                    crate::ast::ast::Query::Return(return_query) => self.execute_return_query(return_query, context),
                    crate::ast::ast::Query::Unwind(unwind_stmt) => self.execute_unwind_statement(unwind_stmt, context),
                    crate::ast::ast::Query::MutationPipeline(pipeline) => self.execute_mutation_pipeline(pipeline, context, session),
                }
            },
            Statement::Select(select_stmt) => {
                ...
                if self.select_statement_needs_graph_context(select_stmt) {
                    ...
                    let select_graph_expr = if let Some(from_clause) = &select_stmt.from_clause {
                        from_clause.graph_expressions.first()
                        ...
                        if let Some(session_lock) = session {
                            if let Ok(session_state) = session_lock.read() {
                                if let Some(current_graph_path) = &session_state.current_graph {
                                    ...
                                    return Err(ExecutionError::RuntimeError(
                                        "No current graph set in session. Use SESSION SET GRAPH.".to_string()
                                    ))
                                }
                            } else {
                                return Err(ExecutionError::RuntimeError(
            ...
            Statement::Call(call_stmt) => {
                ...
                let session_id = session.as_ref().and_then(|s| {
                    s.read().ok().map(|session_state| session_state.session_id.clone())
                });
                self.execute_call_statement_with_graph_deferred(call_stmt, context, graph_expr, session, session_id.as_deref())
            },
            Statement::CatalogStatement(catalog_stmt) => {
                ...
                let mut catalog_manager = self.catalog_manager.write()
                    .map_err(|_| ExecutionError::RuntimeError("Failed to acquire catalog manager lock".to_string()))?;
            ...
            Statement::DataStatement(data_stmt) => {
                ...
                log::debug!("EXECUTOR: Routing DataStatement (type: {:?}) using graph context: {:?}", std::mem::discriminant(data_stmt), graph_expr);
                if let Some(session) = session {
                    log::debug!("EXECUTOR: Calling DataStatementCoordinator::execute_data_statement");
                    ...
                    Err(ExecutionError::RuntimeError("Data statements require a user session".to_string()))
                }
            },
    ...
    fn execute_basic_query(&self, basic_query: &BasicQuery, graph_expr: Option<&GraphExpression>, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
        use crate::plan::optimizer::QueryPlanner;
        use crate::ast::ast::{Query, Document, Statement};
    ...
    fn execute_basic_query_with_context(&self, basic_query: &BasicQuery, outer_context: &ExecutionContext) -> Result<QueryResult, ExecutionError> {
        use crate::plan::optimizer::QueryPlanner;
        use crate::ast::ast::{Query, Document, Statement};

        log::debug!("execute_basic_query_with_context: BasicQuery has GROUP BY: {}", basic_query.group_clause.is_some());

        // Get the graph from the execution context
        let graph = outer_context.current_graph.as_ref()
            .ok_or_else(|| ExecutionError::RuntimeError("No graph available in execution context".to_string()))?;
    ...
    fn execute_subquery(&self, query: &crate::ast::ast::Query, context: &ExecutionContext) -> Result<QueryResult, ExecutionError> {
    ...
    fn check_subquery_exists(&self, query: &crate::ast::ast::Query, context: &ExecutionContext) -> Result<bool, ExecutionError> {
    ...
    fn check_basic_query_exists(&self, query: &BasicQuery, outer_context: &ExecutionContext) -> Result<bool, ExecutionError> {
    ...
    fn execute_subquery_with_context(&self, query: &crate::ast::ast::Query, outer_context: &ExecutionContext) -> Result<QueryResult, ExecutionError> {
        use crate::ast::ast::Query;
        match query {
            Query::Basic(basic_query) => {
            ...
            Query::MutationPipeline(_) => {
                ...
                Err(ExecutionError::RuntimeError("Mutation pipelines cannot be used in subqueries".to_string()))
            }
        }
    }
    ...
    fn execute_with_query_with_context(
        ...
        log::debug!("Starting WITH query execution with {} segments", with_query.segments.len());
        ...
        for (i, segment) in with_query.segments.iter().enumerate() {
            ...
            let mut segment_results = self.execute_match_with_context(&segment.match_clause, &current_context)?;
            ...
            if let Some(where_clause) = &segment.where_clause {
                segment_results = self.apply_where_filter_to_rows(segment_results, where_clause, &current_context)?;
            }
            ...
            let mut with_results = if let Some(with_clause) = &segment.with_clause {
                log::debug!("Executing WITH clause on {} input rows", segment_results.len());
                self.execute_with_clause(with_clause, segment_results, &current_context)?
            } else {
                log::debug!("No WITH clause, passing through {} rows", segment_results.len());
                segment_results
            };
            ...
            if let Some(unwind_clause) = &segment.unwind_clause {
                ...
                with_results = self.execute_unwind_on_rows(unwind_clause, with_results, &current_context)?;
            }
            ...
            if let Some(where_clause) = &segment.post_unwind_where {
                ...
                with_results = self.apply_where_filter_to_rows_vec(with_results, where_clause, &current_context)?;
            }
            ...
            current_context = self.update_context_from_with_results(&current_context, &with_results)?;
            
            // Log segment execution for debugging
            log::debug!("Completed WITH query segment {}/{}", i + 1, with_query.segments.len());
        }
        
        // Execute final RETURN clause on the final results
        log::debug!("Executing final RETURN on {} result rows", final_results.len());
        ...
        let mut result = self.execute_final_return_on_rows(&with_query.final_return, final_results, &current_context)?;
    ...
    fn execute_match_with_context(
        ...
        let return_items: Vec<ReturnItem> = variables.into_iter().map(|var| {
            ReturnItem {
    ...
    fn execute_with_clause(
        ...
        if !input_rows.is_empty() {
            ...
            log::debug!("  Variables: {:?}", input_rows[0].values.keys().collect::<Vec<_>>());
    ...
    fn execute_with_clause_simple(
        ...
        for input_row in input_rows {
            ...
            for with_item in &with_clause.items {
                let value = self.evaluate_expression_in_row(&with_item.expression, &input_row, context)?;
    ...
    fn execute_with_clause_via_processor(
        ...
        let is_simple_variable_passthrough = with_clause.items.iter().all(|item| {
            matches!(item.expression, Expression::Variable(_))
        });
        ...
        if is_simple_variable_passthrough && !input_rows.is_empty() {
                    ...
                    for (key, value) in &input_row.values {
                        if let crate::storage::Value::Node(node) = value {
                            ...
                            for (prop_name, prop_value) in &node.properties {
                                ...
                                temp_context.variables.insert(prop_key.clone(), prop_value.clone());
                            }
                        } else {
            ...
            if let Some(ref where_clause) = with_clause.where_clause {
                let filtered_rows = result_rows.into_iter().filter(|row| {
        ...
        let with_result = WithClauseProcessor::process_with_clause(with_clause, &variable_bindings, &edges, &enhanced_context)?;
    ...
    fn convert_rows_to_processor_format(
        &self,
        input_rows: Vec<Row>
    ) -> Result<(std::collections::HashMap<String, Vec<crate::storage::Node>>, Vec<crate::storage::Edge>), ExecutionError> {
        use std::collections::HashMap;
                ...
                if let Some(node_value) = row.values.get(node_var) {
                    ...
                    for (key, value) in &row.values {
                        if key.starts_with(&format!("{}.", node_var)) {
                            let property_name = key.strip_prefix(&format!("{}.", node_var)).unwrap();
                            node.properties.insert(property_name.to_string(), value.clone());
                        }
                    }
                    
                    variable_bindings.entry(node_var.clone()).or_insert_with(Vec::new).push(node);
                }
            }
        }
    ...
    fn try_convert_value_to_node(&self, _key: &str, value: &crate::storage::Value) -> Result<crate::storage::Node, ExecutionError> {
                ...
                match func_call.name.to_lowercase().as_str() {
                    ...
                    "avg" => {
                        ...
                        if let Some(arg) = func_call.arguments.first() {
                                ...
                                match value {
                                    ...
                                    _ => {
                                        return Err(ExecutionError::ExpressionError(
                                            format!("Cannot compute average of non-numeric value: {:?}", value)
                                        ));
                                    }
                                }
                            }
                            ...
                            Err(ExecutionError::ExpressionError("AVG function requires an argument".to_string()))
                        }
                    },
                    "sum" => {
                        ...
                        if let Some(arg) = func_call.arguments.first() {
                            ...
                            for row in group_rows {
                                ...
                                match value {
                                    ...
                                    _ => {
                                        return Err(ExecutionError::ExpressionError(
                                            format!("Cannot compute sum of non-numeric value: {:?}", value)
                                        ));
                                    }
                                }
                            }
                            ...
                            Err(ExecutionError::ExpressionError("SUM function requires an argument".to_string()))
                        }
                    },
                    _ => {
                        Err(ExecutionError::UnsupportedOperator(format!("Aggregate function {} not implemented", func_call.name)))
                    }
                }
            },
            _ => {
                Err(ExecutionError::ExpressionError("Non-function expression passed to aggregate evaluator".to_string()))
            }
        }
    }
    ...
    fn execute_final_return_on_rows(
        ...
        log::debug!("Executing final RETURN clause on {} WITH result rows", with_rows.len());
        
        // Check if any return item contains aggregate functions
        let has_aggregation = return_clause.items.iter().any(|item| {
            self.contains_aggregate_function(&item.expression)
        });
                ...
                let value = match &return_item.expression {
                    Expression::Variable(var) => {
                        ...
                        if let Some(var_value) = with_row.values.get(&var.name) {
                            ...
                            log::warn!("Variable {} not found in WITH row, available variables: {:?}", 
                                var.name, with_row.values.keys().collect::<Vec<_>>());
                            return Err(ExecutionError::ExpressionError(format!("Variable not found: {}", var.name)));
                        }
                    },
                    Expression::PropertyAccess(_) => {
                        // Handle property access like m.name
                        self.evaluate_expression_in_row(&return_item.expression, &with_row, context)?
                    },
                    Expression::FunctionCall(func_call) => {
                    ...
                    _ => {
                        return Err(ExecutionError::UnsupportedOperator("Complex expressions in WITH RETURN not yet supported".to_string()));
                    }
                };
                
                let column_name = return_item.alias.clone().unwrap_or_else(|| {
        ...
        log::debug!("Final RETURN produced {} rows with variables: {:?}", result.rows.len(), result.variables);
        Ok(result)
    }
    ...
    fn contains_aggregate_function(&self, expr: &Expression) -> bool {
        match expr {
            Expression::FunctionCall(func_call) => {
                ...
                matches!(func_call.name.to_uppercase().as_str(), "COUNT" | "AVG" | "SUM" | "MIN" | "MAX")
            },
    ...
    fn execute_return_with_aggregation(
        ...
        log::debug!("Executing RETURN clause with aggregation on {} rows", with_rows.len());
                    ...
                    match func_call.name.to_uppercase().as_str() {
                                ...
                                DistinctQualifier::Distinct => {
                                    ...
                                    if let Some(arg_expr) = func_call.arguments.first() {
                                        ...
                                        for row in &with_rows {
                                            ...
                                            if let Ok(value) = self.evaluate_expression_in_row(arg_expr, row, context) {
                        ...
                        "AVG" => {
                            ...
                            if let Some(arg_expr) = func_call.arguments.first() {
                                ...
                                for row in &with_rows {
                                    // Evaluate the expression (handles both Variable and PropertyAccess)
                                    if let Ok(Value::Number(n)) = self.evaluate_expression_in_row(arg_expr, row, context) {
                                        sum += n;
                                        count += 1;
                                    }
                                }
                                ...
                                Err(ExecutionError::UnsupportedOperator("AVG requires an argument".to_string()))
                            }
                        },
                        "SUM" => {
                            ...
                            if let Some(arg_expr) = func_call.arguments.first() {
                                ...
                                Err(ExecutionError::UnsupportedOperator("SUM requires an argument".to_string()))
                            }
                        },
                        "MIN" => {
                            ...
                            if let Some(arg_expr) = func_call.arguments.first() {
                                ...
                                Err(ExecutionError::UnsupportedOperator("MIN requires an argument".to_string()))
                            }
                        },
                        "MAX" => {
                            ...
                            if let Some(arg_expr) = func_call.arguments.first() {
                                ...
                                Err(ExecutionError::UnsupportedOperator("MAX requires an argument".to_string()))
                            }
                        },
                        "COLLECT" => {
                            ...
                            if let Some(arg_expr) = func_call.arguments.first() {
                                ...
                                Err(ExecutionError::UnsupportedOperator("COLLECT requires an argument".to_string()))
                            }
                        },
                        _ => {
                            Err(ExecutionError::UnsupportedOperator(format!("Unsupported aggregate function: {}", func_call.name)))
                        }
                    }
                }?,
                ...
                Expression::Variable(var) => {
                    ...
                    if let Some(row) = with_rows.first() {
                        if let Some(value) = row.values.get(&var.name) {
                            ...
                            Err(ExecutionError::UnsupportedOperator(format!("Variable '{}' not found in WITH clause", var.name)))
                        }
                    } else {
                        Err(ExecutionError::UnsupportedOperator("No rows available for variable resolution".to_string()))
                    }
                }?,
                Expression::PropertyAccess(prop_access) => {
                    ...
                    if let Some(row) = with_rows.first() {
                        ...
                        Err(ExecutionError::UnsupportedOperator("No rows available for property access".to_string()))
                    }
                }?,
                _ => {
                    return Err(ExecutionError::UnsupportedOperator(format!("Expression type not supported in aggregate RETURN clause: {:?}", return_item.expression)));
                }
            };
        ...
        log::debug!("Aggregated RETURN produced 1 row with variables: {:?}", result.variables);
        Ok(result)
    }
            ...
            let value = match &return_item.expression {
                Expression::Variable(var) => {
                    ...
                    if let Some(var_value) = context.variables.get(&var.name) {
                        ...
                        log::warn!("Variable {} not found in context, available variables: {:?}", 
                            var.name, context.variables.keys().collect::<Vec<_>>());
                        return Err(ExecutionError::ExpressionError(format!("Variable not found: {}", var.name)));
                    }
                },
                Expression::FunctionCall(func_call) => {
        ...
        match expr {
            Expression::Variable(var) => {
                ...
                if let Some(value) = row.values.get(&var.name) {
                    ...
                    Err(ExecutionError::ExpressionError(format!("Variable not found: {}", var.name)))
                }
            },
            Expression::FunctionCall(func_call) => {
                ...
                match func_call.name.to_lowercase().as_str() {
                    ...
                    "avg" | "sum" | "min" | "max" => {
                        ...
                        if func_name == "sum" {
                            ...
                            log::debug!("DEBUG: evaluate_expression_in_row - {} returning 0.0 placeholder", func_name);
                            Ok(Value::Number(0.0))
                        }
                    },
                    _ => {
                        // For non-aggregation functions, use the regular function evaluation
                        log::debug!("Delegating to regular function evaluation for: {}", func_call.name);
                        self.evaluate_function_call(func_call, context)
                    }
                }
            },
            Expression::PropertyAccess(prop_access) => {
                ...
                log::debug!("Property access: {}.{}, available variables: {:?}",
                ...
                if let Some(obj_value) = row.values.get(&prop_access.object) {
                    ...
                    Err(ExecutionError::ExpressionError(format!("Object {} not found for property access", prop_access.object)))
                }
            },
            _ => {
                Err(ExecutionError::UnsupportedOperator(format!("Expression type not supported in WITH clause: {:?}", expr)))
            }
        }
    }
    ...
    fn apply_where_filter_to_rows(
        ...
        for row in rows.into_iter() {
            ...
            let condition_value = self.evaluate_expression(&where_clause.condition, &row_context)?;
        ...
        log::debug!("WHERE filter kept {} out of {} rows", filtered_rows.len(), original_count);
        Ok(filtered_rows)
    }
    ...
    fn statement_needs_graph_context(&self, statement: &crate::ast::ast::Statement) -> bool {
        ...
        match statement {
            ...
            Statement::ProcedureBody(procedure_body) => {
                ...
                procedure_body.chained_statements.iter().any(|chained| {
                    self.statement_needs_graph_context(&chained.statement)
                })
            }
    ...
    fn procedure_needs_graph_context(&self, procedure_name: &str) -> bool {
        ...
        let result = match normalized_name.as_str() {
            ...
            "gql.clear_cache" |
            "gql.cache_stats" => false,
            
            // Procedures that can work with explicit parameters or session context
            "gql.graph_stats" |
            "gql.sample_data" => false,  // These handle their own graph resolution
    ...
    fn select_statement_needs_graph_context(&self, select_stmt: &crate::ast::ast::SelectStatement) -> bool {
    ...
    fn expression_needs_graph_context(&self, expr: &crate::ast::ast::Expression) -> bool {
        ...
        match expr {
            ...
            Expression::Unary(unary) => {
                self.expression_needs_graph_context(&unary.expression)
            },
            Expression::Case(case_expr) => {
    ...
    fn execute_call_statement_without_graph(&self, call_stmt: &crate::ast::ast::CallStatement, context: &ExecutionContext, session_id: Option<&str>) -> Result<QueryResult, ExecutionError> {
        ...
        let mut result = self.system_procedures.execute_procedure(&call_stmt.procedure_name, evaluated_args, session_id)?;
        ...
        if let Some(yield_clause) = &call_stmt.yield_clause {
            ...
            // Update column list
            result.variables = yield_clause.items.iter()
        ...
        if let Some(where_clause) = &call_stmt.where_clause {
            ...
            let filtered_rows: Result<Vec<_>, ExecutionError> = result.rows.into_iter()
                .filter_map(|row| {
                    ...
                    match self.evaluate_expression(&where_clause.condition, &temp_context) {
                        ...
                        Ok(crate::storage::Value::Number(n)) => {
                            if n != 0.0 { Some(Ok(row)) } else { None }
                        }
    ...
    fn execute_call_statement_with_graph(&self, call_stmt: &crate::ast::ast::CallStatement, context: &mut ExecutionContext, _graph: &Arc<GraphCache>, session_id: Option<&str>) -> Result<QueryResult, ExecutionError> {
    ...
    fn execute_select_statement_with_graph(&self, select_stmt: &crate::ast::ast::SelectStatement, graph: &Arc<GraphCache>, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
        // If there's a FROM clause, validate it
        if let Some(from_clause) = &select_stmt.from_clause {
            if from_clause.graph_expressions.is_empty() {
                return Err(ExecutionError::SyntaxError("FROM clause must specify at least one graph expression".to_string()));
            }
        }
        ...
        let match_clause = match_clause.unwrap_or_else(|| {
            crate::ast::ast::MatchClause {
                patterns: vec![
                    crate::ast::ast::PathPattern {
                        assignment: None, // No path assignment
                        path_type: None, // Default path type
                        elements: vec![
                            crate::ast::ast::PatternElement::Node(crate::ast::ast::Node {
        ...
        let plan = planner.plan_query(&document)
            .map_err(|e| ExecutionError::PlanningError(format!("Failed to plan SELECT query: {}", e)))?;
    ...
    fn execute_select_statement_without_graph(&self, select_stmt: &crate::ast::ast::SelectStatement, context: &ExecutionContext) -> Result<QueryResult, ExecutionError> {
        // Validate that this SELECT can run without graph context
        if select_stmt.from_clause.is_some() {
            return Err(ExecutionError::RuntimeError("SELECT statement with FROM clause requires graph context".to_string()));
        }
        ...
        let items = match &select_stmt.return_items {
            crate::ast::ast::SelectItems::Wildcard { .. } => {
                return Err(ExecutionError::RuntimeError("Wildcard SELECT (*) requires graph context".to_string()));
            },
    ...
    #[deprecated(note = "Use DDLStatementCoordinator::execute_ddl_statement for proper WAL logging and transaction support")]
    #[allow(dead_code)] // ROADMAP v0.4.0 - Legacy catalog statement executor (deprecated, use DDLStatementCoordinator)
    fn execute_catalog_statement(&self, statement: &CatalogStatement) -> Result<QueryResult, ExecutionError> {
        let start_time = std::time::Instant::now();
        ...
        match result {
            ...
            warnings: Vec::new(),
                    ...
                    rows: vec![Row::from_values(std::collections::HashMap::from([
                        ("status".to_string(), crate::storage::Value::String(message)),
                    ]))],
    ...
    fn execute_with_graph(&self, plan: &PhysicalPlan, graph: &Arc<GraphCache>, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
    ...
    fn execute_node_without_graph(&self, node: &PhysicalNode, context: &mut ExecutionContext) -> Result<Vec<Row>, ExecutionError> {
        match node {
    ...
    fn execute_node_with_graph(&self, node: &PhysicalNode, context: &mut ExecutionContext, graph: &Arc<GraphCache>) -> Result<Vec<Row>, ExecutionError> {
        match node {
            PhysicalNode::NodeSeqScan { variable, labels, properties, .. } => {
                self.execute_node_seq_scan_with_graph(variable, labels, properties.as_ref(), context, graph)
            },
            ...
            PhysicalNode::Having { condition, input, .. } => {
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
            ...
            PhysicalNode::GenericFunction { function_name, arguments, input, .. } => {
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
                self.execute_generic_function(function_name, arguments, input_rows, context)
            },
            
            PhysicalNode::HashAggregate { group_by, aggregates, input, .. } => {
                log::debug!("EXECUTING HashAggregate NODE");
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
                self.execute_hash_aggregate(group_by, aggregates, input_rows, context)
            },
            
            PhysicalNode::SortAggregate { group_by, aggregates, input, .. } => {
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
                self.execute_sort_aggregate(group_by, aggregates, input_rows, context)
            },
            
            PhysicalNode::Limit { count, offset, input, .. } => {
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
                self.execute_limit(*count, *offset, input_rows)
            },
            
            PhysicalNode::HashExpand { from_variable, edge_variable, to_variable, edge_labels, direction, properties, input, .. } => {
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
                self.execute_hash_expand_with_graph(from_variable, edge_variable.as_deref(), to_variable, edge_labels, direction, properties.as_ref(), input_rows, context, graph)
            },
            
            PhysicalNode::IndexedExpand { from_variable, edge_variable, to_variable, edge_labels, direction, properties, input, .. } => {
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
                self.execute_indexed_expand_with_graph(from_variable, edge_variable.as_deref(), to_variable, edge_labels, direction, properties.as_ref(), input_rows, context, graph)
            },
            
            PhysicalNode::InMemorySort { expressions, input, .. } => {
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
                self.execute_in_memory_sort(expressions, input_rows, context)
            },
            
            PhysicalNode::ExternalSort { expressions, input, .. } => {
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
            ...
            PhysicalNode::PathTraversal { path_type, from_variable, to_variable, path_elements, input, .. } => {
                let input_rows = self.execute_node_with_graph(input, context, graph)?;
                self.execute_path_traversal(path_type, from_variable, to_variable, path_elements, input_rows, context, graph)
            },
            ...
            PhysicalNode::Unwind { expression, variable, input, .. } => {
                self.execute_unwind(expression, variable, input.as_deref(), context, Some(graph))
            },
            
            // Handle join operations
            PhysicalNode::NestedLoopJoin { join_type, condition, left, right, .. } => {
                self.execute_nested_loop_join(join_type, condition.as_ref(), left, right, context, graph)
            },
            
            PhysicalNode::HashJoin { join_type, condition, build, probe, .. } => {
                self.execute_hash_join(join_type, condition.as_ref(), build, probe, context, graph)
            },
            
            PhysicalNode::SortMergeJoin { join_type, left_keys: _, right_keys: _, left, right, .. } => {
            ...
            PhysicalNode::UnionAll { inputs, all, .. } => {
                ...
                if *all {
                    ...
                    for row in all_rows {
                        if !deduplicated.iter().any(|existing| self.rows_equal(&row, existing)) {
            ...
            PhysicalNode::Intersect { left, right, all, .. } => {
                ...
                for left_row in &left_rows {
                    if right_rows.iter().any(|right_row| self.rows_equal(left_row, right_row)) {
                        if *all || !result.iter().any(|existing| self.rows_equal(left_row, existing)) {
            ...
            PhysicalNode::Except { left, right, all, .. } => {
                let left_rows = self.execute_node_with_graph(left, context, graph)?;
                let right_rows = self.execute_node_with_graph(right, context, graph)?;
        ...
        for left_row in &left_rows {
            for right_row in &right_rows {
                ...
                if let Some(score) = left_row.get_text_score() {
                    ...
                    combined_row.values.insert("TEXT_SCORE()".to_string(), Value::Number(score));
                }
    ...
    fn execute_node_seq_scan_with_graph(
        ...
        for node in nodes {
            ...
            row.values.insert(id_property_name, Value::String(node.id.clone()));
            ...
            for (prop_name, prop_value) in &node.properties {
                ...
                if prop_name == "score" {
                    log::debug!("DEBUG_NODE_SCAN: Added property '{}' = {:?}", full_name, prop_value);
                }
            }
    ...
    fn execute_filter(
        ...
        for row in input_rows {
            ...
            if self.evaluate_expression(condition, context)?.as_boolean().unwrap_or(false) {
                filtered_rows.push(row);
            }
        }
                ...
                if let Expression::FunctionCall(func_call) = binary_expr.left.as_ref() {
                    if func_call.name.to_uppercase() == "COUNT" && 
                       ...
                       matches!(func_call.arguments[0], Expression::Variable(ref var) if var.name == "*") {
                        ...
                        let left_value = if let Some(account_count) = row.values.get("account_count") {
                            account_count.clone()
                        } else {
                        ...
                        let temp_condition = Expression::Binary(crate::ast::ast::BinaryExpression {
    ...
    fn execute_project(
        ...
        let has_aggregates = expressions.iter().any(|expr| {
            self.is_aggregate_function(&expr.expression)
        });
        
        if has_aggregates {
            // Check if we have mixed aggregate and non-aggregate expressions
            let has_non_aggregates = expressions.iter().any(|expr| {
                !self.is_aggregate_function(&expr.expression)
            });
        ...
        for row in input_rows {
            ...
            for proj_item in expressions {
                ...
                if let Expression::Variable(var) = &proj_item.expression {
                    ...
                    if let Some(entity_id) = row.source_entities.get(&var.name) {
                        new_row.source_entities.insert(var.name.clone(), entity_id.clone());
                    }
                }
                // For property access, track the source entity
                    ...
                    if let Some(entity_id) = row.source_entities.get(var_name) {
                        ...
                        new_row.source_entities.insert(var_name.clone(), entity_id.clone());
                    }
                }
            ...
            if let Some(score) = row.get_text_score() {
                ...
                // Also preserve TEXT_SCORE() pseudo-column for ORDER BY support
                new_row.values.insert("TEXT_SCORE()".to_string(), Value::Number(score));
            }
    ...
    fn evaluate_expression_with_types(
        ...
        match expr {
            ...
            Expression::Binary(binary) => {
                let (left_val, left_type) = self.evaluate_expression_with_types(&binary.left, context)?;
                let (right_val, right_type) = self.evaluate_expression_with_types(&binary.right, context)?;
                
                // Use type inference to determine result type
                let result_type = self.type_inference.infer_binary_op_type(
                    &format!("{:?}", binary.operator), &left_type, &right_type
                ).map_err(|e| ExecutionError::RuntimeError(format!("Type inference error: {}", e)))?;
                
                // Apply coercion if needed
                let (coerced_left, coerced_right) = self.apply_coercion(&left_val, &left_type, &right_val, &right_type)?;
                
                // Evaluate with proper types
                let result_val = self.evaluate_binary_op(&binary.operator, coerced_left, coerced_right)?;
            ...
            Expression::FunctionCall(func) => {
                ...
                let return_type = self.validate_and_execute_function(&func.name, &arg_types, &arg_values, context)?;
            ...
            Expression::Variable(var) => {
                let value = context.get_variable(&var.name)
            ...
            Expression::Unary(unary) => {
                let (operand_val, operand_type) = self.evaluate_expression_with_types(&unary.expression, context)?;
                
                let result_type = self.type_inference.infer_unary_operation_type(&operand_type, &unary.operator)
                    .map_err(|e| ExecutionError::RuntimeError(format!("Type inference error: {}", e)))?;
            ...
            Expression::Subquery(subquery_expr) => {
                ...
                if subquery_result.rows.is_empty() {
                    ...
                    return Err(ExecutionError::ExpressionError(
                        format!("Scalar subquery returned {} rows, expected 0 or 1", subquery_result.rows.len())
                    ));
                }
            },
            ...
            Expression::ExistsSubquery(exists_subquery_expr) => {
                ...
                let has_results = self.check_subquery_exists(&exists_subquery_expr.query, context)?;
            ...
            Expression::NotExistsSubquery(not_exists_subquery_expr) => {
                ...
                let has_results = self.check_subquery_exists(&not_exists_subquery_expr.query, context)?;
            ...
            Expression::InSubquery(in_subquery_expr) => {
                ...
                let (left_value, _left_type) = self.evaluate_expression_with_types(&in_subquery_expr.expression, context)?;
            ...
            Expression::NotInSubquery(not_in_subquery_expr) => {
                // Evaluate the left expression
                let (left_value, _left_type) = self.evaluate_expression_with_types(&not_in_subquery_expr.expression, context)?;
                
                // Execute the subquery
                let subquery_result = self.execute_subquery(&not_in_subquery_expr.query, context)?;
            ...
            Expression::QuantifiedComparison(quantified_expr) => {
                ...
                let _left_result = self.evaluate_expression_with_types(&quantified_expr.left, context)?;
                
                // TODO: Execute subquery to get result set
                // For now, return a placeholder
                return Err(ExecutionError::RuntimeError("Quantified comparisons not yet fully implemented".to_string()));
            },
            ...
            Expression::Parameter(parameter) => {
                ...
                Err(ExecutionError::ExpressionError(
                    format!("Parameter '{}' is not bound. Parameter binding not yet implemented.", parameter.name)
                ))
            },
            ...
            Expression::ArrayIndex(array_index) => {
                ...
                let (array_value, _array_type) = self.evaluate_expression_with_types(&array_index.array, context)?;
                let (index_value, _index_type) = self.evaluate_expression_with_types(&array_index.index, context)?;
                
                // Extract index as integer
                let index = match index_value {
                    Value::Number(n) => n as usize,
                    _ => return Err(ExecutionError::ExpressionError(
                ...
                match array_value {
                    Value::List(list) => {
                        if index < list.len() {
                            ...
                            Ok((Value::Null, GqlType::String { max_length: None })) // Out of bounds returns NULL
                        }
                    },
                    Value::Vector(vec) => {
                        if index < vec.len() {
                    ...
                    _ => Err(ExecutionError::ExpressionError(
                        format!("Cannot index non-array value: {:?}", array_value)
                    )),
                }
            },
        }
    ...
    fn evaluate_expression(
        ...
        match expr {
            ...
            Expression::Variable(var) => {
                ...
                log::debug!("Looking for variable '{}' in context with {} variables", var.name, context.variables.len());
                for (key, _) in &context.variables {
                    log::debug!("  Context has variable: '{}'", key);
                }
                context.get_variable(&var.name)
                    .ok_or_else(|| ExecutionError::ExpressionError(format!("Variable not found: {}", var.name)))
            },
            ...
            Expression::Cast(cast_expr) => {
                self.evaluate_cast_expression(cast_expr, context)
            },
            ...
            Expression::NotInSubquery(not_in_subquery_expr) => {
                // Evaluate the left expression
                let left_value = self.evaluate_expression(&not_in_subquery_expr.expression, context)?;
            ...
            Expression::QuantifiedComparison(quantified_expr) => {
                ...
                // TODO: Execute subquery to get result set
                // For now, return a placeholder
                return Err(ExecutionError::RuntimeError("Quantified comparisons not yet fully implemented in legacy method".to_string()));
            },
    ...
    fn evaluate_case_expression(
        ...
        match &case_expr.case_type {
            CaseType::Simple(simple_case) => {
                self.evaluate_simple_case(simple_case, context)
            },
            CaseType::Searched(searched_case) => {
    ...
    fn evaluate_path_constructor(
        ...
        use crate::storage::value::{PathValue, PathElement};
        ...
        for (i, expr) in path_constructor.elements.iter().enumerate() {
            ...
            match value {
                ...
                _ => {
                    return Err(ExecutionError::RuntimeError(
                        format!("PATH constructor element must be a string or number, got: {:?}", value)
                    ));
                }
            }
        }
    ...
    fn cast_value(&self, value: Value, target_type: &crate::ast::ast::TypeSpec) -> Result<Value, ExecutionError> {
        ...
        match target_type {
            ...
            TypeSpec::Decimal { precision, scale } => self.cast_to_decimal(value, *precision, *scale),
            _ => Err(ExecutionError::RuntimeError(
                format!("CAST to {:?} is not yet implemented", target_type)
            )),
        }
    }
    ...
    fn cast_to_boolean(&self, value: Value) -> Result<Value, ExecutionError> {
        match value {
            ...
            _ => Err(ExecutionError::RuntimeError(
                format!("Cannot cast {:?} to BOOLEAN", value.type_name())
            )),
        }
    }
    
    /// Cast value to STRING
    fn cast_to_string(&self, value: Value, max_length: Option<usize>) -> Result<Value, ExecutionError> {
        let string_value = match value {
            ...
            _ => return Err(ExecutionError::RuntimeError(
                format!("Cannot cast {:?} to STRING", value.type_name())
            )),
        };
        ...
        match value {
            ...
            Value::String(s) => {
                match s.parse::<f64>() {
                    ...
                    Err(_) => Err(ExecutionError::RuntimeError(
            ...
            Value::Null => Ok(Value::Null),
            _ => Err(ExecutionError::RuntimeError(
                format!("Cannot cast {:?} to INTEGER", value.type_name())
            )),
        }
    }
        ...
        match int_value {
            Value::Number(n) => {
                if n >= -32768.0 && n <= 32767.0 {
                    ...
                    Err(ExecutionError::RuntimeError(
                        format!("Value {} is out of range for SMALLINT", n)
                    ))
                }
            },
        ...
        match value {
            ...
            Value::String(s) => {
                match s.parse::<f64>() {
                    ...
                    Err(_) => Err(ExecutionError::RuntimeError(
                        format!("Cannot cast '{}' to DOUBLE", s)
                    )),
                }
            },
            Value::Boolean(b) => Ok(Value::Number(if b { 1.0 } else { 0.0 })),
            Value::Null => Ok(Value::Null),
            _ => Err(ExecutionError::RuntimeError(
                format!("Cannot cast {:?} to DOUBLE", value.type_name())
            )),
        }
    }
    
    /// Cast value to DECIMAL
    fn cast_to_decimal(&self, value: Value, _precision: Option<u8>, _scale: Option<u8>) -> Result<Value, ExecutionError> {
    ...
    fn evaluate_literal(&self, literal: &crate::ast::ast::Literal) -> Result<Value, ExecutionError> {
        match literal {
            ...
            crate::ast::ast::Literal::Vector(vec) => Ok(Value::Vector(vec.iter().map(|&f| f as f32).collect())),
            crate::ast::ast::Literal::List(list) => {
                let converted: Vec<Value> = list.iter().map(|lit| self.evaluate_literal(lit))
    ...
    fn evaluate_binary_op(
        ...
        match (op, &left, &right) {
            ...
            (Operator::GreaterThan | Operator::LessThan | Operator::GreaterEqual | Operator::LessEqual, _, _) => {
                ...
                match (&left, &right) {
                    ...
                    _ => Err(ExecutionError::TypeError(
                        format!("Type mismatch in comparison: cannot compare {} with {}",
            ...
            (Operator::And, Value::Boolean(l), Value::Boolean(r)) => {
                Ok(Value::Boolean(*l && *r))
            },
            (Operator::Or, Value::Boolean(l), Value::Boolean(r)) => {
                Ok(Value::Boolean(*l || *r))
            },
            (Operator::Xor, Value::Boolean(l), Value::Boolean(r)) => {
                Ok(Value::Boolean(*l ^ *r))
            },
            ...
            (Operator::Starts, _, _) | (Operator::Ends, _, _) | (Operator::Contains, _, _) | (Operator::Like, _, _) | (Operator::In, _, _) | (Operator::NotIn, _, _) => {
                if left.is_null() || right.is_null() {
                    Ok(Value::Null)
                } else {
                    match op {
                        ...
                        Operator::Starts | Operator::Ends | Operator::Contains | Operator::Like => {
                            ...
                            match op {
                                Operator::Starts => Ok(Value::Boolean(left_str.starts_with(&right_str))),
                                Operator::Ends => Ok(Value::Boolean(left_str.ends_with(&right_str))),
                                Operator::Contains => Ok(Value::Boolean(left_str.contains(&right_str))),
            ...
            _ => Err(ExecutionError::TypeError(format!("Cannot apply {:?} to {:?} and {:?}", op, left, right))),
        }
    }
    ...
    fn value_to_string(&self, value: &Value) -> Result<String, ExecutionError> {
        match value {
            ...
            Value::Null => Err(ExecutionError::TypeError("Cannot concatenate with NULL".to_string())),
    ...
    fn like_match_recursive(&self, text: &[char], text_pos: usize, pattern: &[char], pattern_pos: usize) -> bool {
    ...
    fn evaluate_function_call(
        ...
        if is_system_procedure(&func_call.name) {
            ...
            let result = self.system_procedures.execute_procedure(&func_call.name, evaluated_args, Some("system"))?;
        ...
        let function = self.function_registry.get(&func_call.name)
        ...
        let result = function.execute(&function_context)
            .map_err(|e| {
    ...
    fn execute_generic_function(
        ...
        // Get the function from registry
        let function = self.function_registry.get(function_name)
            .ok_or_else(|| ExecutionError::UnsupportedOperator(format!("Function not found: {}", function_name)))?;
        ...
        // Execute the function
        let result = function.execute(&function_context)
            .map_err(|e| ExecutionError::UnsupportedOperator(format!("Function execution error: {}", e)))?;
    ...
    fn execute_aggregate(
        ...
        if !group_by.is_empty() {
            log::debug!("AGGREGATE DEBUG: GROUP BY expressions: {:?}", group_by.iter().map(|e| format!("{:?}", e)).collect::<Vec<_>>());
        }
        ...
        for row in input_rows {
            ...
            for expr in group_by {
                let value = self.evaluate_expression(expr, context)?;
                log::debug!("AGGREGATE DEBUG: GROUP BY expr {:?} evaluated to: {:?}", expr, value);
        ...
        for (group_key, group_rows) in groups {
            ...
            for aggregate in aggregates {
                ...
                // Handle different argument types
                    ...
                    let result = function.execute(&function_context)
                        .map_err(|e| ExecutionError::UnsupportedOperator(format!("Aggregate function error: {}", e)))?;
                    ...
                    let column_name = aggregate.alias.clone().unwrap_or_else(|| {
                        format!("{}_{}", function_name, self.expression_to_string(&aggregate.expression))
                    });
    ...
    fn is_aggregate_function(&self, expr: &Expression) -> bool {
        match expr {
            Expression::FunctionCall(func_call) => {
                matches!(func_call.name.to_uppercase().as_str(), "COUNT" | "SUM" | "AVG" | "AVERAGE" | "MIN" | "MAX" | "COLLECT")
            },
            _ => false,
        }
    }
    ...
    fn execute_mixed_aggregate_projection(
        ...
        for input_row in input_rows {
            ...
            if let Some(score) = input_row.get_text_score() {
                ...
                // Also preserve TEXT_SCORE() pseudo-column for ORDER BY support
                result_row.values.insert("TEXT_SCORE()".to_string(), Value::Number(score));
            }
    ...
    fn execute_aggregate_projection(
        ...
        for proj_item in expressions {
            let value = if let Expression::FunctionCall(func_call) = &proj_item.expression {
                ...
                return Err(ExecutionError::ExpressionError("Non-aggregate expressions not allowed with aggregate functions".to_string()));
            };
    ...
    fn execute_hash_expand_with_graph(
        ...
        for (_idx, input_row) in input_rows.iter().enumerate() {
            ...
            let from_node_id = input_row.get_value(from_variable)
                .ok_or_else(|| ExecutionError::RuntimeError(format!("Variable not found: {}", from_variable)))?;
            ...
            
            {
                ...
                let edges = match direction {
                    ...
                    EdgeDirection::Incoming => {
                        graph.get_incoming_edges(&from_id)
                    },
                    EdgeDirection::Both => {
                ...
                let filtered_edges: Vec<_> = if edge_labels.is_empty() {
                    ...
                    edges.into_iter()
                        .filter(|edge| {
                            edge_labels.iter().any(|label| &edge.label == label)
                        })
                        .collect()
                };
                ...
                for edge in filtered_edges {
                    ...
                    if let Some(to_node) = graph.get_node(to_node_id) {
                        ...
                        if node_matches {
                            ...
                            result_row.set_value(id_property_name, Value::String(to_node.id.clone()));
    ...
    fn execute_indexed_expand_with_graph(
        ...
        self.execute_hash_expand_with_graph(from_variable, edge_variable, to_variable, edge_labels, direction, properties, input_rows, context, graph)
    }
    ...
    fn execute_path_traversal(
        ...
        for input_row in input_rows {
            ...
            let start_node_id = input_row.get_value(from_variable)
                .ok_or_else(|| ExecutionError::RuntimeError(format!("Variable not found: {}", from_variable)))?;
            ...
            if let Value::String(start_id) = start_node_id {
                ...
                for path in paths {
                    ...
                    for (i, element) in path_elements.iter().enumerate() {
                        if i < path.len() - 1 {
                            row.set_value(element.node_variable.clone(), Value::String(path[i + 1].clone()));
                        }
                    }
    ...
    fn expand_quantified_element(
        ...
        match &element.quantifier {
            None => {
                ...
                self.expand_single_element(current_path, element, path_type, graph, visited_nodes, visited_edges, 1, 1)
            },
            Some(PathQuantifier::Optional) => {
                ...
                let expanded = self.expand_single_element(current_path, element, path_type, graph, visited_nodes, visited_edges, 1, 1)?;
            ...
            Some(PathQuantifier::Exact(n)) => {
                ...
                self.expand_single_element(current_path, element, path_type, graph, visited_nodes, visited_edges, *n, *n)
            },
            Some(PathQuantifier::Range { min, max }) => {
                // Between min and max occurrences
                self.expand_single_element(current_path, element, path_type, graph, visited_nodes, visited_edges, *min, *max)
            },
            Some(PathQuantifier::AtLeast(min)) => {
                ...
                self.expand_single_element(current_path, element, path_type, graph, visited_nodes, visited_edges, *min, max_cap)
            },
            Some(PathQuantifier::AtMost(max)) => {
                // At most max occurrences (minimum 0)
                self.expand_single_element(current_path, element, path_type, graph, visited_nodes, visited_edges, 0, *max)
            },
        }
    }
        ...
        for _ in 0..n {
            ...
            for path in current_paths {
                ...
                let edges = match element.direction {
                    ...
                    EdgeDirection::Both | EdgeDirection::Undirected => graph.get_connected_edges(current_node_id),
                };
    ...
    fn compare_values(&self, a: &Value, b: &Value, nulls_first: bool) -> Option<std::cmp::Ordering> {
        use std::cmp::Ordering;
        use crate::storage::Value;
        
        match (a, b) {
            (Value::Number(a), Value::Number(b)) => Some(a.partial_cmp(b).unwrap_or(Ordering::Equal)),
            (Value::String(a), Value::String(b)) => Some(a.cmp(b)),
            (Value::Boolean(a), Value::Boolean(b)) => Some(a.cmp(b)),
            (Value::Null, Value::Null) => Some(Ordering::Equal),
            (Value::Null, _) => Some(if nulls_first { Ordering::Less } else { Ordering::Greater }),
            (_, Value::Null) => Some(if nulls_first { Ordering::Greater } else { Ordering::Less }),
    ...
    fn execute_session_statement(&self, stmt: &SessionStatement) -> Result<QueryResult, ExecutionError> {
            ...
            SessionStatement::SessionSet(set_stmt) => {
                match &set_stmt.clause {
                    SessionSetClause::Graph { graph_expression } => {
                        ...
                        let validated = self.validate_graph_expression_via_catalog(graph_expression);
                        ...
                        if !validated {
                            ...
                            let graph_name = match graph_expression {
                                ...
                                GraphExpression::CurrentGraph => "CURRENT_GRAPH (invalid in SESSION SET)".to_string(),
                            };
                            return Err(ExecutionError::CatalogError(
                                format!("Graph does not exist: {}", graph_name)
                            ));
                        }
                    ...
                    SessionSetClause::Schema { schema_reference } => {
                        ...
                        if !validated {
                            return Err(ExecutionError::CatalogError(
                                format!("Schema does not exist: {}", schema_reference.to_string())
                            ));
                        }
                    ...
                    _ => {
                        ...
                        Err(ExecutionError::UnsupportedOperator(
                            format!("Session clause type not yet implemented: {:?}", set_stmt.clause)
                        ))
                    }
                }
            },
            SessionStatement::SessionReset(_) => {
                Ok(QueryResult::for_session(SessionResult::Reset))
            },
            SessionStatement::SessionClose(_) => {
                Ok(QueryResult::for_session(SessionResult::Close))
            },
        }
    }
    ...
    fn execute_set_operation(&self, set_op: &crate::ast::ast::SetOperation, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
        ...
        log::debug!("Left result: {} rows, Right result: {} rows", left_result.rows.len(), right_result.rows.len());
        ...
        let mut result = match set_op.operation {
            crate::ast::ast::SetOperationType::Union => self.execute_union(left_result, right_result, false), // UNION removes duplicates
            crate::ast::ast::SetOperationType::UnionAll => self.execute_union(left_result, right_result, true), // UNION ALL keeps duplicates
            crate::ast::ast::SetOperationType::Intersect => self.execute_intersect(left_result, right_result, true),
            crate::ast::ast::SetOperationType::IntersectAll => self.execute_intersect(left_result, right_result, false),
            crate::ast::ast::SetOperationType::Except => self.execute_except(left_result, right_result, true),
            crate::ast::ast::SetOperationType::ExceptAll => self.execute_except(left_result, right_result, false),
        }?;
    ...
    fn execute_query_recursive(&self, query: &crate::ast::ast::Query, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
        ...
        match query {
            crate::ast::ast::Query::Basic(basic_query) => {
                ...
                let document = crate::ast::ast::Document {
                    statement: crate::ast::ast::Statement::Query(crate::ast::ast::Query::Basic(basic_query.clone())),
                    location: crate::ast::ast::Location::default(),
                };
                ...
                let mut planner = crate::plan::optimizer::QueryPlanner::new();
                let plan = planner.plan_query(&document)
                    .map_err(|e| ExecutionError::PlanningError(format!("Failed to plan query: {}", e)))?;
                
                // Execute the plan - get default graph if needed
                let graph_names = self.storage.get_graph_names()
                    .map_err(|e| ExecutionError::RuntimeError(format!("Failed to get graph names: {}", e)))?;
                let first_graph_name = graph_names.first()
                    .ok_or_else(|| ExecutionError::RuntimeError("No graphs available for query execution".to_string()))?;
                let graph = self.storage.get_graph(first_graph_name)
            ...
            crate::ast::ast::Query::WithQuery(with_query) => {
                ...
                if let Some(first_segment) = with_query.segments.first() {
                    ...
                    self.execute_query_recursive(&crate::ast::ast::Query::Basic(basic_query), context)
                } else {
                    Err(ExecutionError::InvalidQuery("WITH query has no segments".to_string()))
                }
            },
            ...
            crate::ast::ast::Query::MutationPipeline(_) => {
                ...
                Err(ExecutionError::RuntimeError("Mutation pipelines require session-aware execution".to_string()))
            },
        }
    }

    /// Execute standalone RETURN query: RETURN [DISTINCT|ALL] items [GROUP BY] [HAVING] [ORDER BY] [LIMIT]
    fn execute_return_query(&self, return_query: &crate::ast::ast::ReturnQuery, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
        ...
        for item in &return_query.return_clause.items {
            ...
            let column_name = item.alias.clone().unwrap_or_else(|| {
                ...
                match &item.expression {
                    crate::ast::ast::Expression::Literal(literal) => {
                        match literal {
    ...
    fn execute_let_statement(&self, let_stmt: &crate::ast::ast::LetStatement, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
        ...
        for var_def in &let_stmt.variable_definitions {
            ...
            context.variables.insert(var_def.variable_name.clone(), value.clone());
    ...
    fn execute_for_statement(&self, for_stmt: &crate::ast::ast::ForStatement, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
    ...
    fn execute_filter_statement(&self, _filter_stmt: &crate::ast::ast::FilterStatement, _context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
        ...
        row.add_value(
            ...
            Value::String("FILTER requires a result set context from a previous operation".to_string())
        );
        row.positional_values.push(Value::String("FILTER requires context".to_string()));
        result.rows.push(row);
    ...
    fn execute_mutation_pipeline(
        ...
        log::debug!("Starting mutation pipeline execution with {} segments", pipeline.segments.len());
        log::debug!("Final mutation type: {:?}", pipeline.final_mutation);

        // Execute the pipeline step by step
        if pipeline.segments.is_empty() {
            return Err(ExecutionError::RuntimeError("Mutation pipeline requires at least one segment".to_string()));
        }
        ...
        let mut match_results = self.execute_match_with_context(&first_segment.match_clause, &context)?;
        
        // Apply pre-WITH WHERE clause if present
        if let Some(where_clause) = &first_segment.where_clause {
            match_results = self.apply_where_filter_to_rows(match_results, where_clause, &context)?;
        }
        ...
        for (row_idx, row) in final_rows.iter().enumerate() {
            log::debug!("Processing row {}/{}: {:?}", row_idx + 1, final_rows.len(), row.values.keys().collect::<Vec<_>>());
            match &pipeline.final_mutation {
                ...
                FinalMutation::Set(set_items) => {
                    log::debug!("Executing SET operation on row with {} items", set_items.len());
                    // Apply SET operations to entities in this row
                    for set_item in set_items {
                        match set_item {
                            crate::ast::ast::SetItem::PropertyAssignment { property, value } => {
                                log::debug!("Processing PropertyAssignment: {}.{} = {:?}", property.object, property.property, value);
                                ...
                                match self.apply_property_assignment(property, new_value, row, &row_context) {
                                    Ok(()) => {
                            ...
                            _ => {
                                log::debug!("Handling non-PropertyAssignment SET item: {:?}", set_item);
                ...
                FinalMutation::Delete { expressions: _, detach: _ } => {
        ...
        values.insert("affected_rows".to_string(), Value::Number(affected_count as f64));
    ...
    fn apply_property_assignment(
        ...
        let entity_value = row.values.get(&property.object)
            .ok_or_else(|| ExecutionError::RuntimeError(format!("Variable '{}' not found in row", property.object)))?;
        
        // Extract node ID from the entity value
        let node_id = match entity_value {
            Value::Node(node_ref) => &node_ref.id,
            _ => {
                return Err(ExecutionError::RuntimeError("Property assignment only supported on nodes".to_string()));
            }
        };
        
        // Get mutable access to the graph through the storage manager
        // This follows the same pattern as DataStatementExecutors
        let graph_name = context.get_current_graph_name()
            .ok_or_else(|| ExecutionError::RuntimeError("No graph context available for property assignment".to_string()))?;
        ...
        if let Some(ref storage_manager) = context.storage_manager {
            ...
            let mut graph = storage_manager.get_graph(&graph_name)
                .map_err(|e| ExecutionError::RuntimeError(format!("Failed to get graph: {}", e)))?
                .ok_or_else(|| ExecutionError::RuntimeError("Graph not found".to_string()))?;
            ...
            if let Some(node) = graph.get_node_mut(node_id) {
                ...
                log::debug!("SET {}.{} = {:?} (node_id: {})", property.object, property.property, new_value, node_id);
                ...
                log::debug!("Graph modification completed - needs proper persistence implementation");
            } else {
                return Err(ExecutionError::RuntimeError(format!("Node {} not found in graph", node_id)));
            }
        } else {
            return Err(ExecutionError::RuntimeError("No storage manager available for property assignment".to_string()));
        }
    ...
    fn apply_where_filter_to_rows_vec(
        ...
        for row in rows {
            ...
            let condition_result = self.evaluate_expression(&where_clause.condition, &row_context)?;
    ...
    fn execute_unwind_statement(&self, unwind_stmt: &crate::ast::ast::UnwindStatement, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
        use crate::storage::Value;
    ...
    fn validate_set_operation_compatibility(&self, left: &QueryResult, right: &QueryResult) -> Result<(), ExecutionError> {
        ...
        let effective_left_count = if left.variables.is_empty() && left.rows.is_empty() && !right.variables.is_empty() {
            right.variables.len() // Assume same variable count as right side
        } else {
            left.variables.len()
        };
        
        let effective_right_count = if right.variables.is_empty() && right.rows.is_empty() && !left.variables.is_empty() {
    ...
    fn convert_to_positional_rows_aligned(&self, rows: Vec<Row>, _source_variables: &[String], target_variables: &[String]) -> Vec<Row> {
        rows.into_iter().map(|row| {
            let mut positional_values = Vec::new();
    ...
    fn execute_union(&self, left: QueryResult, right: QueryResult, distinct: bool) -> Result<QueryResult, ExecutionError> {
        ...
        if use_identity {
            ...
            if distinct {
                ...
                for right_row in right.rows {
                    ...
                    for result_row in &result_rows {
                        if result_row.has_entities() && self.rows_equal_by_identity(&right_row, result_row) {
                            found = true;
                            break;
                        }
                    }
            ...
            let (vars, left_positional, right_positional) = if left.variables.is_empty() && !right.variables.is_empty() {
                // Left is empty, use right variables as target
                let left_pos = self.convert_to_positional_rows_aligned(left.rows, &left.variables, &right.variables);
                ...
                let right_pos = self.convert_to_positional_rows_aligned(right.rows, &right.variables, &left.variables);
    ...
    fn execute_intersect(&self, left: QueryResult, right: QueryResult, distinct: bool) -> Result<QueryResult, ExecutionError> {
        log::debug!("=== EXECUTE_INTERSECT CALLED");
        log::debug!("Left: {} rows, variables: {:?}", left.rows.len(), left.variables);
        log::debug!("Right: {} rows, variables: {:?}", right.rows.len(), right.variables);

        // Check if we should use identity-based or value-based comparison
        let use_identity = left.rows.iter().any(|r| r.has_entities()) ||
                          right.rows.iter().any(|r| r.has_entities());

        log::debug!("Using {} comparison", if use_identity { "identity-based" } else { "value-based" });
        ...
        if use_identity {
            ...
            for (left_idx, left_row) in left.rows.iter().enumerate() {
                ...
                for (right_idx, right_row) in right.rows.iter().enumerate() {
                    ...
                    if self.rows_equal_by_identity(left_row, right_row) {
                        log::debug!("MATCH (identity): Left row {} matches right row {}", left_idx, right_idx);
                        result_rows.push(left_row.clone());
            ...
            let right_positional = self.convert_to_positional_rows_aligned(right.rows, &right.variables, &left.variables);

            log::debug!("DEBUG INTERSECT COMPARISON:");
            for (i, left_row) in left_positional.iter().enumerate() {
                log::debug!("Left row {}: positional={:?}", i, left_row.positional_values);
            }
            for (i, right_row) in right_positional.iter().enumerate() {
                log::debug!("Right row {}: positional={:?}", i, right_row.positional_values);
            }
            ...
            for (left_idx, left_row) in left_positional.iter().enumerate() {
                ...
                for (right_idx, right_row) in right_positional.iter().enumerate() {
                    ...
                    log::debug!("Comparing left row {} with right row {}: equal={}", left_idx, right_idx, rows_equal);
                    if rows_equal {
                        log::debug!("MATCH: Left row {} matches right row {}", left_idx, right_idx);
                        result_rows.push(left_row.clone());
                        break; // Found match, no need to check other right rows
                    }
                }
    ...
    fn extract_node_edge_identities(&self, row: &Row) -> Vec<String> {
        ...
        log::debug!("DEBUG: Extracting identities from row with keys: {:?}", row.values.keys().collect::<Vec<_>>());
        for (key, value) in &row.values {
    ...
    fn execute_except(&self, left: QueryResult, right: QueryResult, distinct: bool) -> Result<QueryResult, ExecutionError> {
        use std::collections::HashSet;
    ...
    fn infer_literal_type(&self, literal: &crate::ast::ast::Literal) -> Result<GqlType, ExecutionError> {
        match literal {
    ...
    fn apply_coercion(
        ...
        match TypeCoercion::coerce(left_type, right_type) {
            ...
            Ok(strategy) => {
                ...
                let coerced_left = self.apply_coercion_strategy(left_val, left_type, right_type, &strategy)?;
            ...
            Err(_) => {
                ...
                match TypeCoercion::coerce(right_type, left_type) {
                    ...
                    Ok(strategy) => {
                        ...
                        let coerced_right = self.apply_coercion_strategy(right_val, right_type, left_type, &strategy)?;
                        Ok((left_val.clone(), coerced_right))
                    }
                    Err(_) => {
                        // No coercion possible, try to find common type as fallback
                        if let Some(common_type) = TypeCoercion::find_common_type(left_type, right_type) {
                            let coerced_left = self.coerce_value_to_type(left_val, &common_type)?;
                            let coerced_right = self.coerce_value_to_type(right_val, &common_type)?;
    ...
    fn apply_coercion_strategy(
        ...
        match strategy {
            ...
            CoercionStrategy::StringToOther => {
                ...
                match (value, to_type) {
                    (Value::String(s), GqlType::Integer) => {
                        s.parse::<i64>()
                            .map(|i| Value::Number(i as f64))
                            .map_err(|_| ExecutionError::RuntimeError(format!("Cannot coerce '{}' to integer", s)))
                    }
                    (Value::String(s), GqlType::Double) => {
                        s.parse::<f64>()
                            .map(Value::Number)
                            .map_err(|_| ExecutionError::RuntimeError(format!("Cannot coerce '{}' to number", s)))
                    }
                    (Value::String(s), GqlType::Boolean) => {
                        match s.to_lowercase().as_str() {
                            "true" => Ok(Value::Boolean(true)),
                            "false" => Ok(Value::Boolean(false)),
                            _ => Err(ExecutionError::RuntimeError(format!("Cannot coerce '{}' to boolean", s)))
                        }
                    }
                    _ => Ok(value.clone()),
                }
    ...
    fn coerce_value_to_type(&self, value: &Value, target_type: &GqlType) -> Result<Value, ExecutionError> {
        match (value, target_type) {
            ...
            // Numeric to string
            (Value::Number(n), GqlType::String { .. }) => {
                Ok(Value::String(n.to_string()))
            },
            
            // Boolean to string
            (Value::Boolean(b), GqlType::String { .. }) => {
    ...
    fn validate_and_execute_function(
        ...
        match func_name.to_uppercase().as_str() {
            ...
            _ => {
                ...
                Ok(arg_types.first().cloned().unwrap_or(GqlType::String { max_length: None }))
            }
        }
    }
    ...
    fn execute_function_call(
        ...
        log::debug!("  Context has storage_manager: {}", context.storage_manager.is_some());
        log::debug!("  Context has current_graph: {}", context.current_graph.is_some());
        log::debug!("  Context session_id: '{}'", context.session_id);
        log::debug!("  Context get_current_graph_name(): {:?}", context.get_current_graph_name());
        ...
        log::debug!("    storage_manager: {}", func_context.storage_manager.is_some());
        log::debug!("    current_graph: {}", func_context.current_graph.is_some());
        ...
        match self.function_registry.get(func_name) {
            Some(function) => {
                function.execute(&func_context)
                    .map_err(|e| ExecutionError::RuntimeError(format!("Function execution error: {}", e)))
            }
            None => Err(ExecutionError::RuntimeError(format!("Unknown function: {}", func_name)))
        }
    }
    
    /// Evaluate a unary operation
    fn evaluate_unary_op(&self, operator: &crate::ast::ast::Operator, operand: Value) -> Result<Value, ExecutionError> {
        match operator {
            crate::ast::ast::Operator::Not => {
                match operand {
                    Value::Boolean(b) => Ok(Value::Boolean(!b)),
                    _ => Err(ExecutionError::RuntimeError("NOT operator requires boolean operand".to_string())),
                }
            },
            crate::ast::ast::Operator::Minus => {
                match operand {
                    Value::Number(n) => Ok(Value::Number(-n)),
                    _ => Err(ExecutionError::RuntimeError("Unary minus requires numeric operand".to_string())),
                }
            },
            _ => Err(ExecutionError::RuntimeError(format!("Unsupported unary operator: {:?}", operator))),
        }
    }
    ...
    fn row_contains_null(&self, row: &Row) -> bool {
        ...
        if !row.positional_values.is_empty() {
            row.positional_values.iter().any(|value| matches!(value, Value::Null))
        } else {
            row.values.values().any(|value| matches!(value, Value::Null))
        }
    }

    /// Apply ORDER BY clause to query result
    fn apply_order_by(&self, mut result: QueryResult, order_clause: &crate::ast::ast::OrderClause, context: &ExecutionContext) -> Result<QueryResult, ExecutionError> {
        use std::cmp::Ordering;
        ...
        result.rows.sort_by(|a, b| {
            for order_item in &order_clause.items {
                ...
                let ordering = match (val_a, val_b) {
                    ...
                    (Ok(Value::Number(n1)), Ok(Value::Number(n2))) => n1.partial_cmp(&n2).unwrap_or(Ordering::Equal),
    ...
    fn apply_limit(&self, mut result: QueryResult, limit_clause: &crate::ast::ast::LimitClause) -> Result<QueryResult, ExecutionError> {
        let offset = limit_clause.offset.unwrap_or(0);
    ...
    fn expand_select_items(&self, select_items: &SelectItems, graph: &Arc<GraphCache>) -> Result<Vec<ReturnItem>, ExecutionError> {
        match select_items {
    ...
    fn execute_declare_statement(&self, declare_stmt: &DeclareStatement, context: &ExecutionContext) -> Result<QueryResult, ExecutionError> {
        ...
        for var_decl in &declare_stmt.variable_declarations {
            ...
            declared_vars.push(format!("{}: {} = {:?}", 
                ...
                var_decl.type_spec, 
                var_decl.initial_value.as_ref().map(|_| "initialized").unwrap_or("default")));
        }
        ...
        Ok(QueryResult {
            ...
            rows: declared_vars.into_iter().map(|var| {
    ...
    fn execute_next_statement(&self, next_stmt: &NextStatement, context: &mut ExecutionContext, graph_expr: Option<&GraphExpression>) -> Result<QueryResult, ExecutionError> {
    ...
    fn execute_at_location_statement(&self, at_stmt: &AtLocationStatement, context: &mut ExecutionContext) -> Result<QueryResult, ExecutionError> {
                        ...
                        if let Some(prev) = &last_result {
                            if let Some(first_row) = prev.rows.first() {
                                ...
                                for item in &yield_clause.items {
                                    ...
                                    let val_opt = first_row.values.get(column_name)
                                        .cloned()
                                        .or_else(|| {
                                            prev.variables.iter()
                                                .position(|v| v == column_name)
                                                .and_then(|idx| first_row.get_value_at_position(idx).cloned())
                                        });
                                ...
                                let result = QueryResult {
                                    rows: vec![row],
                                    variables: yield_clause.items.iter().map(|i| i.alias.as_ref().unwrap_or(&i.column_name).clone()).collect(),
                                    rows_affected: 1,
            session_result: None,
            warnings: Vec::new(),
    ...
    fn execute_procedure_body_statement(&self, procedure_body: &ProcedureBodyStatement, context: &mut ExecutionContext, graph_expr: Option<&GraphExpression>, session: Option<&Arc<std::sync::RwLock<UserSession>>>) -> Result<QueryResult, ExecutionError> {
        ...
        let initial_result = self.execute_statement(&procedure_body.initial_statement, context, graph_expr, session)?;
        ...
        for chained in &procedure_body.chained_statements {
            ...
            let chained_result = self.execute_statement(&chained.statement, context, graph_expr, session)?;
    ...
    pub fn set_session_parameter(&self, parameter: &str, value: Value) -> Result<(), ExecutionError> {
        // TODO: Implement session parameter storage through SessionManager
        // For now, parameters are not persisted across queries
        log::debug!("Session parameter '{}' set to {:?} (not persisted)", parameter, value);
        Ok(())
    }
    ...
    fn check_type_match(&self, value: &Value, type_spec: &crate::ast::ast::TypeSpec) -> Result<bool, ExecutionError> {
    ...
    fn execute_transaction_statement(&self, statement: &TransactionStatement, context: &mut ExecutionContext, session: Option<&Arc<std::sync::RwLock<UserSession>>>) -> Result<QueryResult, ExecutionError> {
        ...
        let result = TransactionCoordinator::execute_transaction_statement(
            statement,
            context,
            self, // Pass the QueryExecutor for WAL logging
        )?;
    ...
    pub fn log_transaction_operation(&self, operation: UndoOperation) -> Result<(), ExecutionError> {
        let current_txn = self.current_transaction.read()
        ...
        if let Some(txn_id) = *current_txn {
            ...
            let mut logs = self.transaction_logs.write()
                .map_err(|_| ExecutionError::RuntimeError("Failed to acquire transaction logs lock".to_string()))?;
            
            if let Some(log) = logs.get_mut(&txn_id) {
                log.log_operation(operation);
                log::info!("LOG_TXN: Operation logged. Total operations: {}", log.operation_count);
            } else {
    ...
    fn is_scalar_only_projection(&self, node: &PhysicalNode) -> bool {
        match node {
            PhysicalNode::Project { expressions, .. } => {
                ...
                expressions.iter().all(|proj| self.is_scalar_expression(&proj.expression))
            },
            _ => false
        }
    }
    ...
    fn is_scalar_expression(&self, expr: &Expression) -> bool {
        match expr {
            ...
            Expression::FunctionCall(func) => {
                ...
                func.arguments.iter().all(|arg| self.is_scalar_expression(arg))
            },
            Expression::Case(case_expr) => {
                match &case_expr.case_type {
                    CaseType::Simple(simple_case) => {
                        ...
                        simple_case.when_branches.iter().all(|when| 
                            when.when_values.iter().all(|val| self.is_scalar_expression(val)) && 
                            self.is_scalar_expression(&when.then_expression)
                        ) &&
                        simple_case.else_expression.as_ref().map_or(true, |e| self.is_scalar_expression(e))
                    },
                    CaseType::Searched(searched_case) => {
                        ...
                        searched_case.else_expression.as_ref().map_or(true, |e| self.is_scalar_expression(e))
                    }
                }
            },
            Expression::Binary(binary_expr) => {
                self.is_scalar_expression(&binary_expr.left) && self.is_scalar_expression(&binary_expr.right)
            },
            Expression::Unary(unary_expr) => {
                self.is_scalar_expression(&unary_expr.expression)
            },
            Expression::Cast(cast_expr) => {
                self.is_scalar_expression(&cast_expr.expression)
            },
    ...
    fn execute_generic_function_node(&self, node: &PhysicalNode, context: &mut ExecutionContext, _graph: Option<&Arc<GraphCache>>) -> Result<Vec<Row>, ExecutionError> {
        match node {
            PhysicalNode::GenericFunction { function_name, arguments, input, .. } => {
            ...
            _ => Err(ExecutionError::RuntimeError("Expected GenericFunction node".to_string()))
        }
    }

    /// Execute a projection node without graph context (scalar-only)
    fn execute_project_node(&self, node: &PhysicalNode, context: &mut ExecutionContext, _graph: Option<&Arc<GraphCache>>) -> Result<Vec<Row>, ExecutionError> {
        match node {
            PhysicalNode::Project { expressions, input, .. } => {
                ...
                for _row in input_rows {
                    ...
                    for proj in expressions {
                        ...
                        let alias = proj.alias.clone().unwrap_or_else(|| format!("col_{}", projected_values.len()));
            ...
            _ => Err(ExecutionError::RuntimeError("Expected Project node".to_string()))
        }
    }
        ...
        match graph_expression {
            ...
            GraphExpression::Reference(path) => {
                ...
                if path.segments.len() == 2 {
                    ...
                    let graph_name = path.segments.last().map(|s| s.as_str()).unwrap_or("unknown");
            ...
            GraphExpression::Union { left: _, right: _, all: _ } => {
    ...
    fn validate_schema_exists_via_catalog(&self, schema_reference: &CatalogPath) -> bool {
        ...
        let schema_name = schema_reference.segments.last().map(|s| s.as_str()).unwrap_or("unknown");
        ...
        if let Ok(mut catalog_manager) = self.catalog_manager.write() {
            ...
            match result {
                Ok(CatalogResponse::Query { results }) => {
                    results.get("exists").and_then(|v| v.as_bool()).unwrap_or(false)
                }
                _ => false,
            }
        } else {
    ...
    fn extract_variables_from_node(&self, node: &PhysicalNode) -> Option<Vec<String>> {
        match node {
            PhysicalNode::Project { expressions, .. } => {
                let variables: Vec<String> = expressions.iter().map(|expr| {
            ...
            PhysicalNode::HashAggregate { group_by, aggregates, .. } |
            PhysicalNode::SortAggregate { group_by, aggregates, .. } => {
    ...
    fn evaluate_pattern_expression(
        ...
        let _graph_name = context.get_graph_name()
            .map_err(|_| ExecutionError::RuntimeError("No graph context available for pattern evaluation".to_string()))?;
        ...
        let has_nodes = pattern_expr.pattern.elements.iter().any(|elem| {
            matches!(elem, crate::ast::ast::PatternElement::Node(_))
        });
        
        let has_edges = pattern_expr.pattern.elements.iter().any(|elem| {
            matches!(elem, crate::ast::ast::PatternElement::Edge(_))
        });
    ...
    fn execute_unwind(
        ...
        let input_rows = if let Some(input_node) = input {
            if let Some(graph_cache) = graph {
                ...
                return Err(ExecutionError::RuntimeError("UNWIND requires graph context when processing input".to_string()));
            }
        } else {
