...
use crate::txn::log::{UndoOperation, TransactionLog};
...
impl TransactionStatementExecutor for RollbackExecutor {
    fn execute_transaction_operation(
        ...
        let transaction_state = context.transaction_state()
            .ok_or_else(|| ExecutionError::RuntimeError("No transaction state available".to_string()))?;
        
        // Get the storage manager from context for applying undo operations
        let storage_manager = context.storage_manager
            .as_ref()
            .ok_or_else(|| ExecutionError::RuntimeError("No storage manager available".to_string()))?;
...
impl RollbackExecutor {
    ...
    pub fn execute_rollback(
        ...
        let current_txn = current_transaction.read()
        ...
        let txn_id = match *current_txn {
            ...
            None => {
                drop(current_txn);
                return Err(ExecutionError::RuntimeError("No transaction in progress".to_string()));
            }
        };
        drop(current_txn);
        ...
        // Apply undo operations from the transaction log
        {
            ...
            if let Some(log) = logs.get(&txn_id) {
                ...
                drop(logs);
                return Err(ExecutionError::RuntimeError("No transaction log available".to_string()));
            }
        ...
        {
            let mut current_txn = current_transaction.write()
                .map_err(|_| ExecutionError::RuntimeError("Failed to acquire transaction lock".to_string()))?;
            *current_txn = None;
        }
    ...
    fn apply_undo_operation(
        ...
        let mut graph = unified_storage.get_graph(graph_name)
            .map_err(|e| ExecutionError::StorageError(format!("Failed to get graph during rollback: {}", e)))?
            .ok_or_else(|| ExecutionError::StorageError(format!("Graph not found during rollback: {}", graph_name)))?;
        ...
        unified_storage.save_graph(graph_name, graph)
            .map_err(|e| ExecutionError::StorageError(format!("Failed to save rollback changes to storage: {}", e)))?;
        
        log::debug!("ROLLBACK: Applied undo operation for graph '{}'", graph_name);
    ...
    fn apply_undo_to_graph(graph: &mut GraphCache, undo_op: &UndoOperation) -> Result<(), ExecutionError> {
        ...
        match undo_op {
            UndoOperation::InsertNode { node_id, .. } => {
                ...
                graph.remove_node(node_id)
                    .map_err(|e| ExecutionError::StorageError(format!("Failed to remove node during rollback: {}", e)))?;
            ...
            UndoOperation::InsertEdge { edge_id, .. } => {
                ...
                graph.remove_edge(edge_id)
                    .map_err(|e| ExecutionError::StorageError(format!("Failed to remove edge during rollback: {}", e)))?;
                log::debug!("ROLLBACK: Removed edge {} (undo insert)", edge_id);
            },
            UndoOperation::UpdateNode { node_id, old_properties, old_labels, .. } => {
                // Undo: restore the old properties and labels
                if let Some(node) = graph.get_node_mut(node_id) {
                    node.properties = old_properties.clone();
                    node.labels = old_labels.clone();
                    log::debug!("ROLLBACK: Restored node {} properties and labels (undo update)", node_id);
                } else {
                    log::warn!("ROLLBACK: Node {} not found for property restoration", node_id);
                }
            },
            UndoOperation::UpdateEdge { edge_id, old_properties, old_label, .. } => {
                ...
                if let Some(edge) = graph.get_edge_mut(edge_id) {
                    ...
                    log::debug!("ROLLBACK: Restored edge {} properties and label (undo update)", edge_id);
                } else {
                    log::warn!("ROLLBACK: Edge {} not found for property restoration", edge_id);
                }
            },
            UndoOperation::DeleteNode { node_id, deleted_node, .. } => {
                // Undo: restore the deleted node
                graph.add_node(deleted_node.clone())
                    .map_err(|e| ExecutionError::StorageError(format!("Failed to restore node during rollback: {}", e)))?;
                log::debug!("ROLLBACK: Restored node {} (undo delete)", node_id);
            },
            UndoOperation::DeleteEdge { edge_id, deleted_edge, .. } => {
                // Undo: restore the deleted edge
                graph.add_edge(deleted_edge.clone())
                    .map_err(|e| ExecutionError::StorageError(format!("Failed to restore edge during rollback: {}", e)))?;
