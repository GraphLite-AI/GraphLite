...
use crate::ast::ast::{MatchDeleteStatement, PatternElement, Expression, Literal};
...
impl MatchDeleteExecutor {
    ...
    fn evaluate_where_clause_on_combination(
        ...
        if let Some(computed_values) = computed_values {
            ...
            Self::evaluate_where_expression_on_combination(node_combination, edge_combination, &where_clause.condition)
        }
    }
    ...
    fn evaluate_where_expression_on_combination(
        ...
        match expr {
            Expression::Binary(binary_op) => {
                let left_val = Self::evaluate_expression_on_combination(node_combination, edge_combination, &binary_op.left);
                let right_val = Self::evaluate_expression_on_combination(node_combination, edge_combination, &binary_op.right);
                ...
                match binary_op.operator {
                    ...
                    crate::ast::ast::Operator::And => {
                        Self::evaluate_where_expression_on_combination(node_combination, edge_combination, &binary_op.left) &&
                        Self::evaluate_where_expression_on_combination(node_combination, edge_combination, &binary_op.right)
                    },
...
impl StatementExecutor for MatchDeleteExecutor {
    ...
    fn operation_description(&self, context: &ExecutionContext) -> String {
        ...
        let prefix = if self.statement.detach { "MATCH DETACH " } else { "MATCH " };
...
impl DataStatementExecutor for MatchDeleteExecutor {
    ...
    fn execute_modification(
        ...
        log::debug!("MATCH-DELETE: Processing {} patterns", self.statement.match_clause.patterns.len());
        ...
        if all_node_matches.len() == 1 {
            ...
            if !edge_matches.is_empty() && node_matches.len() == edge_matches.len() {
                for (node_match, edge_match) in node_matches.into_iter().zip(edge_matches.into_iter()) {
        ...
        let filtered_combined = if let Some(ref with_clause) = self.statement.with_clause {
            ...
            let edges: Vec<Edge> = combined_matches.iter()
            ...
            let with_result = WithClauseProcessor::process_with_clause(with_clause, &node_bindings, &edges, &temp_context)?;
            ...
            log::debug!("DELETE DEBUG: WITH result - has_aggregation: {}, group_results: {}", 
                     with_result.has_aggregation, with_result.group_results.len());
            if with_result.has_aggregation {
                log::debug!("MATCH_DELETE: Processing {} groups from WITH aggregation", with_result.group_results.len());
                for (i, group_result) in with_result.group_results.iter().enumerate() {
                    log::debug!("  Group {}: {} variables", i, group_result.variable_bindings.len());
                ...
                log::debug!("MATCH_DELETE: Total qualifying nodes: {}", qualifying_node_ids.len());
                log::debug!("DELETE DEBUG: Qualifying node IDs: {:?}", qualifying_node_ids);
            } else {
            ...
            combined_matches.into_iter()
                .filter(|(node_combo, edge_combo)| {
                    if let Some(ref where_clause) = self.statement.where_clause {
                        Self::evaluate_where_clause_on_combination(node_combo, edge_combo, where_clause, None)
                    } else {
                        true
                    }
                })
        ...
        log::debug!("MATCH-DELETE: {} combinations passed WHERE clause", filtered_combined.len());
        log::debug!("DELETE DEBUG: {} combinations passed filtering", filtered_combined.len());
                ...
                match expr {
                    Expression::Variable(var) => {
                        ...
                        if let Some(edge_to_delete) = edge_combination.get(var_name) {
                            ...
                            log::debug!("MATCH DELETE: Deleted edge {} matching variable {}", edge_id, var_name);
                        ...
                        // Then check if this variable refers to a node
                            ...
                            log::debug!("MATCH DELETE: Deleted node {} matching variable {}", node_id, var_name);
                    ...
                    _ => {
                        log::warn!("Complex expressions in MATCH DELETE not yet supported: {:?}", expr);
                    }
                }
            }
        }
