...
use crate::ast::ast::{MatchRemoveStatement, PatternElement, Expression, Literal, RemoveItem, LabelFactor};
...
impl DataStatementExecutor for MatchRemoveExecutor {
    ...
    fn execute_modification(
        ...
        log::debug!("MATCH-REMOVE: Processing {} patterns", self.statement.match_clause.patterns.len());
        ...
        log::debug!("MATCH-REMOVE: Found {} variable combinations", variable_combinations.len());
        ...
        let filtered_combinations: Vec<HashMap<String, Node>> = if let Some(ref with_result) = with_result {
                ...
                variable_combinations.into_iter()
                    .filter(|combination| {
                        if let Some(ref where_clause) = self.statement.where_clause {
                            ...
                            Self::evaluate_where_clause_on_combination(combination, where_clause, computed_values)
                        } else {
                            true
                        }
                    })
            ...
            variable_combinations.into_iter()
                .filter(|combination| {
                    if let Some(ref where_clause) = self.statement.where_clause {
                        let result = Self::evaluate_where_clause_on_combination(combination, where_clause, None);
                        log::debug!("DEBUG: WHERE clause evaluation for combination {:?}: {}", 
                            combination.iter().map(|(k,v)| (k.clone(), v.properties.get("name").cloned())).collect::<Vec<_>>(), result);
                        result
                    } else {
                        true
                    }
        ...
        log::debug!("MATCH-REMOVE: {} combinations passed WHERE clause", filtered_combinations.len());
                ...
                match item {
                    RemoveItem::Property(property_access) => {
                        ...
                        if property_access.object == var_name {
                            ...
                            let (old_properties, old_labels, has_property) = if let Some(node) = graph.get_node(&matched_node.id) {
                                let mut old_props = HashMap::new();
                                let has_prop = if let Some(old_val) = node.properties.get(&property_access.property) {
                            ...
                            if has_property {
                                ...
                                if let Some(node_mut) = graph.get_node_mut(&matched_node.id) {
                                    ...
                                    log::debug!("DEBUG: Node after removal - has score: {:?}", node_mut.properties.contains_key("score"));
                                    log::debug!("MATCH REMOVE: Removed property {} from node {}", 
                                               property_access.property, matched_node.id);
                                    removed_count += 1;
                    ...
                    RemoveItem::Label { variable, labels } => {
                        ...
                        if self.statement.match_clause.patterns.iter()
                            ...
                            let old_labels = if let Some(node) = graph.get_node(&matched_node.id) {
                                node.labels.clone()
                            } else {
                                continue;
                            };
                    ...
                    RemoveItem::Variable(variable) => {
                        log::warn!("Variable removal in MATCH REMOVE not yet supported: {}", variable);
