...
use crate::ast::ast::{MatchSetStatement, PatternElement, Expression, SetItem};
...
impl MatchSetExecutor {
    ...
    fn evaluate_expression(
        ...
        if let Expression::Variable(var) = expr {
            if let Some(computed_values) = computed_values {
                if let Some(value) = computed_values.get(&var.name) {
                    log::debug!("MATCH-SET: Using computed value for variable '{}': {:?}", var.name, value);
        ...
        if let Some(value) = Self::evaluate_expression_on_combination(combination, expr) {
            log::debug!("MATCH-SET: Evaluated expression on combination: {:?}", value);
            return Some(value);
        }
    ...
    fn match_path_pattern(
        ...
        if pattern.elements.len() == 3 {
            ...
            if let (Some(PatternElement::Node(source_pattern)), 
                    Some(PatternElement::Edge(edge_pattern)),
                    Some(PatternElement::Node(target_pattern))) = 
                (pattern.elements.get(0), pattern.elements.get(1), pattern.elements.get(2)) {
                ...
                for edge in edges {
                    ...
                    if let (Some(source_node), Some(target_node)) = 
                        (graph.get_node(&edge.from_node), graph.get_node(&edge.to_node)) {
                        
                        // Check if source node matches pattern
                        let source_matches = Self::node_matches_pattern(&source_node, source_pattern);
                        let target_matches = Self::node_matches_pattern(&target_node, target_pattern);
    ...
    fn node_matches_pattern(
        ...
        if !node_pattern.labels.is_empty() {
            let has_required_label = node_pattern.labels.iter()
    ...
    fn evaluate_where_expression_on_combination(
        ...
        match expr {
            Expression::Binary(binary_op) => {
                let left_val = Self::evaluate_expression_on_combination(combination, &binary_op.left);
                let right_val = Self::evaluate_expression_on_combination(combination, &binary_op.right);
                ...
                match binary_op.operator {
                    ...
                    crate::ast::ast::Operator::And => {
                        ...
                        let left_bool = Self::evaluate_where_expression_on_combination(combination, &binary_op.left);
                        let right_bool = Self::evaluate_where_expression_on_combination(combination, &binary_op.right);
    ...
    fn evaluate_expression_on_combination(
        ...
        match expr {
            ...
            Expression::Literal(literal) => {
                Some(Self::literal_to_value(literal))
            },
            Expression::Binary(binary_expr) => {
                // Recursively evaluate left and right operands
                let left_val = Self::evaluate_expression_on_combination(combination, &binary_expr.left)?;
                let right_val = Self::evaluate_expression_on_combination(combination, &binary_expr.right)?;

                // Apply the binary operation
                use crate::ast::ast::Operator;
                match (&left_val, &binary_expr.operator, &right_val) {
                    // Numeric operations
                    (Value::Number(l), Operator::Plus, Value::Number(r)) => Some(Value::Number(l + r)),
                    (Value::Number(l), Operator::Minus, Value::Number(r)) => Some(Value::Number(l - r)),
                    (Value::Number(l), Operator::Star, Value::Number(r)) => Some(Value::Number(l * r)),
                    ...
                    (Value::Number(l), Operator::Percent, Value::Number(r)) => Some(Value::Number(l % r)),

                    // String concatenation
                    (Value::String(l), Operator::Plus, Value::String(r)) => Some(Value::String(format!("{}{}", l, r))),

                    // Comparison operations
                    (Value::Number(l), Operator::Equal, Value::Number(r)) => Some(Value::Boolean(l == r)),
                    (Value::Number(l), Operator::NotEqual, Value::Number(r)) => Some(Value::Boolean(l != r)),
                    (Value::Number(l), Operator::LessThan, Value::Number(r)) => Some(Value::Boolean(l < r)),
                    (Value::Number(l), Operator::LessEqual, Value::Number(r)) => Some(Value::Boolean(l <= r)),
                    (Value::Number(l), Operator::GreaterThan, Value::Number(r)) => Some(Value::Boolean(l > r)),
                    (Value::Number(l), Operator::GreaterEqual, Value::Number(r)) => Some(Value::Boolean(l >= r)),

                    (Value::String(l), Operator::Equal, Value::String(r)) => Some(Value::Boolean(l == r)),
                    (Value::String(l), Operator::NotEqual, Value::String(r)) => Some(Value::Boolean(l != r)),

                    (Value::Boolean(l), Operator::Equal, Value::Boolean(r)) => Some(Value::Boolean(l == r)),
                    (Value::Boolean(l), Operator::NotEqual, Value::Boolean(r)) => Some(Value::Boolean(l != r)),

                    // Logical operations
                    (Value::Boolean(l), Operator::And, Value::Boolean(r)) => Some(Value::Boolean(*l && *r)),
                    (Value::Boolean(l), Operator::Or, Value::Boolean(r)) => Some(Value::Boolean(*l || *r)),

                    _ => {
                        log::warn!("Unsupported binary operation: {:?} {:?} {:?}", left_val, binary_expr.operator, right_val);
                        None
                    }
                }
            },
...
impl DataStatementExecutor for MatchSetExecutor {
    ...
    fn execute_modification(
        ...
        log::debug!("MATCH-SET: Starting execute_modification with {} patterns", self.statement.match_clause.patterns.len());
        log::debug!("MATCH-SET: SET items count: {}", self.statement.items.len());
        log::debug!("MATCH-SET: Graph has {} nodes", graph.node_count().unwrap_or(0));
        ...
        if all_node_matches.len() == 1 {
            ...
            for (node_matches, edge_matches) in all_node_matches.into_iter().zip(all_edge_matches.into_iter()) {
                let mut new_node_combinations = Vec::new();
                let mut new_edge_combinations = Vec::new();
                
                for (existing_node_combo, existing_edge_combo) in node_combinations.iter().zip(edge_combo_list.iter()) {
        ...
        log::debug!("MATCH-SET: Found {} variable combinations", variable_combinations.len());
        ...
        let with_result = if let Some(ref with_clause) = self.statement.with_clause {
            ...
            let edges: Vec<Edge> = edge_combinations.iter()
                .flat_map(|combo| combo.values().cloned())
                .collect();
                
            let temp_context = context.clone()
                .with_function_registry(Arc::new(FunctionRegistry::new()));
            Some(WithClauseProcessor::process_with_clause(with_clause, &node_bindings, &edges, &temp_context)?)
        } else {
            None
        };
        ...
        let combined_combinations: Vec<(HashMap<String, Node>, HashMap<String, Edge>)> = if variable_combinations.is_empty() {
            Vec::new()
        } else if edge_combinations.is_empty() {
            // Node-only patterns: create combinations with empty edge maps
            variable_combinations.into_iter()
        ...
        let filtered_combinations: Vec<(HashMap<String, Node>, HashMap<String, Edge>)> = if let Some(ref with_result) = with_result {
            log::debug!("MATCH_SET: with_result present - has_aggregation: {}, group_results count: {}",
                       with_result.has_aggregation, with_result.group_results.len());
            log::debug!("MATCH_SET: Main variable_bindings has {} variables", with_result.variable_bindings.len());
            ...
            if with_result.has_aggregation {
                if with_result.group_results.is_empty() {
                    ...
                    return Ok((UndoOperation::UpdateNode {
                        graph_path: graph_name,
                    ...
                    for node_combo in group_combinations {
                        ...
                        for (node_combination, edge_combination) in &combined_combinations {
                            if node_combination == &node_combo {
                                filtered_groups.push((node_combo.clone(), edge_combination.clone()));
                                break;
                            }
                        }
                    }
                ...
                combined_combinations.into_iter()
                    .filter(|(node_combination, _edge_combination)| {
                        if let Some(ref where_clause) = self.statement.where_clause {
                            ...
                            Self::evaluate_where_clause_on_combination(node_combination, where_clause, computed_values)
                        } else {
                            true
                        }
                    })
            ...
            combined_combinations.into_iter()
                .filter(|(node_combination, _edge_combination)| {
                    if let Some(ref where_clause) = self.statement.where_clause {
                        Self::evaluate_where_clause_on_combination(node_combination, where_clause, None)
                    } else {
                        true
                    }
                })
        ...
        log::debug!("MATCH-SET: {} combinations passed WHERE clause", filtered_combinations.len());
        ...
        for (combo_idx, (combination, edge_combination)) in filtered_combinations.iter().enumerate() {
            log::debug!("MATCH-SET: Processing combination {}: {:?}", combo_idx, combination.keys().collect::<Vec<_>>());
            log::debug!("MATCH-SET: Edge combination has {} variables", edge_combination.len());
            ...
            for item in &self.statement.items {
                match item {
                    SetItem::PropertyAssignment { property, value } => {
                        log::debug!("MATCH-SET: SET property assignment: {}.{} = {:?}", property.object, property.property, value);
            ...
            let mut node_old_states: HashMap<String, (HashMap<String, Value>, Vec<String>)> = HashMap::new();
            ...
            for (property, new_value) in evaluated_items {
                        ...
                        for (var_name, matched_node) in combination {
                            log::debug!("MATCH-SET: Checking node variable '{}' with node '{}'", var_name, matched_node.id);
                            ...
                            if node_property_matches {
                                ...
                                if let Some(node_mut) = graph.get_node_mut(&matched_node.id) {
                                    ...
                                    if let Some((old_properties, old_labels)) = node_old_states.get(&matched_node.id) {
                        ...
                        if !property_applied {
                            for (var_name, matched_edge) in edge_combination {
                                log::debug!("MATCH-SET: Checking edge variable '{}' with edge '{}'", var_name, matched_edge.id);
                                ...
                                if edge_property_matches {
                                    ...
                                    let old_properties = if let Some(edge) = graph.get_edge(&matched_edge.id) {
                                    ...
                                    if let Some(edge_mut) = graph.get_edge_mut(&matched_edge.id) {
                                        edge_mut.properties.insert(property.property.clone(), new_value.clone());
                                        updated_count += 1;
                                        property_applied = true;
                        ...
                        if !property_applied {
                            log::warn!("MATCH-SET: Property assignment {}.{} did not match any variables",
            ...
            for item in &self.statement.items {
                match item {
                    ...
                    SetItem::VariableAssignment { variable, value } => {
                        log::warn!("Variable assignment in MATCH SET not yet fully supported: {} = {:?}", variable, value);
                    },
                    SetItem::LabelAssignment { variable, labels } => {
                        // Handle label assignment for matched nodes
                        for (var_name, matched_node) in combination {
                            if self.statement.match_clause.patterns.iter()
                                .any(|p| p.elements.iter()
                                    .any(|e| match e {
                                        PatternElement::Node(node_pattern) => {
                                ...
                                if let Some(node_mut) = graph.get_node_mut(&matched_node.id) {
                                    ...
                                    for term in &labels.terms {
                                        for factor in &term.factors {
                                            if let crate::ast::ast::LabelFactor::Identifier(new_label) = factor {
                                                if !node_mut.labels.contains(new_label) {
                                                    node_mut.labels.push(new_label.clone());
                                                    log::debug!("MATCH SET: Added label {} to node {}", new_label, matched_node.id);
                                                    updated_count += 1;
                                                }
                                            }
                                        }
