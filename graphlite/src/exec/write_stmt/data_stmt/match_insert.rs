...
use crate::ast::ast::{MatchInsertStatement, PatternElement, Expression, Literal};
...
impl MatchInsertExecutor {
    ...
    fn match_path_pattern(
        ...
        if pattern.elements.len() == 3 {
            if let (
                ...
                let matching_edges: Vec<&Edge> = graph.get_all_edges().into_iter()
                    .filter(|edge| {
        ...
        if pattern.elements.len() == 1 {
            if let Some(PatternElement::Node(node_pattern)) = pattern.elements.get(0) {
                if let Some(ref identifier) = node_pattern.identifier {
                    let matching_nodes: Vec<Node> = graph.get_all_nodes().into_iter()
    ...
    fn node_matches_pattern(node: &Node, node_pattern: &crate::ast::ast::Node) -> bool {
        ...
        if !node_pattern.labels.is_empty() && 
           !node_pattern.labels.iter().any(|label| node.labels.contains(label)) {
            return false;
        }
    ...
    fn generate_variable_combinations(
        ...
        generate_recursive(&variables, variable_candidates, HashMap::new(), &mut combinations);
        combinations
    }
            ...
            crate::ast::ast::Expression::Binary(binary_op) => {
                ...
                match &binary_op.operator {
                    ...
                    crate::ast::ast::Operator::And => {
                        ...
                        Self::evaluate_where_expression_on_combination(combination, &binary_op.right)
                    },
    ...
    fn evaluate_expression_on_combination(
        ...
        match expr {
            crate::ast::ast::Expression::Variable(var) => {
                combination.get(&var.name)
                    .map(|node| Value::String(node.id.clone()))
                    .unwrap_or(Value::Null)
            },
            crate::ast::ast::Expression::PropertyAccess(prop_access) => {
                log::debug!("INSERT PropertyAccess: Looking for {}.{} in combination with variables: {:?}", 
                    prop_access.object, prop_access.property, combination.keys().collect::<Vec<_>>());
                if let Some(node) = combination.get(&prop_access.object) {
                    log::debug!("INSERT PropertyAccess: Found node {} with properties: {:?}", 
                        prop_access.object, node.properties.keys().collect::<Vec<_>>());
                    let result = node.properties.get(&prop_access.property).cloned().unwrap_or(Value::Null);
                    log::debug!("INSERT PropertyAccess: {}.{} = {:?}", prop_access.object, prop_access.property, result);
                    result
                } else {
                    log::debug!("INSERT PropertyAccess: Variable '{}' not found in combination", prop_access.object);
                    Value::Null
                }
            },
            ...
            let value = match &property.value {
                ...
                Expression::PropertyAccess(prop_access) => {
                    ...
                    if let Some(bound_node) = variable_bindings.get(&prop_access.object) {
                        bound_node.properties.get(&prop_access.property).cloned().unwrap_or(Value::Null)
                    } else {
                        Value::Null
                    }
                },
...
impl DataStatementExecutor for MatchInsertExecutor {
    ...
    fn execute_modification(
        ...
        log::debug!("Processing {} MATCH patterns", self.statement.match_clause.patterns.len());
        for (pattern_idx, pattern) in self.statement.match_clause.patterns.iter().enumerate() {
            log::debug!("Processing pattern {}: {} elements", pattern_idx, pattern.elements.len());
            ...
            if pattern.elements.len() == 3 {
                ...
                log::debug!("Relationship pattern matching found {} matches", relationship_matches.len());
                for binding in relationship_matches {
                    for (var_name, node) in binding {
                        log::debug!("Adding relationship match: variable '{}' -> node '{}'", var_name, node.id);
                        variable_candidates.entry(var_name).or_insert_with(Vec::new).push(node);
                    }
                }
            } else {
                // Original single-node pattern matching for backward compatibility
                log::debug!("Using single-node pattern matching for {}-element pattern", pattern.elements.len());
                for (element_idx, element) in pattern.elements.iter().enumerate() {
                    log::debug!("Processing element {} in pattern {}", element_idx, pattern_idx);
                    if let PatternElement::Node(node_pattern) = element {
                        if let Some(ref identifier) = node_pattern.identifier {
                            log::debug!("Looking for nodes matching variable '{}' with labels {:?}", identifier, node_pattern.labels);
                            ...
                            log::debug!("Found {} candidates for variable '{}'", matching_nodes.len(), identifier);
        ...
        log::debug!("Generated {} variable combinations", variable_combinations.len());
        ...
        let filtered_combinations = if let Some(ref where_clause) = self.statement.where_clause {
            ...
            let filtered: Vec<_> = variable_combinations.into_iter()
                .filter(|combination| {
                    // Check WHERE clause against the combination
                    Self::evaluate_where_clause_on_combination(combination, where_clause)
                })
                .collect();
            log::debug!("WHERE clause filtering: {} -> {} combinations", original_count, filtered.len());
            filtered
        } else {
        ...
        let processed_combinations = if let Some(ref with_clause) = self.statement.with_clause {
            ...
            for combination in &filtered_combinations {
                for (var_name, node) in combination {
                    multi_bindings.entry(var_name.clone())
                        .or_insert_with(Vec::new)
                        .push(node.clone());
                }
            }
            
            log::debug!("Processing WITH clause with {} variable bindings", multi_bindings.len());
            ...
            let temp_context = if context.function_registry.is_some() {
                ...
                context.clone().with_function_registry(Arc::new(FunctionRegistry::new()))
            };
            match WithClauseProcessor::process_with_clause(with_clause, &multi_bindings, &edges, &temp_context) {
                Ok(with_result) => {
                    log::debug!("WITH clause processed successfully, has_aggregation: {}", with_result.has_aggregation);
                    ...
                    if with_result.has_aggregation && !with_result.group_results.is_empty() {
                            ...
                            for (alias, value) in &group_result.computed_values {
                                ...
                                if !final_binding.contains_key(alias) {
                                    let virtual_node = Node {
                                        id: format!("computed_{}_{}", alias, aggregated_bindings.len()),
                        ...
                        for combination in filtered_combinations {
                            ...
                            match WithClauseProcessor::process_with_clause(with_clause, &multi_binding, &edges, &temp_context) {
                                Ok(individual_result) => {
                                    // Check WHERE clause
                                    if let Some(ref where_clause) = self.statement.where_clause {
                                        if !WithClauseProcessor::evaluate_where_with_computed_values(where_clause, &individual_result.computed_values) {
                                            continue;
                                        }
                                    }
                                ...
                                Err(e) => {
                                    log::error!("Failed to process WITH clause for individual binding: {}", e);
                                }
                            }
                        }
        ...
        for (i, element) in pattern.elements.iter().enumerate() {
            match element {
                PatternElement::Node(node_pattern) => {
                    ...
                    let should_create_node = if let Some(ref identifier) = node_pattern.identifier {
                    ...
                    if should_create_node {
                        ...
                        graph.add_node(node)
                            .map_err(|e| ExecutionError::RuntimeError(
                                format!("Failed to insert node in MATCH INSERT: {}", e)
                            ))?;
                        ...
                        if let Some(ref identifier) = node_pattern.identifier {
                            ...
                            if let Some(created_node) = graph.get_node(&node_id) {
                                variable_bindings.insert(identifier.clone(), created_node.clone());
                            }
                        }
                    } else {
                ...
                PatternElement::Edge(edge_pattern) => {
                    ...
                    if i == 0 || i >= pattern.elements.len() - 1 {
                        return Err(ExecutionError::RuntimeError(
                            "Edge patterns in MATCH INSERT must be between two nodes".to_string()
                        ));
                    }
                    ...
                    // Create the edge
                    let edge_label = edge_pattern.labels.first().cloned()
                    ...
                    match graph.add_edge(edge) {
                        ...
                        Err(e) => {
                            return Err(ExecutionError::RuntimeError(
                                format!("Failed to insert edge in MATCH INSERT: {}", e)
                            ));
                        }
                    }
