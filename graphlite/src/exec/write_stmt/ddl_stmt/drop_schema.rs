...
impl StatementExecutor for DropSchemaExecutor {
    ...
    fn operation_description(&self, _context: &ExecutionContext) -> String {
        format!("DROP SCHEMA {}", self.statement.schema_path.name().map_or("unknown", |v| v))
    }
}
...
impl DDLStatementExecutor for DropSchemaExecutor {
    fn execute_ddl_operation(
        ...
        let schema_name = self.statement.schema_path.name()
        ...
        log::info!("DROP SCHEMA: cascade={}, schema_name={}", self.statement.cascade, schema_name);
                ...
                if let crate::catalog::operations::CatalogResponse::List { items } = response {
                    ...
                    items.iter()
                        .filter_map(|graph_entry| {
                            let graph_schema = graph_entry.get("id")
                                .and_then(|id_obj| id_obj.get("schema_name"))
                                .and_then(|v| v.as_str());

                            let schema_matches = graph_schema
                                .map(|s| s == schema_name)
                                .unwrap_or(false);

                            if schema_matches {
                                graph_entry.get("id").and_then(|id| id.get("name")).and_then(|v| v.as_str()).map(|s| s.to_string())
                            } else {
                                None
                            }
                        })
        ...
        if self.statement.cascade {
            ...
            log::info!("CASCADE: Starting dependency cleanup for schema '{}'", schema_name);
            for graph_name in &dependent_graphs {
                log::info!("CASCADE: Dropping dependent graph '{}' in schema '{}'", graph_name, schema_name);
            ...
            if !dependent_graphs.is_empty() {
                ...
                return Err(ExecutionError::CatalogError(
                    format!("Cannot drop schema '{}': contains objects (graphs: {})", schema_name, graph_list)
                ));
            }
        }
        ...
        match drop_result {
            Ok(response) => {
                match response {
                    crate::catalog::operations::CatalogResponse::Success { data: _ } => {
                                ...
                                if let Some(session_arc) = session_manager.get_session(&session_id) {
                                    if let Ok(mut session) = session_arc.write() {
                                        ...
                                        if let Some(current_schema) = &session.current_schema {
                                            if current_schema == &schema_name || current_schema == &format!("/{}", schema_name) {
                        ...
                        if self.statement.cascade {
                            let graph_persist_result = catalog_manager.persist_catalog("graph_metadata");
                    ...
                    crate::catalog::operations::CatalogResponse::Error { message } if message.contains("not found") => {
                        if self.statement.if_exists {
                            let message = format!("Schema '{}' does not exist (if exists)", schema_name);
                            Ok((message, 0))
                        } else {
                    ...
                    crate::catalog::operations::CatalogResponse::Error { message } => {
                        Err(ExecutionError::CatalogError(
                            format!("Failed to drop schema '{}': {}", schema_name, message)
                        ))
                    }
                    _ => {
                        Err(ExecutionError::CatalogError(
                            format!("Unexpected response from schema catalog")
                        ))
                    }
                }
            },
            Err(e) => {
                if e.to_string().contains("not found") && self.statement.if_exists {
                    Ok((format!("Schema '{}' does not exist (if exists)", schema_name), 0))
                } else {
                    Err(ExecutionError::CatalogError(
