...
impl DDLStatementCoordinator {
    ...
    pub fn execute_ddl_statement(
        ...
        let result = match stmt {
            ...
            CatalogStatement::CreateProcedure(create_procedure) => {
                ...
                DDLStatementCoordinator::execute_create_procedure(create_procedure, &context, catalog_manager, &storage)
            },
            CatalogStatement::DropProcedure(drop_procedure) => {
                // Remove procedure from catalog
                DDLStatementCoordinator::execute_drop_procedure(drop_procedure, &context, catalog_manager, &storage)
            },
        };
                ...
                match stmt {
                    CatalogStatement::CreateSchema(_) | CatalogStatement::DropSchema(_) => {
                        match catalog_manager.persist_catalog("schema") {
                            ...
                            Err(e) => log::warn!("Failed to persist schema catalog (non-fatal): {}", e),
                        }
                    },
                    CatalogStatement::CreateGraph(_) | CatalogStatement::DropGraph(_) |
                    CatalogStatement::CreateGraphType(_) | CatalogStatement::DropGraphType(_) |
                    CatalogStatement::AlterGraphType(_) => {
                        match catalog_manager.persist_catalog("graph_metadata") {
                            ...
                            Err(e) => log::warn!("Failed to persist graph_metadata catalog (non-fatal): {}", e),
                        }
                    },
                    CatalogStatement::CreateUser(_) | CatalogStatement::DropUser(_) |
                    CatalogStatement::CreateRole(_) | CatalogStatement::DropRole(_) |
                    CatalogStatement::GrantRole(_) | CatalogStatement::RevokeRole(_) => {
                        match catalog_manager.persist_catalog("security") {
                            Ok(_) => log::debug!("Security catalog persisted successfully"),
                            Err(e) => log::warn!("Failed to persist security catalog (non-fatal): {}", e),
                        }
                    },
                    _ => {
    ...
    fn execute_create_procedure(
        ...
        use serde_json::json;
        use crate::catalog::operations::{CatalogOperation, EntityType, CatalogResponse};
        ...
        let response = catalog_manager.execute("procedure", CatalogOperation::Create {
            entity_type: EntityType::Procedure,
            name: create_procedure.procedure_name.clone(),
            params: procedure_json,
        }).map_err(|e| crate::exec::error::ExecutionError::CatalogError(format!("Failed to create procedure: {}", e)))?;

        match response {
            CatalogResponse::Success { data: _ } => {
                Ok((format!("Procedure '{}' created successfully", create_procedure.procedure_name), 0))
            },
            CatalogResponse::Error { message } => {
                Err(crate::exec::error::ExecutionError::CatalogError(format!("Failed to create procedure: {}", message)))
            },
            _ => {
                Err(crate::exec::error::ExecutionError::CatalogError("Unexpected response from catalog".to_string()))
            }
        }
    }
    ...
    fn execute_drop_procedure(
        ...
        let response = catalog_manager.execute("procedure", CatalogOperation::Drop {
            entity_type: EntityType::Procedure,
            name: drop_procedure.procedure_name.clone(),
            cascade: false,
        }).map_err(|e| crate::exec::error::ExecutionError::CatalogError(format!("Failed to drop procedure: {}", e)))?;

        match response {
            CatalogResponse::Success { data: _ } => {
                Ok((format!("Procedure '{}' dropped successfully", drop_procedure.procedure_name), 0))
            },
            CatalogResponse::Error { message } => {
                if drop_procedure.if_exists && message.contains("not found") {
                    Ok((format!("Procedure '{}' does not exist (IF EXISTS)", drop_procedure.procedure_name), 0))
                } else {
                    Err(crate::exec::error::ExecutionError::CatalogError(format!("Failed to drop procedure: {}", message)))
                }
            },
            _ => {
                Err(crate::exec::error::ExecutionError::CatalogError("Unexpected response from catalog".to_string()))
