...
impl StatementExecutor for DropGraphExecutor {
    ...
    fn operation_description(&self, _context: &ExecutionContext) -> String {
        ...
        let graph_path = if self.statement.graph_path.segments.len() >= 2 {
            ...
            self.statement.graph_path.segments.last()
...
impl DDLStatementExecutor for DropGraphExecutor {
    fn execute_ddl_operation(
        ...
        storage.delete_graph(&full_path)
            .map_err(|e| ExecutionError::StorageError(
                format!("Failed to delete graph data for '{}': {}", full_path, e)
            ))?;
                ...
                match response {
                    crate::catalog::operations::CatalogResponse::Success { data: _ } => {
                        ...
                        if let Some(session_manager) = crate::session::get_session_manager() {
                            let sessions_invalidated = session_manager.invalidate_sessions_for_graph(&full_path);
                            log::info!("Invalidated {} sessions using dropped graph '{}'", sessions_invalidated, full_path);
                        } else {
                    ...
                    crate::catalog::operations::CatalogResponse::Error { message } => {
                        Err(ExecutionError::CatalogError(
                            format!("Failed to drop graph '{}': {}", full_path, message)
                        ))
                    }
                    _ => {
                        Err(ExecutionError::CatalogError(
