...
impl StatementExecutor for ClearGraphExecutor {
    ...
    fn operation_description(&self, context: &ExecutionContext) -> String {
        let graph_name = if let Some(ref catalog_path) = self.statement.graph_path {
            ...
            context.get_graph_name().unwrap_or_else(|_| "current_session".to_string())
        };
    ...
    fn execute_ddl_operation(
        ...
        storage.save_graph(&full_path, empty_graph)
            .map_err(|e| ExecutionError::StorageError(
                format!("Failed to clear graph data for '{}': {}", full_path, e)
            ))?;
        ...
        let simple_graph_name = if let Some(ref catalog_path) = self.statement.graph_path {
            catalog_path.segments.last().map_or("unknown".to_string(), |v| v.clone())
        } else {
                ...
                match response {
                    ...
                    crate::catalog::operations::CatalogResponse::Error { message } => {
                        Err(ExecutionError::CatalogError(
                            format!("Failed to clear graph '{}': {}", full_path, message)
                        ))
                    }
                    _ => {
                        Err(ExecutionError::CatalogError(
