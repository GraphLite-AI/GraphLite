    ...
    fn execute_ddl_operation(
            ...
            1 => {
                ...
                match context.get_current_schema() {
                    Some(session_schema) => {
                        ...
                        let schema_name = session_schema.strip_prefix('/').unwrap_or(&session_schema).to_string();
        ...
        match schema_result {
            ...
            Ok(crate::catalog::operations::CatalogResponse::Error { message }) => {
                return Err(ExecutionError::CatalogError(
                    format!("Failed to validate schema '{}': {}", schema_name, message)
                ));
            }
            Err(e) => {
                return Err(ExecutionError::CatalogError(
                    format!("Failed to validate schema '{}': {}", schema_name, e)
                ));
            }
            _ => {
                return Err(ExecutionError::CatalogError(
                    format!("Unexpected response when validating schema '{}'", schema_name)
                ));
            }
        }
        ...
        let graph_type_name: Option<String> = if let Some(_spec) = &self.statement.graph_type_spec {
            ...
            match catalog_manager.execute("graph_type", graph_type_query) {
                Ok(CatalogResponse::Success { data: Some(_) }) => {
                    info!("Using graph type '{}' for new graph '{}'", type_name, graph_name);
                    ...
                    match enforcement_mode.as_str() {
                        "strict" => {
                            return Err(ExecutionError::SchemaValidation(
                                format!("Graph type '{}' not found for CREATE GRAPH", type_name)
                            ));
                        },
                        "advisory" => {
                            warn!("Graph type '{}' not found, creating untyped graph", type_name);
                            None
                        },
                        _ => None
                    }
                ...
                match response {
                    crate::catalog::operations::CatalogResponse::Success { data: _ } => {
                        ...
                        if let Err(e) = storage.save_graph(&full_graph_path, empty_graph) {
                            ...
                            return Err(ExecutionError::StorageError(
                                format!("Failed to create graph '{}' in storage: {}", full_graph_path, e)
                            ));
                        }
                    ...
                    crate::catalog::operations::CatalogResponse::Error { message } => {
                        if message == "Already exists" && self.statement.if_not_exists {
                            ...
                            let message = format!("Graph '{}' already exists (if not exists)", graph_name);
                            Ok((message, 0))
                        } else if message == "Already exists" {
                            Err(ExecutionError::CatalogError(
                                format!("Graph '{}' already exists", graph_name)
                            ))
                        } else {
                            Err(ExecutionError::CatalogError(
                                format!("Failed to create graph '{}': {}", graph_name, message)
                            ))
                        }
                    }
                    _ => {
