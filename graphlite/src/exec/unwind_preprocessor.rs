...
impl UnwindPreprocessor {
    ...
    pub fn is_unwind_query(query: &str) -> bool {
        ...
        if with_count > 1 {
            log::debug!("UNWIND PREPROCESSOR: Skipping query with {} WITH clauses (not supported)", with_count);
            return false;
        }
    ...
    pub fn parse_unwind_query(query: &str) -> Result<UnwindQueryComponents, ExecutionError> {
        ...
        let operation_pos = match (remove_pos, set_pos) {
            ...
            (Some(r), None) => Some(r),
            (None, Some(s)) => Some(s),
            (None, None) => return Err(ExecutionError::RuntimeError("No REMOVE or SET operation found in UNWIND query".to_string())),
        };
        
        // Extract components
        let match_clause = &query[match_start..with_start].trim();
        let with_clause = Self::extract_between(query, with_start, Some(unwind_start), "WITH", "UNWIND")?.trim();
        let unwind_clause = Self::extract_unwind_clause(query, unwind_start, where_pos.or(operation_pos))?;
        
        let where_clause = if let Some(where_start) = where_pos {
            Some(Self::extract_between(query, where_start, operation_pos, "WHERE", "")?.trim().to_string())
        } else {
            None
        };
        ...
        let operation_clause = if let Some(op_start) = operation_pos {
            ...
            return Err(ExecutionError::RuntimeError("No operation clause found".to_string()));
        };
        ...
        E: Fn(&str) -> Result<QueryResult, ExecutionError>
    {
        ...
        log::debug!("UNWIND PREPROCESSOR: Getting nodes with query: {}", nodes_query);
        
        let nodes_result = executor_fn(&nodes_query)
            .map_err(|e| ExecutionError::RuntimeError(format!("Failed to execute nodes query: {}", e)))?;
        ...
        log::debug!("UNWIND PREPROCESSOR: Found {} nodes to process", items.len());
        ...
        if components.with_clause.contains("avg(") {
            ...
            log::debug!("UNWIND PREPROCESSOR: Computed avg_price manually: {} from {} prices: {:?}", avg_price, prices.len(), prices);
        }
        ...
        for (index, item) in items.iter().enumerate() {
            let individual_query = Self::generate_individual_query(&components, item, index, &computed_values)?;
            log::debug!("UNWIND PREPROCESSOR: Executing individual query {}: {}", index + 1, individual_query);
            
            let result = executor_fn(&individual_query)
                .map_err(|e| ExecutionError::RuntimeError(format!("Failed to execute individual query {}: {}", index + 1, e)))?;
            
            log::debug!("UNWIND PREPROCESSOR: Query {} result: {:?}", index + 1, result);
            total_affected += result.rows_affected;
        }
        
        log::debug!("UNWIND PREPROCESSOR: Completed. Total rows affected: {}", total_affected);
    ...
    fn extract_match_variable(match_clause: &str) -> Result<String, ExecutionError> {
        ...
        let open_paren = match_clause.find('(')
            .ok_or_else(|| ExecutionError::RuntimeError("Invalid MATCH clause: no opening parenthesis".to_string()))?;
        
        let text_after_paren = &match_clause[open_paren + 1..];
        
        // Find the variable name - it's the first word after (
        let var_end = text_after_paren.find(|c: char| c == ':' || c == ')' || c == ' ')
            .ok_or_else(|| ExecutionError::RuntimeError("Invalid MATCH clause: cannot find variable name".to_string()))?;
        
        let var_name = text_after_paren[..var_end].trim();
        
        if var_name.is_empty() {
            return Err(ExecutionError::RuntimeError("Invalid MATCH clause: empty variable name".to_string()));
        }
    ...
    fn find_keyword_pos(text: &str, keyword: &str) -> Result<usize, ExecutionError> {
        ...
        upper_text.find(keyword)
            .ok_or_else(|| ExecutionError::RuntimeError(format!("Keyword '{}' not found in query", keyword)))
    }
    ...
    fn extract_between<'a>(text: &'a str, start_pos: usize, end_pos: Option<usize>, start_keyword: &str, end_keyword: &str) -> Result<&'a str, ExecutionError> {
        let start = start_pos + start_keyword.len();
        let end = end_pos.unwrap_or(text.len());
        
        if start >= end {
            return Err(ExecutionError::RuntimeError(format!("Invalid range for extracting between '{}' and '{}'", start_keyword, end_keyword)));
        }
        
        Ok(&text[start..end])
    }
    
    fn extract_unwind_clause(text: &str, unwind_start: usize, end_pos: Option<usize>) -> Result<UnwindClause, ExecutionError> {
        ...
        if parts.len() >= 3 && parts[1].to_uppercase() == "AS" {
            ...
            Err(ExecutionError::RuntimeError(format!("Invalid UNWIND syntax: {}", unwind_part)))
        }
    }
    
    
    fn generate_individual_query(components: &UnwindQueryComponents, item: &Value, _index: usize, computed_values: &std::collections::HashMap<String, Value>) -> Result<String, ExecutionError> {
        // Convert the item to a node pattern for MATCH
        let item_pattern = Self::value_to_match_pattern(item, &components.unwind_clause.item_alias)?;
    ...
    fn value_to_match_pattern(value: &Value, alias: &str) -> Result<String, ExecutionError> {
        match value {
            Value::Node(node_ref) => {
                ...
                Ok(format!("({}{}{{{}: {}}})", alias, labels_part, "id", id_value))
            },
            _ => {
                // For non-node values, we can't create a meaningful MATCH pattern
                Err(ExecutionError::RuntimeError(format!("Cannot create MATCH pattern for non-node value: {:?}", value)))
            }
        }
    }
}
