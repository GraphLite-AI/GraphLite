...
use crate::ast::ast::{Document, Query, BasicQuery, SetOperation, SetOperationType, MatchClause, ReturnClause, PathPattern, PatternElement, OrderClause, OrderDirection, Expression, BinaryExpression, Operator, Variable, LetStatement};
use crate::plan::logical::{LogicalPlan, LogicalNode, VariableInfo, EntityType, ProjectExpression, SortExpression, JoinType};
use crate::plan::physical::{PhysicalPlan, PhysicalNode};
use crate::plan::cost::{CostModel, Statistics, CostEstimate};
use crate::plan::trace::{PlanTrace, PlanTracer, PlanningPhase, TraceMetadata};
use crate::storage::GraphCache;
use crate::plan::pattern_optimization::integration::{
...
impl QueryPlanner {
    ...
    pub fn optimize_with_indexes(&self, logical_plan: LogicalPlan, available_indexes: &[IndexInfo]) -> Result<PhysicalPlan, PlanningError> {
        ...
        let physical_plan = self.create_physical_plan_with_indexes(optimized_logical, available_indexes)?;
    ...
    pub fn plan_query(&mut self, document: &Document) -> Result<PhysicalPlan, PlanningError> {
        ...
        let query = match &document.statement {
            ...
            _ => return Err(PlanningError::InvalidQuery("Document does not contain a query statement".to_string())),
        };
    ...
    pub fn plan_query_with_trace(&mut self, document: &Document) -> Result<PlanTrace, PlanningError> {
        ...
        // Generate logical plan
        tracer.start_step(PlanningPhase::LogicalPlanGeneration, "Create logical plan from AST".to_string());
        let logical_plan = self.create_logical_plan(query)?;
        tracer.end_step(
        ...
        // Optimize logical plan
        tracer.start_step(PlanningPhase::LogicalOptimization, "Apply logical optimizations".to_string());
        let optimized_logical = self.optimize_logical_plan(logical_plan.clone())?;
        tracer.end_step(
            PlanningPhase::LogicalOptimization,
            format!("Applied {} optimization level", self.optimization_level_name()),
            None,
            None,
            None,
        ...
        // Convert to physical plan
        tracer.start_step(PlanningPhase::PhysicalPlanGeneration, "Convert to physical plan".to_string());
        ...
        tracer.end_step(
            ...
            TraceMetadata::with_estimates(physical_plan.estimated_rows, physical_plan.estimated_cost),
        );
        
        // Optimize physical plan
        tracer.start_step(PlanningPhase::PhysicalOptimization, "Apply physical optimizations".to_string());
        let optimized_physical = self.optimize_physical_plan(physical_plan)?;
        tracer.end_step(
            PlanningPhase::PhysicalOptimization,
            format!("Physical optimization complete (avoid_index_scan: {})", self.avoid_index_scan),
            None,
            None,
            Some(self.estimate_plan_cost(&optimized_physical)),
            TraceMetadata::with_estimates(optimized_physical.estimated_rows, optimized_physical.estimated_cost),
        );
        
        // Final cost estimation
        tracer.trace_step(
            PlanningPhase::CostEstimation,
            format!("Final cost estimation: {:.2}", optimized_physical.estimated_cost),
            TraceMetadata::with_estimates(optimized_physical.estimated_rows, optimized_physical.estimated_cost),
        );
    ...
    pub fn plan_query_with_alternatives(&mut self, document: &Document) -> Result<QueryPlanAlternatives, PlanningError> {
        ...
        if matches!(self.optimization_level, OptimizationLevel::Advanced | OptimizationLevel::Aggressive) {
    ...
    fn create_logical_plan(&mut self, query: &Query) -> Result<LogicalPlan, PlanningError> {
        match query {
            ...
            Query::SetOperation(set_op) => {
                self.create_set_operation_plan(set_op)
            }
            Query::Limited { query, order_clause, limit_clause } => {
                ...
                if let Some(order) = order_clause {
                    let sort_expressions: Vec<_> = order.items.iter()
            ...
            Query::WithQuery(with_query) => {
                ...
                use crate::plan::logical::{LogicalPlan, LogicalNode, VariableInfo, EntityType};
                use std::collections::HashMap;
                    ...
                    if let Some(with_clause) = &segment.with_clause {
                        for with_item in &with_clause.items {
                            if let Some(alias) = &with_item.alias {
                                variables.insert(alias.clone(), VariableInfo {
                                    name: alias.clone(),
            ...
            Query::Let(let_stmt) => {
                self.create_let_logical_plan(let_stmt)
            },
            Query::For(_) => {
                // FOR statements don't need optimization yet  
                Err(PlanningError::UnsupportedFeature("FOR queries not yet implemented".to_string()))
            },
            Query::Filter(_) => {
                // FILTER statements don't need optimization yet
                Err(PlanningError::UnsupportedFeature("FILTER queries not yet implemented".to_string()))
            },
            Query::Return(return_query) => {
                self.create_return_logical_plan(return_query)
            },
            Query::Unwind(unwind_stmt) => {
                self.create_unwind_logical_plan(unwind_stmt)
            },
    ...
    fn create_basic_logical_plan(&mut self, query: &BasicQuery) -> Result<LogicalPlan, PlanningError> {
        ...
        if let Some(group_clause) = &query.group_clause {
            ...
            let group_expressions = self.plan_group_clause_with_aliases(group_clause, &query.return_clause, &context)?;
            ...
            // Apply aggregation
            logical_plan = logical_plan.apply_aggregation(group_expressions.clone(), project_expressions.clone());
            ...
            if has_aggregates {
                ...
                let non_aggregate_expressions = self.extract_non_aggregate_expressions(&project_expressions);
                
                if !non_aggregate_expressions.is_empty() {
                    // Mixed expressions - add implicit GROUP BY for non-aggregate expressions
                    logical_plan = logical_plan.apply_aggregation(non_aggregate_expressions, project_expressions);
                } else {
                    // Pure aggregation - empty GROUP BY (implicit aggregation)
                    let empty_group_expressions = Vec::new();
                    logical_plan = logical_plan.apply_aggregation(empty_group_expressions, project_expressions);
                }
            } else {
                // Normal projection
                logical_plan = logical_plan.apply_projection(project_expressions);
        ...
        if let Some(having_clause) = &query.having_clause {
            if query.group_clause.is_none() {
                return Err(PlanningError::InvalidQuery("HAVING clause requires GROUP BY clause".to_string()));
            }
            // Resolve aliases in HAVING clause expressions
            let resolved_condition = self.resolve_having_expression_with_aliases(&having_clause.condition, &query.return_clause);
    ...
    fn create_let_logical_plan(&mut self, let_stmt: &LetStatement) -> Result<LogicalPlan, PlanningError> {
        let mut context = PlanningContext {
            variables: HashMap::new(),
            _next_variable_id: 0,
        };
    ...
    fn create_return_logical_plan(&self, return_query: &crate::ast::ast::ReturnQuery) -> Result<LogicalPlan, PlanningError> {
        ...
        if let Some(group_clause) = &return_query.group_clause {
            ...
            let group_expressions = self.plan_group_clause_with_aliases(group_clause, &return_query.return_clause, &context)?;
        ...
        if let Some(having_clause) = &return_query.having_clause {
            // Resolve aliases in HAVING clause expressions
            let resolved_condition = self.resolve_having_expression_with_aliases(&having_clause.condition, &return_query.return_clause);
            logical_plan = logical_plan.apply_having(resolved_condition);
        }
        
        // Process ORDER BY clause if present
        if let Some(order_clause) = &return_query.order_clause {
            let sort_expressions: Vec<_> = order_clause.items.iter()
                .map(|item| SortExpression {
                    expression: item.expression.clone(),
                    ascending: matches!(item.direction, crate::ast::ast::OrderDirection::Ascending),
                })
    ...
    fn create_set_operation_plan(&mut self, set_op: &SetOperation) -> Result<LogicalPlan, PlanningError> {
    ...
    fn plan_match_clause(&mut self, match_clause: &MatchClause, context: &mut PlanningContext) -> Result<LogicalPlan, PlanningError> {
        if match_clause.patterns.is_empty() {
            return Err(PlanningError::InvalidQuery("Empty MATCH clause".to_string()));
        }
        ...
        if self.should_apply_pattern_optimization(match_clause) {
            ...
            match self.optimize_comma_separated_patterns(match_clause, context) {
                ...
                Err(e) => {
                    log::debug!("⚠️  Pattern optimization failed ({}), falling back to original behavior", e);
    ...
    fn optimize_comma_separated_patterns(
        ...
        match self.pattern_optimizer.optimize_query(&dummy_query, &base_logical_plan, &base_physical_plan) {
            Ok(result) => {
                if result.optimized {
                    log::debug!("🎉 Pattern optimization successful: {}", result.explanation);
                    log::debug!("   Performance improvement: {}", result.improvement.describe());
                    ...
                    Err(PlanningError::InvalidQuery(format!("Pattern optimization skipped: {}", result.explanation)))
                }
            }
            Err(e) => {
                Err(PlanningError::InvalidQuery(format!("Pattern optimization error: {}", e)))
            }
        }
    }
    
    /// Create a simple physical plan for optimization analysis
    fn create_simple_physical_plan(&self, logical_plan: &LogicalPlan) -> Result<PhysicalNode, PlanningError> {
    ...
    fn logical_to_physical_node(&self, logical_node: &LogicalNode) -> Result<PhysicalNode, PlanningError> {
        match logical_node {
            ...
            LogicalNode::Expand { from_variable, edge_variable, to_variable, edge_labels, direction, properties, input } => {
    ...
    fn create_optimized_logical_plan(
        ...
        if match_clause.patterns.len() < 2 {
            return Err(PlanningError::InvalidQuery("Need multiple patterns for optimization".to_string()));
        }
        ...
        for pattern in &match_clause.patterns[1..] {
            ...
            log::debug!("🔍 Current plan variables: {:?}", current_plan.variables.keys().collect::<Vec<_>>());
            log::debug!("🔍 Pattern plan variables: {:?}", pattern_plan.variables.keys().collect::<Vec<_>>());
            ...
            if !shared_vars.is_empty() {
                ...
                log::debug!("🔗 Creating connected PATH TRAVERSAL for shared variables: {:?}", shared_vars);
                ...
                let merged_path_result = self.merge_patterns_into_path(&current_plan, &pattern_plan, &shared_vars[0]);
                ...
                match merged_path_result {
                    ...
                    Err(e) => {
                        log::debug!("⚠️  Path merging failed ({}), falling back to inner join", e);
                        // Fall back to join approach
                        let join_condition = self.create_join_condition_for_shared_vars(&shared_vars)?;
        ...
        log::debug!("🎯 OPTIMIZATION COMPLETE: Returning optimized plan with {} variables", current_plan.variables.len());
        Ok(current_plan)
    }
    ...
    fn populate_plan_variables_from_context(&self, plan: &mut LogicalPlan, context: &PlanningContext) {
    ...
    fn merge_patterns_into_path(&self, left_plan: &LogicalPlan, right_plan: &LogicalPlan, shared_var: &str) -> Result<LogicalPlan, String> {
        ...
        if let LogicalNode::Expand { from_variable, edge_variable, to_variable, edge_labels, direction, properties, .. } = &right_plan.root {
            // Verify the right pattern starts from the shared variable
            if from_variable != shared_var {
                return Err(format!("Right pattern doesn't start from shared variable {}", shared_var));
            }
            ...
            log::debug!("✅ Created chained Expand nodes connecting patterns via: {}", shared_var);
    ...
    fn create_join_condition_for_shared_vars(&self, shared_vars: &[String]) -> Result<Option<Expression>, PlanningError> {
        if shared_vars.is_empty() {
            return Ok(None);
        }
    ...
    fn extract_pattern_variables(&self, pattern: &PathPattern, context: &mut PlanningContext) -> Result<(), PlanningError> {
        for element in &pattern.elements {
            match element {
    ...
    fn plan_return_clause(&self, return_clause: &ReturnClause, _context: &PlanningContext) -> Result<Vec<ProjectExpression>, PlanningError> {
        ...
        match expr {
            ...
            Expression::FunctionCall(func_call) => {
                ...
                for arg in &func_call.arguments {
                    resolved_args.push(self.resolve_having_expression_with_aliases(arg, return_clause));
                }
            ...
            Expression::Binary(binary_expr) => {
                Expression::Binary(crate::ast::ast::BinaryExpression {
                    left: Box::new(self.resolve_having_expression_with_aliases(&binary_expr.left, return_clause)),
                    operator: binary_expr.operator.clone(),
                    right: Box::new(self.resolve_having_expression_with_aliases(&binary_expr.right, return_clause)),
            ...
            Expression::Unary(unary_expr) => {
                Expression::Unary(crate::ast::ast::UnaryExpression {
                    ...
                    expression: Box::new(self.resolve_having_expression_with_aliases(&unary_expr.expression, return_clause)),
    ...
    fn plan_order_clause(&self, order_clause: &OrderClause, _context: &PlanningContext) -> Result<Vec<SortExpression>, PlanningError> {
        ...
        match node {
            ...
            LogicalNode::Filter { condition, input } => {
                ...
                match optimized_input {
                    ...
                    LogicalNode::Join { left, right, join_type, condition: join_condition } => {
            ...
            LogicalNode::Join { left, right, join_type, condition } => {
                Ok(LogicalNode::Join {
                    left: Box::new(self.optimize_logical_node(*left)?),
                    right: Box::new(self.optimize_logical_node(*right)?),
                    join_type,
    ...
    fn apply_projection_elimination(&self, plan: LogicalPlan) -> Result<LogicalPlan, PlanningError> {
    ...
    fn unnest_subqueries_in_node(&self, node: LogicalNode) -> Result<LogicalNode, PlanningError> {
        match node {
            ...
            LogicalNode::InSubquery { expression, subquery, outer_variables, .. } => {
    ...
    fn can_unnest_exists_subquery(&self, subquery: &LogicalNode, outer_variables: &[String]) -> bool {
    ...
    fn can_unnest_not_exists_subquery(&self, subquery: &LogicalNode, outer_variables: &[String]) -> bool {
        // Same conditions as EXISTS
        self.can_unnest_exists_subquery(subquery, outer_variables)
    }
    
    /// Check if IN subquery can be unnested
    fn can_unnest_in_subquery(&self, subquery: &LogicalNode, outer_variables: &[String], _expression: &Expression) -> bool {
    ...
    fn unnest_exists_subquery(&self, subquery: LogicalNode, outer_variables: Vec<String>) -> Result<LogicalNode, PlanningError> {
    ...
    fn unnest_not_exists_subquery(&self, subquery: LogicalNode, outer_variables: Vec<String>) -> Result<LogicalNode, PlanningError> {
        let join_condition = self.build_correlation_condition(&outer_variables)?;
    ...
    fn unnest_in_subquery(&self, subquery: LogicalNode, outer_variables: Vec<String>, expression: Expression) -> Result<LogicalNode, PlanningError> {
    ...
    fn build_correlation_condition(&self, outer_variables: &[String]) -> Result<Expression, PlanningError> {
        if outer_variables.is_empty() {
            return Err(PlanningError::InvalidQuery("No correlation variables for join".to_string()));
        }
    ...
    fn contains_aggregate_functions(&self, expressions: &[ProjectExpression]) -> bool {
        expressions.iter().any(|expr| self.is_aggregate_expression(&expr.expression))
    }
    ...
    fn is_aggregate_expression(&self, expr: &Expression) -> bool {
        match expr {
            ...
            Expression::Binary(binary) => {
                ...
                self.is_aggregate_expression(&binary.left) || self.is_aggregate_expression(&binary.right)
            },
    ...
    fn extract_non_aggregate_expressions(&self, expressions: &[ProjectExpression]) -> Vec<Expression> {
    ...
    fn collect_non_aggregate_subexpressions(&self, expr: &Expression, group_expressions: &mut Vec<Expression>) {
        match expr {
            Expression::FunctionCall(func_call) => {
                // If it's an aggregate function, don't add it to GROUP BY
                if matches!(func_call.name.to_uppercase().as_str(), "COUNT" | "SUM" | "AVG" | "AVERAGE" | "MIN" | "MAX" | "COLLECT") {
                    return;
                }
    ...
    fn create_physical_plan(&self, logical_plan: LogicalPlan) -> Result<PhysicalPlan, PlanningError> {
    ...
    fn transform_node_disable_indexes(&self, node: PhysicalNode) -> Result<PhysicalNode, PlanningError> {
        ...
        match node {
            ...
            PhysicalNode::Filter { condition, input, selectivity, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::Having { condition, input, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::Project { expressions, input, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::HashAggregate { group_by, aggregates, input, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::SortAggregate { group_by, aggregates, input, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::ExternalSort { expressions, input, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::InMemorySort { expressions, input, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::Limit { count, offset, input, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::Distinct { input, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::UnionAll { inputs, all, estimated_rows, estimated_cost } => {
            ...
            PhysicalNode::PathTraversal { path_type, from_variable, to_variable, path_elements, input, estimated_rows, estimated_cost } => {
                let transformed_input = Box::new(self.transform_node_disable_indexes(*input)?);
            ...
            PhysicalNode::ExistsSubquery { subplan, estimated_rows, estimated_cost, optimized } => {
            ...
            PhysicalNode::NotExistsSubquery { subplan, estimated_rows, estimated_cost, optimized } => {
                let transformed_subplan = Box::new(self.transform_node_disable_indexes(*subplan)?);
            ...
            PhysicalNode::InSubquery { expression, subplan, estimated_rows, estimated_cost } => {
                let transformed_subplan = Box::new(self.transform_node_disable_indexes(*subplan)?);
            ...
            PhysicalNode::NotInSubquery { expression, subplan, estimated_rows, estimated_cost } => {
                let transformed_subplan = Box::new(self.transform_node_disable_indexes(*subplan)?);
            ...
            PhysicalNode::ScalarSubquery { subplan, estimated_rows, estimated_cost } => {
                let transformed_subplan = Box::new(self.transform_node_disable_indexes(*subplan)?);
            ...
            PhysicalNode::WithQuery { original_query, estimated_rows, estimated_cost } => {
            ...
            PhysicalNode::Unwind { expression, variable, input, estimated_rows, estimated_cost } => {
            ...
            PhysicalNode::GraphIndexScan { parameters, estimated_rows, estimated_cost, .. } => {
            ...
            PhysicalNode::IndexJoin { left, right, join_type, join_condition, estimated_rows, estimated_cost, .. } => {
            ...
            PhysicalNode::Intersect { left, right, all, estimated_rows, estimated_cost } => {
                let transformed_left = Box::new(self.transform_node_disable_indexes(*left)?);
                let transformed_right = Box::new(self.transform_node_disable_indexes(*right)?);
            ...
            PhysicalNode::Except { left, right, all, estimated_rows, estimated_cost } => {
                let transformed_left = Box::new(self.transform_node_disable_indexes(*left)?);
                let transformed_right = Box::new(self.transform_node_disable_indexes(*right)?);
    ...
    fn generate_join_alternatives(&self, logical_plan: &LogicalPlan) -> Result<PhysicalPlan, PlanningError> {
    ...
    fn select_best_plan(&self, plans: &[PhysicalPlan]) -> Result<PhysicalPlan, PlanningError> {
        if plans.is_empty() {
            return Err(PlanningError::InvalidQuery("No plans generated".to_string()));
        }
        
        // Find plan with lowest cost
        let best = plans.iter()
            .min_by(|a, b| a.get_estimated_cost().partial_cmp(&b.get_estimated_cost()).unwrap())
            .unwrap();
    ...
    pub fn estimate_plan_cost(&self, plan: &PhysicalPlan) -> CostEstimate {
        self.cost_model.estimate_node_cost(&plan.root, &self.statistics)
    }
    ...
    fn create_unwind_logical_plan(&self, unwind_stmt: &crate::ast::ast::UnwindStatement) -> Result<LogicalPlan, PlanningError> {
    ...
    fn create_mutation_pipeline_logical_plan(&mut self, pipeline: &crate::ast::ast::MutationPipeline) -> Result<LogicalPlan, PlanningError> {
        ...
        if let Some(first_segment) = pipeline.segments.first() {
            ...
            let basic_query = crate::ast::ast::BasicQuery {
                ...
                return_clause: crate::ast::ast::ReturnClause {
                    ...
                    items: vec![crate::ast::ast::ReturnItem {
                        expression: crate::ast::ast::Expression::Variable(crate::ast::ast::Variable {
                            name: "*".to_string(),
            ...
            Err(PlanningError::InvalidQuery("Mutation pipeline requires at least one segment".to_string()))
        }
    }
}
...
impl<'a> IndexAwareOptimizer<'a> {
    ...
    fn extract_text_search_predicate(&self, expr: &Expression) -> Option<(String, String, String, f64)> {
        match expr {
            // Match: TEXT_SEARCH(doc.content, 'query') - standalone boolean predicate (Phase 4)
            Expression::FunctionCall(func) if func.name.eq_ignore_ascii_case("text_search") && func.arguments.len() >= 2 => {
                ...
                if let (Some((variable, field)), Some(query)) = (
                    ...
                    let min_score = if func.arguments.len() >= 3 {
                        self.extract_number_literal(&func.arguments[2]).unwrap_or(0.0)
                    } else {
            ...
            Expression::Binary(binary) if matches!(binary.operator, Operator::GreaterThan | Operator::GreaterEqual) => {
    ...
    fn select_best_plan(&self, plan: PhysicalPlan, _statistics: &Statistics) -> Result<PhysicalPlan, PlanningError> {
...
impl QueryPlanner {
    ...
    fn create_physical_plan_with_indexes(&self, logical_plan: LogicalPlan, _available_indexes: &[IndexInfo]) -> Result<PhysicalPlan, PlanningError> {
