...
fn print_statement(statement: &Statement, indent: usize) {
    match statement {
        ...
        Statement::Declare(declare_stmt) => {
            ...
            debug!("{}Variables: {}", get_indent(indent + 1), declare_stmt.variable_declarations.len());
        },
        Statement::Next(next_stmt) => {
            debug!("{}{}", get_indent(indent), "Next Statement");
            debug!("{}Has target: {}", get_indent(indent + 1), next_stmt.target_statement.is_some());
        },
        Statement::AtLocation(at_stmt) => {
            debug!("{}{}", get_indent(indent), "At Location Statement");
            debug!("{}Path: {}", get_indent(indent + 1), at_stmt.location_path.to_string());
            debug!("{}Statements: {}", get_indent(indent + 1), at_stmt.statements.len());
        },
        ...
        Statement::ProcedureBody(procedure_body) => {
            ...
            debug!("{}Variable definitions: {}", get_indent(indent + 1), procedure_body.variable_definitions.len());
            debug!("{}Initial statement:", get_indent(indent + 1));
            print_statement(&procedure_body.initial_statement, indent + 2);
            debug!("{}Chained statements: {}", get_indent(indent + 1), procedure_body.chained_statements.len());
        ...
        Statement::IndexStatement(index_stmt) => {
            ...
            match index_stmt {
                crate::ast::ast::IndexStatement::CreateIndex(create_idx) => {
                    debug!("{}CREATE INDEX: {}", get_indent(indent + 1), create_idx.name);
                },
                ...
                crate::ast::ast::IndexStatement::OptimizeIndex(optimize_idx) => {
                    debug!("{}OPTIMIZE INDEX: {}", get_indent(indent + 1), optimize_idx.name);
                },
        ...
        Statement::Let(let_stmt) => {
            ...
            for (i, var_def) in let_stmt.variable_definitions.iter().enumerate() {
                debug!("{}Variable {}: {} = [expression]", get_indent(indent + 1), i, var_def.variable_name);
            }
        },
    }
}
...
fn print_query(query: &Query, indent: usize) {
    match query {
        ...
        Query::WithQuery(with_query) => {
            ...
            debug!("{}WITH query with {} segments", get_indent(indent + 1), with_query.segments.len());
            // TODO: Implement full WITH query pretty printing
        }
        Query::Let(let_stmt) => {
            debug!("{}{}", get_indent(indent), "LET Statement");
            debug!("{}LET statement with {} variable definitions", 
                get_indent(indent + 1), let_stmt.variable_definitions.len());
        }
        Query::For(for_stmt) => {
            debug!("{}{}", get_indent(indent), "FOR Statement");
            debug!("{}FOR statement with variable '{}'", 
                get_indent(indent + 1), for_stmt.variable);
        }
        Query::Filter(stmt) => {
        ...
        Query::Unwind(unwind_stmt) => {
            ...
            debug!("{}UNWIND expression AS '{}'", 
                get_indent(indent + 1), unwind_stmt.variable);
        }
        Query::MutationPipeline(pipeline) => {
            debug!("{}{}", get_indent(indent), "Mutation Pipeline");
            debug!("{}Pipeline with {} segments", 
                get_indent(indent + 1), pipeline.segments.len());
            debug!("{}Final mutation action (REMOVE/SET/DELETE)", 
                get_indent(indent + 1));
        }
    }
}
...
fn print_call_statement(call_stmt: &CallStatement, indent: usize) {
    debug!("{}Procedure: {}", get_indent(indent), call_stmt.procedure_name);

    if !call_stmt.arguments.is_empty() {
        debug!("{}Arguments ({})", get_indent(indent), call_stmt.arguments.len());
...
fn print_yield_clause(yield_clause: &YieldClause, indent: usize) {
    debug!("{}Yield Items ({})", get_indent(indent), yield_clause.items.len());
    
    for (i, item) in yield_clause.items.iter().enumerate() {
        debug!("{}Item {}: {}{}", get_indent(indent + 1), i, item.column_name,
            if let Some(alias) = &item.alias { format!(" AS {}", alias) } else { String::new() });
    }
}

fn print_match_clause(match_clause: &MatchClause, indent: usize) {
    debug!("{}Patterns ({})", get_indent(indent), match_clause.patterns.len());
...
fn print_path_pattern(path_pattern: &PathPattern, indent: usize) {
    debug!("{}Elements ({})", get_indent(indent), path_pattern.elements.len());
...
fn print_return_clause(return_clause: &ReturnClause, indent: usize) {
    debug!("{}Items ({})", get_indent(indent), return_clause.items.len());
...
fn print_expression(expr: &Expression, indent: usize) {
    match expr {
        Expression::Binary(binary) => {
            debug!("{}BinaryExpression ({:?})", get_indent(indent), binary.operator);
        ...
        Expression::Unary(unary) => {
            debug!("{}UnaryExpression ({:?})", get_indent(indent), unary.operator);
            print_expression(&unary.expression, indent + 1);
        }
        Expression::FunctionCall(func) => {
            debug!("{}FunctionCall: {}", get_indent(indent), func.name);
            debug!("{}Arguments ({})", get_indent(indent + 1), func.arguments.len());
        ...
        Expression::PropertyAccess(prop) => {
            debug!("{}PropertyAccess: {}.{}", get_indent(indent), prop.object, prop.property);
        }
        ...
        Expression::Case(case_expr) => {
            ...
            match &case_expr.case_type {
                crate::ast::ast::CaseType::Simple(simple) => {
                    ...
                    debug!("{}WHEN branches: {}", get_indent(indent + 2), simple.when_branches.len());
                    for (i, branch) in simple.when_branches.iter().enumerate() {
                        debug!("{}Branch {}", get_indent(indent + 3), i);
                        debug!("{}WHEN values: {}", get_indent(indent + 4), branch.when_values.len());
                ...
                crate::ast::ast::CaseType::Searched(searched) => {
                    ...
                    debug!("{}WHEN branches: {}", get_indent(indent + 2), searched.when_branches.len());
        ...
        Expression::Cast(cast_expr) => {
            ...
            debug!("{}Target Type: {:?}", get_indent(indent + 1), cast_expr.target_type);
        },
        ...
        Expression::QuantifiedComparison(quantified_expr) => {
            debug!("{}QuantifiedComparison ({:?})", get_indent(indent), quantified_expr.quantifier);
            debug!("{}Left:", get_indent(indent + 1));
            print_expression(&quantified_expr.left, indent + 2);
            debug!("{}Operator: {:?}", get_indent(indent + 1), quantified_expr.operator);
            debug!("{}Subquery:", get_indent(indent + 1));
        ...
        Expression::IsPredicate(is_predicate) => {
            ...
            debug!("{}Predicate Type: {:?}", get_indent(indent + 1), is_predicate.predicate_type);
            debug!("{}Negated: {}", get_indent(indent + 1), is_predicate.negated);
...
fn print_from_clause(from_clause: &FromClause, indent: usize) {
    debug!("{}Graph Expressions ({})", get_indent(indent), from_clause.graph_expressions.len());
...
fn print_group_clause(group_clause: &GroupClause, indent: usize) {
    debug!("{}Expressions ({})", get_indent(indent), group_clause.expressions.len());
...
fn print_order_item(order_item: &OrderItem, indent: usize) {
    ...
    debug!("{}Direction: {:?}", get_indent(indent), order_item.direction);
...
fn print_transaction_statement(transaction_stmt: &TransactionStatement, indent: usize) {
    match transaction_stmt {
        TransactionStatement::StartTransaction(start_stmt) => {
            ...
            if let Some(ref characteristics) = start_stmt.characteristics {
                ...
                if let Some(ref isolation_level) = characteristics.isolation_level {
                    debug!("{}Isolation Level: {}", get_indent(indent + 2), isolation_level.as_str());
                }
                if let Some(ref access_mode) = characteristics.access_mode {
                    debug!("{}Access Mode: {}", get_indent(indent + 2), access_mode.as_str());
                }
            }
        },
        ...
        TransactionStatement::SetTransactionCharacteristics(set_stmt) => {
            debug!("{}{}", get_indent(indent), "Set Transaction Characteristics Statement");
            debug!("{}Characteristics:", get_indent(indent + 1));
