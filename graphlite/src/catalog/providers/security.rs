...
impl ACE {
    ...
    pub fn from_params(params: &Value) -> CatalogResult<Self> {
        let principal_name = params.get("principal_name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| CatalogError::InvalidParameters("Missing 'principal_name'".to_string()))?
            .to_string();
        
        let principal_type = params.get("principal_type")
            .and_then(|v| v.as_str())
        ...
        let resource_path = params.get("resource_path")
            .and_then(|v| v.as_str())
        ...
        let granted = params.get("granted")
...
impl SecurityCatalog {
    ...
    pub fn new() -> Box<dyn CatalogProvider> {
        ...
        admin_role.description = Some("Default administrator role with full system access".to_string());
    ...
    fn add_user(&mut self, user: User) -> CatalogResult<()> {
        if self.users.contains_key(&user.id.name) {
            return Err(CatalogError::DuplicateEntry(
                format!("User '{}' already exists", user.id.name)
            ));
        }
    ...
    fn add_role(&mut self, role: Role) -> CatalogResult<()> {
        if self.roles.contains_key(&role.id.name) {
            return Err(CatalogError::DuplicateEntry(
                format!("Role '{}' already exists", role.id.name)
            ));
        }
    ...
    fn remove_role(&mut self, name: &str, cascade: bool) -> CatalogResult<Role> {
        ...
        if !cascade {
            let users_with_role: Vec<_> = self.users.values()
            ...
            if !users_with_role.is_empty() {
                return Err(CatalogError::InvalidOperation(
                    format!("Cannot drop role '{}': assigned to users: {:?}", name, users_with_role)
                ));
            }
        }
    ...
    fn update_role(&mut self, name: &str, updates: &Value) -> CatalogResult<()> {
        let role = self.roles.get_mut(name)
            .ok_or_else(|| CatalogError::NotFound(format!("Role '{}' not found", name)))?;
        ...
        match storage.load_catalog_provider("security") {
            Ok(Some(data)) => {
                ...
                match self.load(&data) {
                    Ok(_) => {
                        log::info!("Security catalog loaded from storage with {} users, {} roles",
            ...
            Err(e) => {
                log::warn!("Error loading security catalog: {}. Using default initialization.", e);
            }
        }
            ...
            CatalogOperation::Query { query_type, params } => {
                match query_type {
                    ...
                    QueryType::GetAcesForResource => {
                        if let Some(resource_path) = params.get("resource_path").and_then(|v| v.as_str()) {
                            let aces = self.get_aces_for_resource(resource_path);
                            Ok(CatalogResponse::Query { 
                                results: serde_json::to_value(aces)?
                            })
                        } else {
                            Err(CatalogError::InvalidParameters("Missing 'resource_path' parameter".to_string()))
                        }
                    }
                    QueryType::GetAcesForPrincipal => {
                        if let Some(principal_name) = params.get("principal_name").and_then(|v| v.as_str()) {
                            let aces = self.get_aces_for_principal(principal_name);
                            Ok(CatalogResponse::Query { 
                                results: serde_json::to_value(aces)?
                            })
                        } else {
                            Err(CatalogError::InvalidParameters("Missing 'principal_name' parameter".to_string()))
                        }
                    }
                    QueryType::Authenticate => {
                        let username = params.get("username")
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| CatalogError::InvalidParameters("Missing 'username'".to_string()))?;
                        let password = params.get("password")
                            .and_then(|v| v.as_str())
                            .ok_or_else(|| CatalogError::InvalidParameters("Missing 'password'".to_string()))?;
                ...
                match query_type {
                    ...
                    _ => {
                        ...
                        Err(CatalogError::NotSupported(
                            format!("Query type {:?} not supported in read-only mode", query_type)
                        ))
                    }
                }
            }
